...
4. **不得修改 `specs/` 底下的任何檔案。**
   ...
   ⚠️ **這一條同時涵蓋根層的 `specs/`**（v1 對採用團隊的框架承諾，`v1.8.0` 起）。
   **在上游**，那批規格與切片的規格差別在：閘門會**照著它去執行**（`promise-check`），
   所以把一句「必須紅」改成「必須綠」是一行**合法而且全綠**的 diff —— 它把一條
   保證拿掉了，而看得出來的只有讀 diff 的人。

   **在 fork 裡**它們照樣跟著下發，但 `promise-check` 不接線（驗證承諾是上游的責任，
   C215 §九），章也不涵蓋 `specs/` —— 改了它們，沒有任何閘門會變色。**照樣不准改，
   理由換成歸屬**：那是上游寫給你們的承諾，不是團隊的需求；在 fork 裡改它，只會讓
   你們手上那份承諾與上游的分岔（C224）。這一半同樣沒有機制在守，靠的是你。
