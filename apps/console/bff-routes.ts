 ...
 * ── 這個檔案是什麼 ──────────────────────────────────────────────────
 *
 * 本機的 mock BFF 提供 session／CSRF／401／403 那條路徑，以及兩片示範切片的
 * 唯讀資料。**應用自己的端點補在這裡**，由 mock 啟動時載入；它會把載入的路由
 * 印出來 —— 那是唯一看得出「有沒有真的載到」的地方。
 *
 * ── ⚠️ 為什麼這裡一個 import 都沒有 ─────────────────────────────────
 *
 * 路由的型別屬於 mock，但這個檔案**刻意不 import 它**：mock 是開發工具，
 * 把它加進這個 app 的相依清單，等於讓一支「拒絕在 NODE_ENV=production 啟動」
 * 的伺服器進了應用的相依圖 —— 然後總有一天會有人從 `src/` 裡 import 它。
 * 正式環境的 app 不該知道 mock 的存在。
 *
 * 代價是下面那個 `DevRouteRequest` 要自己寫一份。路由的形狀由 mock 在啟動時驗，
 * 錯了會直接拒絕啟動並指出是第幾條。
 *
 * ── 開新案子時怎麼用 ────────────────────────────────────────────────
 *
...
export const routes = [
  {
    /**
     * `features/order` 的 `cancelOrder()` 打的就是這一條，而 mock 本身
     * 沒有它 —— 也就是說在補上這條之前，那支函式在本機**一定** 404：
     * 示範切片有一段程式碼在跑起來的應用裡根本到不了。
     *
     * ⚠️ 訂單列表是 mock 的唯讀示範資料，所以取消不會改變
 ...
 *
 * mock 的預設 session 只有示範切片的**唯讀**權限，
