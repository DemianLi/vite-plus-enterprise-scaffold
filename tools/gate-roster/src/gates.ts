...
export const UNGATED: readonly Ungated[] = [
  ...
  },
  {
    ...
    why:
      ...
      "⚠️ 這裡原本寫的是「#90 已經把它歸為正交」——寫下的當時 #90 是 OPEN、零留言，" +
      "那句話把一張待決票的**題目**當成了裁決。裁決是 C135 做的（2026-08-27）。" +
      "⚠️ 而 **C135 那張清單不是這裡在等的那一張**：它問「承載哪一條軸」，" +
      "這支閘門問「准不准在樹上」（C135 §五 判定兩者不是平行體系，正因為問題不同）。" +
      "所以上面那句「等 #90／#93」**至今沒有被兌現** —— 那兩張票都不生產" +
      "`main` 的範疇清單，談完也不會讓這支轉綠。⚠️ 這是量到的，不是裁決 ——" +
      "要不要為那張清單另開一張票，還沒有人判。" +
