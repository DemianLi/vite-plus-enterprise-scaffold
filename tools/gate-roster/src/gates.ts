...
export const UNGATED: readonly Ungated[] = [
  ...
  },
  {
    pkg: "scope-check",
    why:
      "**`main` 的範疇清單還沒定義，等 #180**。它比對的是 `SCOPE.md`" +
      ...
      "工具與 `SCOPE.md` 都留著、一個字都不改：**C135** 把 `SCOPE.md` 歸為**正交**" +
      "（機制不准改、但要報），而刪掉之後 #93 那道閘門要從零長。" +
      "⚠️ **而刪掉 `scope-check` 也不是乾淨的選項**：對 fork 這份腳手架的團隊，" +
      "它買到的「你們的交付樹長出東西的時候有人看見」一個字沒變（`SCOPE.md:8-11`）" +
      "—— C134 收回的是**上游那一半**的後果。" +
      "⚠️ 這裡原本寫「等 #90／#93」（#159 §四），而那兩張票**都不生產這張清單**：" +
      "C135 §五 判定它那張清單與 `SCOPE.md` 不是平行體系，正因為問題不同" +
      "（「承載哪一條軸」vs「准不准在樹上」）。那句話兌現不了，#180 是為此開的。" +
      "⚠️ 也曾寫過「#90 已經把它歸為正交」—— 寫下的當時 #90 是 OPEN、零留言，" +
      "把一張待決票的**題目**當成了裁決。裁決是 C135 做的（2026-08-27）。" +
