...
export const FACTS: readonly Fact[] = [
  {
    ...
    // ⚠️ 下面每一條的示意句子刻意寫 `N` 而不是當前值。第一版抄的是實際數字，
    ...
    citations: [
      ...
      // 後者是 2026-08-16 改套件數時才發現的 —— 它一直是一個沒人守的數字。
      /(\d+) 個套件全帶 sha512/,
      // ⬇ 以下三句是 2026-08-16 補的，見本檔「摘要表是最先被讀的」一節。
      // HANDOFF 摘要表第 5 列：「內部 registry 鏡像 **N 個**套件」
      /鏡像 \*\*(\d+) 個\*\*套件/,
      // HANDOFF 第 5–7 節的指令註解：「# N 筆，含 sha512，可直接餵給鏡像工具」
      /(\d+) 筆，含 sha512/,
      // HANDOFF 交付表：「N 筆鏡像清單，含 sha512」
      /(\d+) 筆鏡像清單/,
      // UI-SURVEY 的既有約束表：「供應鏈閘門（N 套件／144 原生二進位進版控）」
      /閘門（(\d+) 套件/,
    ],
  },
  {
  ...
  },
  {
     ...
     */
    source: "git ls-files platform/ui/src/components 底下的 .tsx 數",
    citations: [
