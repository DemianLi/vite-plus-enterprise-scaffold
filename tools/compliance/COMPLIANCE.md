...
- **§11 I** 前端只負責傳輸面：HSTS 與 Secure cookie 由 @org/security-headers 與 D8 契約守。靜態加密是 gateway 與資料庫的事。⚠️ csp-verify 刻意不列在這裡 —— CSP 不是加密，把它掛到這條會讓覆蓋看起來比實際好。
- **§11 II ①** 基礎設施。腳手架不提供，也不該假裝提供。
- **§11 II ②** 同上。前端無從觀測。
- **§11 II ③** 這一條有兩半：**檢測**由 Trivy 做（每日 21:00 UTC 排程 —— 沒有它，三個月沒人動的專案就三個月沒掃過），**因應**由 Renovate 提出（D13 的 critical 3 天／high 14 天 SLA 要有人真的去升，才是 SLA）。⚠️ 仍只涵蓋**相依套件**：自己寫的程式碼沒有 SAST，而 eslint 的安全規則不是漏洞掃描器。
- **§11 II ④** 端點與伺服器。
- **§11 II ⑤** 契約守的是 cookie 屬性、CSRF 與 401／403 路徑。密碼複雜度在 OIDC provider，腳手架驗不到 —— 那是 HANDOFF #8 交給 gateway 的部分。
- **§11 II ⑥** 現行有效、前端做得到、而一條檢查都沒有。bff-mock 用的是 DEMO_ORDERS／DEMO_SHIPMENTS，方向對，但沒有規則擋住有人塞真資料進 fixture。
- **§11 II ⑦** 治理面（切片邊界、platform API 表面）、程式碼面（XSS、eval）與執行期（CSP enforce 實測）三層。⚠️ 覆蓋是滿的，欠的是**證明** —— 四道裡只有 conformance 證明過會紅。
- **§11 II ⑧** 營運面。屬於上線後的維運流程。
- **§11 II ⑨** 現行有效、前端做得到、而一條檢查都沒有。適用的正是 apps/console 這種內部控制台畫面 —— 防的是客服與內部人員看到完整個資，不是防使用者看自己的資料。
- **§12 III** ⚠️ **這一列是「有閘門、閘門也會紅、法條仍然沒被滿足」的樣本。** conformance 守的是「CODEOWNERS 有條目」，法條要的是權限設定與定期檢視；而目前那 22 條全是 Unknown owner（HANDOFF #15）—— 存在不等於生效，本機看不出來，只有 GitHub 知道。
- **§15** 本表是那份「評估報告」機器可推導的那一半。人要做的另一半（每 12 個月看一次並簽名）還沒有任何機制承接。
- **§16** evidence.json 與 provenance.json 進了版控（git 歷史本身就是保存），但**沒有一份是為 §16 設計的**，也沒有寫下保存期政策。SBOM 目前是 90 天 artifact —— 差 5 年很遠。
- **§18** ⚠️ 門檻極低，中型 B2C 幾乎必然落入。目前沒有任何閘門帶 12 個月的 cadence。⚠️ exit-drill 的 120 天**刻意不列在這裡** —— 它檢討的是「退不退得回上游」，不是「安全維護計畫」。兩者都是每季，但掛上去會讓這條看起來已經有人在做。
...
- **`api-surface`** — `node tools/api-surface/src/cli.ts`  
  四道閘門裡唯一連零件測試都沒有的。基準線進版控，但沒有東西證明過比對會紅。
...
- **`trivy-sca`** — `aquasecurity/trivy-action（.github/workflows/tier2-security.yml）`  
  唯一直接對得上 §11 II ③ 的閘門，而且有兩個已知失明模式（C33 dev 相依被抑制掃 0 個、C34 只解第一份 YAML 文件），兩者都是綠燈。
- **`trivy-sbom`** — `node tools/supply-chain/src/cli.ts --verify-sbom sbom.cdx.json`  
  SBOM 上傳為 artifact（保留 90 天），沒有進版控 —— 而 §16 要的是 5 年。
...
- **§11 II ⑥** 測試環境應避免使用真實個人資料  
  現行有效、前端做得到、而一條檢查都沒有。bff-mock 用的是 DEMO_ORDERS／DEMO_SHIPMENTS，方向對，但沒有規則擋住有人塞真資料進 fixture。
- **§11 II ⑨** 隱碼機制，隱藏個人資料之呈現  
  現行有效、前端做得到、而一條檢查都沒有。適用的正是 apps/console 這種內部控制台畫面 —— 防的是客服與內部人員看到完整個資，不是防使用者看自己的資料。
- **§16** 軌跡資料與安全維護計畫執行證據，保存五年  
  evidence.json 與 provenance.json 進了版控（git 歷史本身就是保存），但**沒有一份是為 §16 設計的**，也沒有寫下保存期政策。SBOM 目前是 90 天 artifact —— 差 5 年很遠。
