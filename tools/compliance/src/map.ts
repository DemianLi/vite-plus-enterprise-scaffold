...
export const GATES: readonly Gate[] = [
  ...
  },
  {
    id: "pii-masking",
    kind: "gate",
    what: "切片宣告為個資的欄位，在 .vue 的渲染運算式裡必須走 @org/pii 的 maskXxx()",
    command: "node tools/pii-check/src/cli.ts --masking",
    evidence: null,
    negativeTest: "tools/pii-check/tests/masking.test.ts",
    note: "宣告住在切片契約的 `personalData`，而且是**必填**的 —— 空陣列是一個答案，沒寫則代表沒有人想過這件事，兩者必須長得不一樣。只讀字面陣列、不執行切片程式碼：一道法遵閘門不該執行被它稽核的東西，否則宣告可以是算出來的，review 就看不出這個切片碰了哪些個資。⚠️ 這是靜態層，它證明原始碼裡寫了 mask，不證明遮罩真的會遮 —— 後者由 features/*/tests 的元件測試證明（與 csp-verify 的兩層分工相同）。",
  },
  {
...
export const CONTROLS: readonly Control[] = [
  ...
  },
  {
    ...
    scope: "frontend",
    gates: ["conformance", "api-surface", "eslint-security", "sast", "csp-verify"],
    coverage: "full",
    owed: false,
  ...
  },
  {
    ...
    coverage: "partial",
    owed: false,
    note: "防的是客服與內部人員在日常作業畫面上看到完整個資，不是防使用者看自己的資料 —— 適用的正是 apps/console 這種東西。兩層：靜態層讀每一個 .vue 的模板，元件測試證明遮罩真的會遮（斷言的是**完整值在整份 HTML 裡不存在**，不是「遮罩後的字串有出現」—— 後者在沒有隱碼時也會過）。⚠️ 仍是 partial：它只守宣告過的欄位，而「哪些欄位算個資」是人填的。",
  },
  {
    article: "§12 III",
