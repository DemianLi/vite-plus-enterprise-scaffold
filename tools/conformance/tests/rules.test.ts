...
describe("要一棵目錄樹，但仍然不用起行程", () => {
  ...
  it("hook 檔名與匯出的函式名對不上 → 紅", () => {
    const root = tree({
      "features/order/src/hooks/useOrderList.ts": "export function useOrders() {}\n",
    });
    expect(rules(checkSliceLayering(join(root, "features/order"), SLICE))).toEqual(["hook 命名"]);
  });
