...
/**
 * D14：切片**內部**的分層 —— 元件只呈現，有狀態的邏輯住在 hook 裡。
 *
 ...
 *
 *   1. `src/hooks/` 底下的檔案必須叫 `useXxx.ts` 且匯出同名函式
 *   2. **`src/views/` 不得直接 import 資料層** —— 禁的是位置，不是相依
 *   3. **`src/store.ts` 不得 value import 資料層** —— store 存 id 不存 entity
...
export function checkSliceLayering(slicePath: string, slice: string): Finding[] {
  return collect((fail) => {
    const hooksDir = join(slicePath, HOOKS_DIR);

    if (existsSync(hooksDir)) {
      for (const entry of readdirSync(hooksDir)) {
        if (statSync(join(hooksDir, entry)).isDirectory()) continue;

        if (!isValidHookFile(entry)) {
          fail(
            slice,
            "hook 命名",
        ...
        const expected = hookFunctionName(entry);
        const source = readFileSync(join(hooksDir, entry), "utf8");
        if (
          ...
          fail(
            slice,
            "hook 命名",
            `${HOOKS_DIR}/${entry} 沒有匯出同名的 ${expected}`,
    ...
    // 客戶端是權威（篩選條件、選取的 id）→ store；伺服器是權威 → TanStack Query。
      ...
      for (const match of source.matchAll(IMPORT_SPECIFIER_PATTERN)) {
        ...
        if (forbidden !== undefined) {
          fail(
            ...
            "store 只放客戶端才是權威的東西（意圖、選取的 id）。伺服器狀態走 " +
              `${HOOKS_DIR}/use<Xxx>.ts（D14）。存進 store 等於做了第二份快取，` +
        ...
        if ((STORE_FORBIDDEN_LOCAL_MODULES as readonly string[]).includes(localModule)) {
          fail(
            ...
            `只借型別的話請寫成 \`import type\`（那完全允許）。真的要取數請放到 ${HOOKS_DIR}/`,
          );
        }
      }
    }
      ...
      for (const match of source.matchAll(IMPORT_SPECIFIER_PATTERN)) {
        ...
        if (forbiddenPackage !== undefined) {
          fail(
            ...
            `把取數搬進 ${HOOKS_DIR}/use<Xxx>.ts，元件只留呈現（D14）。` +
        ...
        if ((VIEW_FORBIDDEN_LOCAL_MODULES as readonly string[]).includes(localModule)) {
          fail(
            ...
            `資料層只准被 ${HOOKS_DIR}/ 使用。元件從 hook 拿已經整理好的資料（D14）`,
          );
        }
      }
    }
