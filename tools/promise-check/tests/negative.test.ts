...
describe("承諾綁到一道看不見副本的閘門 → 紅", () => {
   ...
   *
   * `tools/scope-check` 真的存在、也真的在 `scripts.gate` 上 —— 它只是
   * **忽略 `--root`**（C123 §一：這條線上的閘門一律靜默忽略不認得的旗標）。
   * 承諾綁到它的話，每一次「照規格弄壞副本」之後跑的都是**沒有被弄壞的真樹**。
   *
   * ⚠️ 少了這一格，那個症狀會長成〈承諾沒有牙齒〉—— 而那則訊息給的兩條
   * 修法（修閘門／改規格）都不是真正的原因，其中一條還能靠改規格變綠。
   *
   * ⚠️ `tools/scope-check` 哪天讀了 `--root`，這一條會安靜地不再守任何東西。
   * 那天是好消息，換一支還沒改的閘門就好 —— 但要記得換。
   */
  it("指名一支存在、在 gate 上、卻看不見副本的閘門", () => {
    expect(rules(patched("當 跑 tools/conformance", "當 跑 tools/scope-check"))).toContain(
      "閘門指不到副本",
    );
  });
});
