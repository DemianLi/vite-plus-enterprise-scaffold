...
| `style` **屬性**       | **被擋**         | `style-src-attr 'none'` 真的在 enforce（C245 之前期望「生效」） |
...
期望「被擋」的三項要說清楚：**「被擋」與「注入的程式碼根本沒跑」在觀測上
一模一樣**。所以判定要求同時有對應的 violation（`effectiveDirective` 對得上那一條），
而且 `disposition === "enforce"` —— 那是整份證據唯一機器驗得出「不是 report-only」的地方。

⚠️ C245 把探針二從「生效」翻成「被擋」之後，證明「CSP 不是把什麼都擋了」的
**只剩探針三**（外部樣式表有載入、`cssRules` 讀得到）。在那之前探針二也扛這一格。
...
靜態那一半（掃已發佈的套件，找出「有這個能力」的程式碼）與這支是**互補的**，
後者證明「執行期實際發生了什麼」。兩個都需要。C244 之前靜態那一半是 `tools/ui-survey --csp`
（掃候選元件庫）；它隨 Vue 退場刪了，現在由 `platform/ui/tests/base-ui-no-style.test.ts`
對實際裝上去的 Base UI 閉包逐檔掃。
