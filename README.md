...
它對採用團隊承諾四件事，每一條都由 [`specs/`](specs) 底下的規格逐條執行：
...
│   └── ui-survey/            UI-SURVEY.md 的資料來源（決策期工具，刻意不進閘門）
│
...
預設只接六道：腳手架的章、一致性檢查、platform API 表面檢查、.vue 型別檢查、
設計系統接縫、退出面檢查。其餘閘門在上游照跑，不下發給 fork。
...
|                       | 內容                                                                                                                                                                                                     | 何時跑                  |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| **Tier 1 — 品質**     | 閘門名冊一致 + SCOPE.md 與版控內容一致 + 腳手架的章 + oxlint + oxfmt + 型別檢查 + .vue 型別檢查 + 驗收規格完成率 + 無障礙靜態檢查 + 設計系統接縫 + 複雜度門檻沒有過期                                    | 本機、每次 PR           |
...
底下 36 個 workspace 套件全部是 `private`、不對外發佈，因此不逐一標註授權。
