...
> ⚠️ **凍結（C244，Q101）**：產生本文件的 `tools/ui-survey` 已隨 Vue 退場刪除，§六 那兩條
> `tools/ui-survey` 指令跑不起來了。選型後來也從 shadcn-vue 換成 React ＋ Base UI（C232、C233）。
> 本文件留作 D15 當時的依據，內容不再重跑、不再更新 —— 唯一的例外是 `doc-facts` 守著的那一格
> 套件總數，它跟著 lockfile 走。
...
| 供應鏈閘門（625 套件／146 原生二進位進版控）  | 每個新相依都要進盤點；新的**原生二進位家族**要 PR ＋ CODEOWNERS |
