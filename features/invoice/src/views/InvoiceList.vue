...
import { useInvoiceList } from "../composables/useInvoiceList.ts";
import { useInvoiceFilterStore } from "../store.ts";

/**
 * 這個元件**只負責呈現**（D14）。取數在 composables/useInvoiceList.ts。
 *
 ...
 */
const filter = useInvoiceFilterStore();
const { items, isPending, isError, error } = useInvoiceList(() => filter.query);
...
<template>
  <section>
    <h1 class="text-xl font-semibold text-fg">{{ $t("invoice.title") }}</h1>
    ...
    <p v-else-if="items.length === 0">{{ $t("invoice.empty") }}</p>
    ...
    >
      ...
      <template #close>
        <UiButton>{{ $t("invoice.close") }}</UiButton>
      </template>
    </UiDialog>
  </section>
</template>
