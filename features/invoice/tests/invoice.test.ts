...
describe("invoice 切片契約", () => {
  it("所有路由 name 落在自己的命名空間下", () => {
    for (const route of feature.routes) {
      expect(route.name).toMatch(/^invoice\//);
    }
  });

  it("所有權限碼落在自己的命名空間下", () => {
    for (const permission of feature.permissions) {
      expect(permission).toMatch(/^invoice:/);
    }
  });

  it("每個 locale 的 i18n 頂層 key 恰好只有 invoice", () => {
    for (const messages of Object.values(feature.i18n)) {
      expect(Object.keys(messages)).toEqual(["invoice"]);
    }
  });
