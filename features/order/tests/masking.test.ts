 ...
 *
 * ⚠️ **這支是 `platform/pii` 那個設計決定的舉證，不要搬也不要刪。** `platform/pii/src/index.ts`
 * 檔頭說「現在的形狀指得出來的是……畫面上實際渲染的結果」，指的就是它；而 `platform/pii` 刻意
 * 零框架相依（沒有 happy-dom、零 runtime deps），接不住這支。原本寫在切片 README，C250 把
 * README 改成只講現況時搬來這裡（C205 §一）。
 *
 * ── 這裡最容易寫錯的一條 ────────────────────────────────────────────
 *
