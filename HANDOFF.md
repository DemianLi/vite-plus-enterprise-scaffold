...
五條，每一條都有會失敗的檢查在守 —— **而每一條下面都寫著那道檢查刻意不看什麼。**

> ⚠️ 這句話在 2026-08-17 之前是假的：需求二當時沒有任何檢查，
...
前三段由 `platform/ui/tests/component-contract.test.ts` 守，
...
⚠️ **這一條在 v1.0.0 是「機制完整、內容很少」。** `platform/ui` 目前只有
`UiButton` 與 `UiDialog` 兩個元件 —— 接縫是通的、而且**兩個元件都被檢查過**，
但**能換的東西還不多**。補元件是 v1.x 的工作。

⚠️ **shadcn 沒有安裝，也不會安裝。** 這個 repo 用的是它的三個原料
（reka-ui ＋ clsx ＋ tailwind-merge）與它的模型（原始碼在自己手上），
但**不採用它對「各案怎麼換樣式」的答案**。它的答案是「你擁有原始碼，直接改它」，
而接案公司照做的結果是 20 個案子 20 份 `UiButton.vue`，上游修一個 bug 要同步 20 次。
具名槽存在的理由就是這一句：**樣式各案不同，原始碼只有一份。**
...
### 二、`platform/ui` 只有兩個元件

接縫（代幣／具名槽／slot）都通了、**兩個元件都被檢查器驗過**，
但實際能換的東西只有按鈕與對話框。要拼出一個完整版型，元件還要補。
