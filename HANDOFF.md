...
### 這個決策帶來的實作待辦（技術面，不需要組織裁決）

| #   | 事項                                     | 為什麼非做不可                                                                                                |
| --- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| 1   | `@tailwindcss/vite` 加進 `DRILL_PLUGINS` | 它改變建置產物。不登記的話退出演練會產出**沒有樣式的應用**然後回報 pass（C36）                                |
| 2   | 重跑 `vpr supply-chain --capture`        | +61 套件、+23 原生二進位。基線不更新，閘門會紅                                                                |
| 3   | 新增 `platform/ui` package               | CODEOWNERS、api-surface、退出演練的 alias 清單都要跟著加                                                      |
| 4   | `checkSliceLayering` 要管 `components/`  | 目前只管 `composables/`／`views/`／`store.ts`。最需要擋的是「切片自己複製一份元件」，那正好繞過 `platform/ui` |
| 5   | 瀏覽器實測 CSP                           | 市調的探測掃的是已發佈的 dist，證明「有這個能力」，**不證明執行期會發生**                                     |
