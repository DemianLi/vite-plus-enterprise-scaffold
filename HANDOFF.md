...
> 最後更新：2026-08-15。
>
> **16 項裡有 5 項已由技術面處理掉或降級**（4／7／9／12／14）—— 那幾項原本被歸成
> 「組織的事」，但其中有一半是技術能做的：把事實查到底、把成本量化、
> 讓工具在失敗當下自己講清楚。
>
> 剩下 11 項，其中 **第 15、16 項不是判斷題**，是還沒做完的事：
> 15 是採用這份腳手架的第一個步驟（把 `@org/*` 換成真的團隊），
> 16 是一件還沒寫的檢查。**真正只有人能裁決的是 9 項。**
>
> 第 15 項是 2026-08-15 查 #14 時撞到的。
> 第 16 項是同日做 C41 時撞到的，刻意沒有夾帶進那次改動：
> 它涵蓋 `@org/http-client`、`@org/slice-kit` 等所有內部套件，
> 只補 `@org/ui` 會讓規則讀起來比它解的問題還窄。
>
...
| 14  | 架構／設計   | ~~樣式策略選型~~ **已決策＋實作＋驗證（D15）**。剩：設計系統要不要獨立 owner | 破壞性變更被擋下來之後沒有收件人                      | ⬜           |
| 15  | repo 管理者  | **把 `@org/*` 換成真的團隊** —— 現在 22 條全是 Unknown owner                 | D12 的擁有權治理目前是一份文字檔，完全沒生效          | ✅           |
| 16  | 平台（實作） | 補「import 了但 `package.json` 沒宣告」的一般性檢查                          | 幽靈依賴 —— 本機建置得起來，退出演練與獨立發佈會爆    | ⬜           |
...
| 5   | 瀏覽器實測 CSP                           | ✅ 五個探針全對，結果進版控（`tools/csp-verify/evidence.json`），CI 守其有效期（C47） |
...
## 18. 平台（實作）— 文件數字守衛只涵蓋 5 個事實

**`tools/doc-facts` 目前登記 5 個事實。README 與 HANDOFF 裡還有別的數字沒有守。**
