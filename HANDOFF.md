...
| **形狀**（variant、尺寸）      | `defineProps` 的 union ＋ `createUiTheme` | 換整條 class 字串     |
| **結構**（哪一塊可以整組換掉） | `<slot>`                                  | 在使用端填 slot       |

⚠️ **這一條在 v1.0.0 是「機制完整、內容很少」。** `platform/ui` 目前只有
`UiButton` 與 `UiDialog` 兩個元件 —— 接縫是通的（下面第四條有實測證據），
...
接縫（代幣／variant／slot）都通了、都有閘門，但實際能換的東西只有
按鈕與對話框。要拼出一個完整版型，元件還要補。
