...
>
> **還沒做完的事**（15、16、17、18、21）—— 不是判斷題，是缺口。
> 其中 **15 是採用這份腳手架的第一個步驟**（把 `@org/*` 換成真的團隊），
> 其餘四項是腳手架自己知道、也寫下來的盲點。
>
...
| 2   | 資安         | 原生工具鏈的**政策性**例外（144 個二進位）                                   | SCA 判 fail，閘門過不了                               | ✅           |
| 3   | 資安         | 接受 43 個只有發佈簽章的佐證                                                 | 同上，且覆核時會被抓                                  | ✅           |
| 4   | 法務         | MPL-2.0 ＋ 22 個**完全無授權聲明**的套件                                     | 授權政策掃描標記，可能上線前才爆                      | ✅           |
| 5   | 平台／IT     | 內部 registry 鏡像 **563 個**套件                                            | CI 直接爆                                             | ✅           |
...
| 17  | 平台（實作） | `api-surface` 看不見**型別形狀**的破壞性變更                                 | 下游只拿到型別錯誤，沒有遷移路徑                      | ⬜           |
| 18  | 平台（實作） | 文件數字守衛的守備範圍仍不完整                                               | 沒被登記的數字仍會過期，而且沒有人會發現              | ⬜           |
| 19  | 法遵／IT     | §16 五年保存：**腳手架交出清單，保存排程是你們的**                           | 證據存在但沒進保存制度；`sbom.cdx.json` 90 天就沒了   | ✅           |
| 20  | 法遵／資安   | §11 II ⑨ 隱碼**現在是慣例，不是機制**（2026-08-16 拆掉閘門）                 | 新加的個資欄位忘了遮罩時，沒有任何東西會說話          | ✅           |
| 21  | 資安         | 開發期**源碼掃描（SAST）沒有接上**，只有 lint 那一半                         | 汙點傳遞類的問題要等廠商掃描才會發現                  | ⬜           |
...
1. `semgrep --test` 在找不到任何 fixture 時會印 `No unit tests found`
   ...
   這是這個 repo 一路在防的那個形狀，第八次。已加上防呆（`grep -q` 後 `exit 1`），
   所以 PR 現在是**正確地紅**。
2. 規則產生器讀的是 `Feature.personalData`，而那個欄位已隨第 20 項移除。

**這一項放進交接清單，不是因為要貴組織決定做不做，是因為要說清楚現況：**
弱點掃描與源碼掃描既然會請專業廠商協助，那麼腳手架這一側的 SAST
是「讓廠商報告上少幾條」的守衛，不是報告本身。**它現在不在防線上。**
廠商掃描的範圍與時程不受影響。
...
犯過七次錯（人抄下來的數字沒有人再推導一次），所以給外部團隊的數字一律由機器算。
細節見 `DECISIONS.md` 的 C17／C24／C25／C27／C31／C36，以及 §11 II ⑦ 那次註記。
守備範圍現在由 `tools/doc-facts` 覆蓋 README 與本檔（見第 18 項的限制）。
