...
⚠️ **而「`main` 那邊有」，這份文件已經不再替它擔保。**
需要其中任何一項的案子：去 `main` 查它**現在**長什麼樣，
**不要照這張表假設它在那裡**。

原本這裡寫的是「每一項在 `main` 都有完整實作（含閘門、反向測試與文件）」。
那句話拿掉了，因為它宣稱的是**另一條分支的當下狀態** ——
而 v1 這邊沒有任何機制看得到那件事，**也不該有**：跨分支的存在性檢查會讓
v1 的閘門相依於另一條分支的內容，那比一句過期的話更糟。

**它已經發作過兩次**（兩次都是同一個成因，`main` 動了而這張表沒有）：

| 哪一項                 | 發生什麼                                                                      |
| ---------------------- | ----------------------------------------------------------------------------- |
| 強制遮罩（§11 II ⑨）   | C52 移除，`main` **也沒有** —— 而〈誠實缺口〉第一條**從寫下的那天起就是假的** |
| **CSP 瀏覽器實測探針** | 伺服器與 `--print-probe` 還在，但 `--record`／`--verify` 與指紋證據檔沒了     |

通則見 `DECISIONS.md` 的 C102 §三：**排除一件事時寫下的必須是判準跑出來的
結果，不是它現在放在哪 —— 位置會過期，判準不會。**
...
⚠️ **這一列在 2026-08-22 查過一次**（`main` 有 `platform/eslint-config` 的
a11y 設定與 fixture、`tools/compliance` 的 `ACCESSIBILITY.md` 與產生器，
而且 a11y 那道 eslint 在 `main` 的 `scripts.gate` 裡）—— 但**那是那一天的
狀態，上面那段話同樣適用**。這裡不列其餘各項的查證結果：寫下來就是
再造一批會過期的位置斷言，而這一節剛拆掉的就是那個。
...
> 由 C52 移除，連同切片契約那個必填的 `Feature.personalData`。
...
>
> ⚠️ **這裡的日期對過 git，不是抄那三處文件的。** 兩個 commit 都在
> `origin/main`、都是 **2026-08-16**：`2721ba8` 建立（「§11 II ⑥⑨ …
> 欠的紅字 3 → 1」），`68fce63` 移除（「每次成本乘上觸發頻率太貴」）。
> **那道閘門的壽命是同一天** —— 這不是後來才發現太貴，是做出來當天就發現。
