...
>
> 第 15 項是 2026-08-15 查 #14 時撞到的：它不是判斷題，是**採用這份腳手架的第一個步驟**。
>
> 第 16 項是同日做 C41 時撞到的，也不是判斷題 —— 它是一件**還沒做的實作**，
> 只是刻意沒有夾帶進那次改動：它涵蓋 `@org/http-client`、`@org/slice-kit` 等
> 所有內部套件，只補 `@org/ui` 會讓規則讀起來比它解的問題還窄。
>
