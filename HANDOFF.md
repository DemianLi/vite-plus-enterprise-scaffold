...
2. **版控的那七份跟著 repo 走。** git 歷史本身就是保存機制，只要 repo 活著
   就超過五年 —— 但 repo 被刪、歷史被重寫（force push、filter-repo），
   證據就沒了。要不要備份、備份到哪，是你們的決定。

3. **這七份要不要寫進安全維護計畫。** §16 保存的是「落實執行**安全維護
   計畫**之證據」—— 如果計畫裡沒有提到這些閘門，這些檔案在法律上就不是
   那個「證據」。**先有計畫，這張清單才有法律意義。**

## 20. 資安 — semgrep 在本 repo 的供應鏈論證之外

**`semgrep` 是 Python 工具，以 pinned container image 進 CI —— `tools/supply-chain`
的盤點看不到它，SBOM 裡也不會有它。**

這與 happy-dom 那次不同（C48）：那次是量得到的（原生二進位 144 → 144，
只是多了純 JS 套件）。這次是**整個在盤點範圍之外** —— 沒有 integrity、
沒有 provenance、不進 mirror 清單、封閉網路要另外處理。

**要決定的**

1. **信任那個 image 嗎。** 目前釘在 `semgrep/semgrep:1.145.0`。
   升版是一次要有人看的變更，而 Renovate 管不到它（它不在 lockfile 裡）。
2. **封閉網路怎麼辦。** `vpr airgap` 的清單不含它。要嘛把 image 拉進內部
   registry，要嘛在封閉環境裡放棄這道閘門 —— 後者是可以接受的選項，
   因為**它本來就不是要交給稽核的那份 SAST 報告**。
3. **與廠商掃描的分工寫清楚。** 這裡的規則是開發當下的守衛，
   目的是讓廠商報告上少幾條。兩者的發現要不要合併去重，是流程問題。
