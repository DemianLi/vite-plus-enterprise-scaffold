...
### C70 — 設計系統的第二條軸只做在一個元件上，而檢查寫死了那個元件的檔名（2026-08-17）
...
### C71 — 註解規範進 AGENTS.md：一條明知**不可驗證**所以不設閘門的規範（2026-08-18）
...
### C72 — 收回一條寫在程式碼裡的規則：「空形狀等於沒有守」守錯了東西（2026-08-17）
...
這與 C70 之前那道「原始碼裡出現 `defineEmits` 就丟例外」是**同一種錯**：
...
### C73 — 閘門底下那一層抽成 `tools/gate-kit`：三個 export，而不是四個（2026-08-18）
...
### C74 — 閘門名冊接到 `main`：同一個缺陷的**第三次**發作，以及這裡多出來的三種閘門形狀（2026-08-18）

`tools/gate-roster` 原本寫在 `release/v1`（那條線的 C71）。依 v1 的範疇判準
...
這個缺陷 C71 抓到過（那是第二次），而 `v1.0.2` **只修了 `release/v1` 上的那一份**
...
| `gate-kit`     | `UNGATED`         | **它是 library**（C73），沒有 cli.ts，壞了會讓消費它的閘門紅           |
...
#### ⚠️ 這個 C74 不是 `release/v1` 的 C74

兩條線的 C 編號從 C70 起就分岔了（`main` 的 C70 是〈第二條軸只做在一個元件上〉，
v1 的 C70 是〈承諾有五條，檢查只有四條〉）。v1 那邊的 C71 是**這件事的前身**、
C72 是**範疇判準**；`main` 這邊 C71 起是另一串。
