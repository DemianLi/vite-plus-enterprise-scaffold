...
> # ⛔ 這一卷到 **C138** 為止 —— C139 起寫在 [DECISIONS-2.md](DECISIONS-2.md)
>
> **凍結於 2026-08-29，裁決是本檔最後一則 C-條 C138**（量測基準 `1c39132`）。
>
> ⚠️ **這不是搬遷** —— 這一卷的內容一個字都沒有搬走，**C1–C138 全部留在這裡**，
> 指向它們的約 2,700 處交叉引用因此一處都不用動。檔名也刻意不改：
> 樹上有數十個檔提到 `DECISIONS.md` 這個檔名，改名會讓每一處指向錯的一卷
> （帶基準 commit 的數字在 C138 §三）。
>
> ⚠️ **要註記 C1–C138 的某一則，寫在新一卷裡**，這裡最多加一行指標。
> ⚠️ **不熟這份日誌？先讀第二卷的 C140** —— 三串編號的體例、兩卷的分界點、入門序列（#189）。
>
> ⚠️ 下面那段開頭寫的「D1–D14 為定案決策；C1–C35 是……」是**當時的**範圍描述，
> 不是定義。凍結時的實際範圍是 **C1–C138（C51 從未使用）、D1–D16、R1–R9**。
> 原文不改，沿用本檔慣例（留原文、加標註）。
> ⚠️ 體積與行數刻意不寫在這裡 —— 那個數字在 C138 §二 標著基準 commit。
...
> ⚠️ **迭代軸那格的「上游」是代理，判準是「對象在外 ＋ 壞法安靜」——讀法見 C142**（第二卷；舊卷加這一行的依據是 C138 §四「舊卷最多加一行指標」）。
...
>
> ⚠️⚠️ **而 2026-08-27 起，變的是它還有沒有後果 —— 沒有了**，見 **C136 §三**。
> 兩條線併回一條之後，「送 `main`」＝「留在原地」，這條判準退化成分類。
> 原文一樣不改，理由一樣：它擋下 `gate-roster` 那一次真的發生過，而且是對的。
>
> ⚠️⚠️ **這一則漏了一半，而那一半是 C136 的內容**：它推翻了「不服務五條承諾
> 就出局」，卻**沒有回頭問前一天 `49b36da` 依那條判準砍掉的那一批該怎麼辦**。
> 那批東西隨併線原封回到樹上，十天後才被看見。C136 §八 因此立了一條書寫規則：
> **推翻一則 C-條時，要寫明它產出過什麼、重跑了沒有。**
...
### C90 — 記號：分開「量過的」與「查規格來的」（2026-08-20）

> ⚠️ **編號跳過 C75–C89。** 那一段在 `release/v1` 上（元件那條線），會隨交付線
> 併上來。這一條刻意取在它們後面，避免兩邊各長一個同號的條目。

#### 一、這條規矩是被什麼逼出來的

`release/v1` 的 C89 在體檢一條慣例時，掃出三個關於**可及名稱計算**的宣稱：
一個是假的、一個當理由用是假的、一個是規格灰區。三個的共同點不是「寫錯了」，
是**這個 repo 結構上量不到它們** —— page JS 沒有算可及名稱的 API，
`computedName` 只存在於 DevTools protocol。

而本 repo 的規矩是「**寫了論證就要去量它**」（C86 §B4，之後在 C88 §四／§五／
§五之二 連續用三次推翻自己寫的論證）。那條規矩很好，但它**沒有替「量不到的
東西」留位置** —— 於是查規格查來的宣稱長得跟量過的宣稱一模一樣，讀的人分不出
哪一句背後有實測。三個宣稱在同一份檔案裡活了一整版沒被撞到，就是這麼來的。

#### 二、記號

    ⚠️ **規格來源，本 repo 量不到**

標在宣稱的**旁邊**，不是檔頭 —— 它修飾的是那一句，不是那份檔案。

**什麼時候該標：** 這句話的依據是規格文字、上游原始碼的閱讀、或別人的實測，
而本 repo 的閘門形狀**做不到**去驗它。

**什麼時候不該標：**

- 量得到卻沒去量 → 那不是邊界，是還沒做。**去量**，或寫成待辦。
- 量過了 → 那就寫量到了什麼、怎麼量的。這個 repo 對這種寫法已經有一整套
  （E1／E20／E21 那類編號實驗、變異表）。
- 整份檔案的能力邊界 → 那是檔頭的事（`theme-verify` README 那句「綠燈的意思
  是配色與形狀實測可換，不是設計系統可換」是既有的範例）。

#### 三、⚠️ 它不是免責聲明

標了記號**不代表那句話可以留著含糊**。記號要求的是三件事同時出現：宣稱、
來源（哪一份規格的哪一段、哪一個檔案的哪一行）、以及**量不到的原因**。
只寫「規格來源」而不說為什麼量不到，下一個人沒辦法判斷那個限制還在不在 ——
而閘門形狀是會變的（C86 就替 `platform/ui` 開了 happy-dom 那條路，
在那之前「DOM 行為量不到」是真的）。

#### 四、⚠️ 沒有守到的

**沒有閘門。** 沒有東西會在「一句沒量過的宣稱沒有標記號」時說話 ——
這是一條 review 期的規矩，同 C81 那條「預設值該寫在哪」的處置
（「這不是加閘門解得掉的，是要進 review 清單的」）。
...
### C118 — 閘門讀規格，而抄本與唯一來源只差一件事：**誰會被執行**（2026-08-23）

C117 §四 判了層 3 框架承諾這一格「規格必須是唯一來源」，並且明說那把它從
「寫文件」變成「重寫閘門」。這一則是那句話落地的結果，以及**範圍為什麼只有
一條承諾**。
...
| 東西                                   | 是什麼                                                     |
| -------------------------------------- | ---------------------------------------------------------- |
| `specs/promise-1-architecture.feature` | 承諾一的原文，中文 Gherkin，**人逐字讀**                   |
| `tools/promise-check`                  | 讀那份規格，**照著它把一份切片副本弄壞、跑閘門、比對結果** |
| `scripts.gate` ＋ Tier 2 多一步        | 它是閘門，不是報表                                         |

跑一次 0.7 秒，四個場景各建一份沙盒、各跑一次 `tools/conformance`。
⚠️ **repo 的原始碼一個位元組都沒被動到** —— 破壞發生在暫存目錄的副本上，
形狀沿用 `tools/conformance/tests/negative.test.ts`（改副本，不改 repo）。

#### 二、判別式：規格裡必須有一樣東西是閘門推導不出來的

`SCOPE.md` 開頭警告的是「加一份清單而不加守它的機制 ＝ 第五份手抄本」，
而一份描述閘門行為的規格正是那個東西。判別方式只有一條：

> **規格裡有沒有一樣東西，是任何閘門都推導不出來的？**

有，而且它今天只活在散文裡：**承諾 → 守它的那道閘門**這個綁定。
`HANDOFF.md`〈v1.0.0 承諾什麼〉那幾張「守它的／守什麼」表，在此之前
**沒有任何機制在斷言它是真的** —— 那份文件自己寫著這句話，逐字是
「承諾與閘門對不對得上，只有人讀得出來 —— 這是這份文件最脆弱的一格」。

所以 `promise-check` 驗的三件事裡，最實在的一件不是「弄壞了會不會紅」，是
**規格指名的那道閘門必須真的接在 `scripts.gate` 上**。`tools/spec-report`
是現成的反例：它存在、有 CLI、跑得動 —— 但它接在 `vpr ready` 的最後一步，
不在閘門鏈上。承諾綁到它，那條承諾在 CI 上是空的，而「這支工具存在嗎」
這種靜態比對看不出差別。

#### 三、反向測試要兩個方向，而第二個方向才是重點

「把承諾改成假的 → 紅」只證明它讀了那句話。**最常見的失效模式不是說謊，
是沒被讀到**：規格檔改名、場景被刪、「假設」句改寫之後接不上。三種的共同
症狀是**閘門全綠而承諾一條都沒驗**。

這條學費這個 repo 付過兩次，兩次都寫在原始碼裡：`tools/doc-facts` 的
`unguarded`（清空 `citations` 讓閘門靜默解除武裝，C97）、`tools/spec-report`
的第四態 ❓ 未執行（測試全綠而場景一條都沒跑，C115）。

所以接線是**雙向**的：規格有而接線沒有 → 紅；**接線有而規格沒有 → 也紅**。
刪掉一個場景，那條破壞手法就變成孤兒，當場說話。

⚠️ 還有第三個方向，它同樣不是裝飾：規格裡**必須有一條期待綠燈的對照組**
（「沒有人違規的時候，這道邊界不得說話」），而且它**先跑**。沙盒建壞掉的話
每一條「必須紅」都會成功變紅 —— 那時閘門全綠，而原因是環境壞了。
把那條對照組改成「必須紅」，閘門會報〈沒有對照組〉。

#### 四、接線檔是 `.ts`，而這一格沒有因此白做

#135 的警告是「如果最後產出的是另一份 `.ts` 常數檔，這一格就白做了」。
產出的確有一支 `.ts`（`src/breakage.ts`，把中文的「假設」句接到一段破壞動作）。
它不是那個東西，判別方法很直接：

- 把 `breakage.ts` 整份刪掉 —— `specs/` 仍然說得出 v1 承諾了什麼。
- 把 `.feature` 刪掉 —— 剩下一堆沒有人知道為什麼存在的破壞手法。

分工與 `slice-gen` 產出的接線檔（`tests/specs/*.spec.ts`）完全一樣，而那個
形狀在 C114 就被接受過：**人讀的是 `.feature`，接線越薄越好，且不得包含承諾。**
鍵要與規格的句子**逐字相同**，寬鬆比對換來的是「改了規格但閘門沒感覺」。
...
| 閘門                           | 接不接得上「照規格弄壞一份副本」                                        |
| ------------------------------ | ----------------------------------------------------------------------- |
| `tools/conformance`            | ✅ 有 `--root`，判定純讀檔案，這一則用的就是它                          |
| `tools/api-surface`            | 讀進版控的基準檔；沙盒裡沒有基準，得先想清楚「副本的基準」是什麼        |
| `tools/vue-typecheck`          | 要完整的型別環境與 `node_modules`，副本裡沒有                           |
| `tools/theme-verify`           | **真的建置兩次**。技術上接得上，代價是每個場景多幾十秒                  |
| `tools/doc-facts`              | 吃的是整份 README／HANDOFF，副本裡那兩份文件不存在                      |
| `tools/scope-check`            | 事實來源是 `git ls-files`，副本不是 git repo                            |
| `eslint`／`.semgrep`／gitleaks | 不是 `tools/*/src/cli.ts` 的形狀，`.semgrep` 還跑在釘住 digest 的容器裡 |

所以擴張的下一步**不是再寫四份規格**，是讓更多閘門能在一份副本上跑 ——
而那是各自獨立的工作，硬塞進這一則只會生出四份接不上的規格檔，
那正是 §二 要擋掉的東西。

四條承諾的處置寫在這裡，不另開票以外的地方重述：**承諾二／三／四**都落在
`theme-verify` 與 `platform/ui/tests/component-contract.test.ts`，前者的代價
是時間、後者根本不是 CLI 閘門（是一支 vitest），兩件事都要先解決；
**承諾五**的守衛有三個在 `tools/` 之外，形狀差最遠。

#### 六、代價與還沒做的

- **`HANDOFF.md` 那句「這是這份文件最脆弱的一格」只被改掉一部分**，
  而改寫時刻意寫成「守到一條，還差四條」而不是拿掉它。⚠️ 一份文件宣稱
  自己被守住的程度，比它實際被守住的程度高 —— 那正是這一整格要治的病。
- **新增了根層 `specs/` 目錄**，所以 `SCOPE.md` 根層表要有一列（C117 §六
  對元層那格寫過同一句：檔案一進版控就要有一列，那是該格的工作）。
- **`promise-check` 與 `spec-report` 都讀 `.feature`，路徑刻意不重疊。**
  混在一起的話框架承諾會被算進**業務功能完成率**，而那份報表是拿去對外
  報進度的。分界有測試守著（真的跑一次 `spec-report --check`），不靠人記得。
- **`CODEOWNERS` 多一列 `/specs/`。** 把「必須紅」改成「必須綠」是一行合法的
  diff，閘門會照著新的那一句去驗而且全綠 —— **只有人看得出它把一條保證拿掉了。**
- 剩下三格（層 1 複雜度、層 2 覆蓋率下限、元層機械化）一個都還沒做，
  地圖仍是 #127。

### C119 — 層 1 複雜度：門檻是這棵樹的形狀，而量它的工具與執行它的工具不是同一個（2026-08-23）

C117 §二 判了這一格落 `release/v1`，論證是「規則隨 `platform/eslint-config`
**下發**」。#129 把四個維度量了一遍。這一則是設定真的落到樹上的結果 ——
而**那句下發的論證在動手第一步就不成立了**，換了一個更強的。

#### 一、落點：不是 `platform/eslint-config`，而且理由不是取捨

票上那個「真正的開放問題」問的是：複雜度塞進 Tier 2 安全閘門會不會稀釋它的
身分。答案比預期乾淨 —— **不必塞**：

| 維度       | 規則                     | oxlint 1.77 |
| ---------- | ------------------------ | ----------- |
| 函式大小   | `max-lines-per-function` | ✅ 有       |
| 巢狀深度   | `max-depth`              | ✅ 有       |
| 參數個數   | `max-params`             | ✅ 有       |
| 循環複雜度 | `complexity`             | ✅ 有       |
| 認知複雜度 | —                        | ❌ 沒有     |

四條裡有三條加一條替代品**已經在 Tier 1 的工具裡**，而 Tier 1 正是
`TESTING.md` §三 說的層 1（`vp check` ＝ Oxfmt／Oxlint／型別）。同一節還寫著
「**`eslint.config.js` 不在這一層**」。所以複雜度放 oxlint 不是繞路，是回到
它本來的層。連帶：

- **「與 oxlint 零重疊」那句話不用重驗** —— `platform/eslint-config` 裡一條
  複雜度規則都沒有，往 oxlint 加不會製造重疊。反過來才會。
- **紅燈分得出身分**。塞進 Tier 2 的話，一條紅線同時可能是「有 XSS」與
  「函式太長」。
- **`scripts.gate` 一個字都不用改** —— `vp check` 已經在 `ready` 的第一步。

⚠️ **但 oxlint 的設定不在 `.oxlintrc.json`，在根層 `vite.config.ts` 的 `lint:`
區塊。** 這條線根本沒有 `.oxlintrc.json`（動手時建了一個，`--print-config`
證明它**完全沒被讀到** —— 那個「設定寫了但沒生效」與全綠長得一樣，是 C115
第四態的又一個實例）。

#### 二、那句受益者 —— 換掉的論證比原本那個強

C117 §三 的形狀在這裡原樣重演：規則加進**既有的**檔案、不新增目錄，
`tools/scope-check` 根本不會問。**閘門不問不等於判準通過**，所以那句話要自己寫：

> **`vite.config.ts` 裡已經有一條 `files: ["features/*/**"]` 的 override**
> （D4 邊界防護第 2 層，擋跨切片 import）。這個檔案本來就在治理專案組寫的
> 程式碼 —— 不需要另外論證它「下發」得到，同一個檔案已經在做這件事。

⚠️ **而它有一句誠實版本，必須一起寫下來**：下面那些門檻是從**這棵樹**量出來的。
「不擋任何既有程式碼」這句承諾的射程**只到這個 repo**。採用團隊 clone 之後
寫的 `features/*`，門檻對他們而言是別人家樹的形狀。這不是缺陷、不用修 ——
但漏掉它，這一則就在宣稱一個沒有驗過的性質。

#### 三、門檻：取觀測最大值，而不是「max ＋ 一個餘裕」

定法是既定的（`TESTING.md`〈校準〉，C108 付過反例的代價）：**第一次先設寬到
不擋任何既有程式碼。** 落到具體數字上還差一步 —— 「不擋」是個下界，
max ＋ 10 也滿足。取 max 本身的理由是：**任何 +N 都是沒有論證的數字，
而 max 有 —— 它是這棵樹此刻的形狀。**

| 規則                     | 產品碼                | 測試碼 |
| ------------------------ | --------------------- | ------ |
| `max-lines-per-function` | **185**（＋一個放行） | 455    |
| `max-depth`              | 5                     | 3      |
| `max-params`             | 6                     | 4      |
| `complexity`             | 36                    | 11     |
| `vue/max-props`          | 5                     | 2      |

**兩類分開設**是 `TESTING.md` §六 已經承諾的事，而它不是為了寬鬆：測試碼在
巢狀深度與循環複雜度上比產品碼**乾淨得多**（89% 的測試函式 depth 0），
卻在函式大小上有一條產品碼沒有的長尾。同一組數字會在一個維度太鬆、
另一個維度把人卡死。

⚠️ **唯一一個 per-file 放行：`tools/slice-gen/src/files.ts` 的 841 行。**
不把全域門檻抬到 841 的理由是它是次高值（185）的 4.5 倍 —— 抬上去等於這條
規則對其餘所有產品碼形同不存在。孤立的極端值用放行隔離，連續分佈用觀測
max，兩者都滿足「不擋任何既有程式碼」，但只有前者留下一條還在守東西的線。
**那一行是債不是豁免**：它擋下的第一件事就是收緊的論證起點。

#### 四、認知複雜度那一格是空的，而空著比補起來好

oxlint 1.77 沒有這條規則 —— `cognitive-complexity`、`sonarjs/cognitive-complexity`、
`oxc/cognitive-complexity` 三種寫法逐一用 `--print-config` 驗過，都不存在。
唯一的來源是 `eslint-plugin-sonarjs`，而那要嘛把一條相依加進 Tier 2 安全閘門
（正是第一節說不必做的事），要嘛為了一個維度另養一軌。

⚠️ **`complexity` 是替代不是填滿，而 #129 §五 已經付好論證**：`buildSliceFiles`
841 行，**認知複雜度 0**。四個維度不互為代理，而這一格是最乾淨的實例 ——
用循環複雜度當認知複雜度的代理，會漏掉的正是那一類。

#### 五、反向測試：三個方向，加上「閘門真的會紅」那一格

| 做什麼                      | 期待             | 實測                                     |
| --------------------------- | ---------------- | ---------------------------------------- |
| 產品碼 `max-depth` 5 → 4    | 紅               | ✅ `doc-facts/src/facts.ts` 兩處，exit 1 |
| 測試碼 `complexity` 11 → 10 | **只有測試碼**紅 | ✅ `platform/ui/tests/styles.test.ts`    |
| 拿掉 `slice-gen` 的放行     | 紅               | ✅ `files.ts:18` 841 行，exit 1          |
| 還原                        | 全綠             | ✅ 五條規則零違規                        |

⚠️ **第四列不是裝飾。** 前三列證明規則會說話，只有第四列證明它**此刻不擋任何
東西** —— 這一票的到達條件就是那句話。

⚠️ **還有一個對照組，它守的是一句已經印出去的承諾。** C114 寫著「`slice-gen`
產出的切片**開箱即綠**」，而這一版換掉了它跑的規則 —— 樹上零違規不等於
**新產生的**切片零違規，那些檔不在版控裡，`vpr ready` 一次都沒看過它們。
實測：走真正的入口 `bin/index.ts` 產一片，正式門檻下**複雜度零違規**
（唯一的紅是 `tsconfig-error`，因為新切片還沒 install，與這一版無關）。

⚠️ **那個零自己也要對照組** —— 把門檻壓到 0／1 重跑同一個目錄，**命中 36 條**，
證明規則確實在看那些檔。新切片的實際最大值是函式 72 行、depth 1，離門檻很遠。
沒有這一步，「規則沒在跑」與「切片很乾淨」長得一模一樣。

⚠️ **而「說話」與「讓 `vp check` 非零」是兩件事**，所以絆線在 `vp check` 上
又跑了一次：門檻 −1 → `Found 2 errors`、**exit 1**；還原 → `Found 0 errors and
3 warnings`、**exit 0**。那三個 warning 是既有的（`unicorn/no-useless-spread`
等），與這一版無關。

#### 六、量測的述詞與執行的 glob 是兩份手抄本 —— 已經對過

C118 的形狀：**閘門與描述閘門的東西各寫一份，今天全綠證明不了它們等價。**
這裡有一模一樣的一對 —— 量測用 Python 述詞判「什麼是測試碼」，設定用 glob 判：

| 定義   | 內容                                                                              |
| ------ | --------------------------------------------------------------------------------- |
| Python | `/tests/`、`tests/` 開頭、`/fixtures/`、**`/__tests__/`**、`*.test.*`／`*.spec.*` |
| glob   | `**/tests/**`、`**/*.test.*`、`**/*.spec.*`、`**/fixtures/**`                     |

`__tests__` 在前者、不在後者。**驗法不是讀，是跑**：把測試碼那條 override 的
`max-lines-per-function` 壓到 1、產品碼抬到 99999，回報的檔案集合就是 glob
判定的測試集合，拿去跟 Python 的集合對 —— **各 53 個檔，雙向差集為空**
（另查 `git ls-files | grep __tests__` 為 0，所以那是空差異）。

**glob 是唯一定義**，Python 那份只是量測台的一次性述詞，不流傳。

⚠️ **這件事重要，是因為 glob 打錯不會報錯。** 把 `files` 指到一個不存在的路徑，
oxlint **靜默 exit 0**（絆線第三列就是這樣造出來的），症狀是**測試碼安靜地
套用產品碼門檻**。而 `max-lines-per-function` 是唯一一個**產品碼比測試碼緊**
的維度（185 < 455），所以那個靜默失效有真的咬人的方向。

⚠️ **同一個工具的兩個介面在這件事上給不同答案**：設定檔裡規則名打錯會報
`Rule 'max-dept' not found in plugin 'eslint'` 並 exit 1（好消息）；
CLI 的 `-D 不存在的規則` 則**什麼都不說**。判「這條規則到底存不存在」不能用
CLI 的沉默，要用 `--print-config` 看它有沒有出現在輸出裡 —— 上面第四節那三種
認知複雜度寫法就是這樣驗掉的。

#### 七、代價與還沒做的

- **`.vue` 的盲點原樣存在。** `UiDropdownMenu.vue`（script 275 行）、
  `UiDatePicker.vue`（186 行）在這五條規則下**零違規** —— 與 #129 §六 用
  ESLint 量到的完全一致，不是 oxlint 特有的問題。`vue/max-props` 補回
  「參數個數」那一格（實測與 #129 補量逐檔吻合：`UiAlertDialog` 5、
  `UiButton` 4、`UiDropdownMenu` 3、`UiField` 3）；**區塊行數那一格仍然空著**
  —— oxlint 沒有 `vue/max-lines-per-block`。
- **行數含空行與註解**（oxlint 沒有 `skipComments` 選項）。這條線的註解密度
  遠高於一般專案，函式內的註解直接灌進 `max-lines-per-function`。收緊那條
  門檻時，這個慣例要一起算進去。
- **量測範圍與執行範圍差一個檔**：`.semgrep/rules.ts` 在分佈裡被排除（故意寫壞
  的 SAST fixture），執行時沒有排除。查過了，它的值全部 ≤5 行／複雜度 1／
  參數 1，遠低於任何門檻 —— 不改設定。
- **層 2 覆蓋率下限的第二類那半已由 C120 交付，第一類那半與元層機械化還沒做**，地圖仍是 #127。

### C120 — 層 2 覆蓋率下限：門檻收在規格打的那一層，而放它的位置由「哪裡放了會失效」決定（2026-08-24）

C117 §二 把這一格判成**分裂**：第二類那組（`features/*`、`apps/*`）落 `release/v1`，
第一類那組（`platform/*`、`tools/*`）送 `main`（#86 凍著）。#130 量完了數字。
這一則做第二類那半，並把第一類那半寫成裁決。

#### 一、切法：位置不是風格問題，放錯會**失效而且是綠的**

先查的是「設定放根層行不行」，答案是不行，而且被同一個機制否掉兩次。

- **`vp test` 的設定以 package 為根解析。** 一個 package 只要有自己的
  `vite.config.ts`，根層那份的 `test` 區塊就**整塊不繼承**。
  ⚠️ `apps/console` 正在這個狀態，而它的症狀是：覆蓋率退回 v8 的預設射程
  （只有測試載入過的檔案進分母），報表寫 **100%**，一個 99% 的門檻**照樣
  通過、exit 0**。三個第二類 package 有一個直接失效，沒有任何錯誤訊息。
- **`coverage.include` 不能像 thresholds 那樣掛 glob 分流。** 要修 `features/*`
  的假高分就得在根層放一個全域 `include: ["src/**"]`，而那對
  `tools/codemods`（產品碼在 package 根目錄）、`tools/slice-gen`（多 `bin/**`）、
  `apps/console`（多 `bff-routes.ts`）**全部是錯的射程**。第一類沒有門檻所以
  不會紅 —— 它只會安靜地印出錯的數字給下一個人讀。

所以是**逐 package 的設定，而切片那一份由 `slice-gen` 產生**。形狀與 C111 同一條：
規格 runner 的相依「必須列在切片自己的相依裡，不能靠 workspace 根目錄提升」，
而且「那一行必須由模板生成 —— 靠專案組記得寫，第二類就會在第一個切片上壞掉」。

⚠️ 順帶掉出來的一條：切片的 `vite.config.ts` 裡 **`plugins: [vue()]` 不是贅字**。
少了它 `.vue` 不會被轉譯，畫面那支會整支從覆蓋率報表消失 —— `features/order`
實測，行覆蓋率從 **7.84% 變成 14.28%**，而程式碼一個字都沒改。

#### 二、數字：只收 `src/usecases/**`，四維都是 100

門檻不是「測試寫夠了沒」。那種東西量的是紀律，而 C117 §五 正是用「紀律不是
交付物」把第一類判出 v1 的。這一條量的是**另一件事**：規格打 usecase（C114），
所以一行沒被走過的 usecase 就是**一個沒有規格在驗的 usecase**。

- **校準對象是產生器的輸出，不是樹上那兩支示範切片。** 第二類的定義是「下發給
  一百個不知道長什麼樣的業務場景的**預設值**」，那個預設值的校準對象只能是
  `vp create slice` 產出的東西。實測（2026-08-24）：產出的切片 20 條測試全綠，
  `src/usecases/` 行 5/5、分支 5/5、函式 2/2、敘述 7/7 —— **四維皆滿**。
  整包則是行 22.22%、分支 22.72%、函式 13.79%、敘述 21.05%。
  ⚠️ 對照樹上：`features/order` 行 7.84%、`features/shipment` 9.30%。
  **照樹上的觀測最小值設門檻，一個帶 20 條測試的新切片會輕鬆飄過** ——
  門檻對真正的下游消費者形同不存在。這正是 C119 第二個 commit 的同一句話：
  樹上零違規證明不了「開箱即綠」，產生器的輸出不在樹上。
- **取 100 而不是 100 − N**，理由與 C119 取觀測最大值同一條：任何 −N 都是沒有
  論證的數字，而觀測值有。
- **切片整體不設數字。** `src/views/**` 佔行分母的 40%、函式分母的 45%，而 `.vue`
  只算 `<script setup>`（template 一行都不進分母，#130 §六.2）—— 一個套在整包上
  的數字會被那件事帶著走，而不是被測試的多寡帶著走。
- ⚠️ **代價是明知而選的**：先寫 usecase、後寫規格的那段時間它必定紅。
  照〈校準〉的規矩，那不是缺陷，那是這條線第一次擋下的東西，也是下一則
  C 編號的內容。

#### 三、「算不算閘門」：問題其實是**誰在執行它**

覆蓋率預設是關的，只有 `--coverage` 才會開。一組只在有人手動加旗標時才成立的
門檻，與「沒有門檻」是同一個東西 —— 而且長得跟全綠一樣，那正是 `spec-report`
設計時多長出來的第四態「未執行」（C115 §二）。

所以選的不是「另外加一道閘門」，是 **`enabled: true`**：`vp test` 這條所有人本來
就會跑的路徑就是它的執行者。連帶：

- **不動 `scripts.gate`、不動 `scripts.ready`、不新增 CI 步驟。** 它跟著測試跑。
- 與 Tier 分軌一致（C119）：覆蓋率不是安全，不進 Tier 2。
- 實測過 `vp run -r test -- --coverage` 與 `ready` 既有的
  `--reporter=default --reporter=json --outputFile=.vitest-results.json`
  **並存無誤**（20 個 task 全綠、結果檔照樣落地、`spec-report --check` 綠）——
  這一條查的是 v1.7.0 CHANGELOG 記過的那條唯一升級破壞路徑，沒有踩到。

#### 四、`apps/console`：修射程，**不設數字**

它的射程在此之前是錯的（見 §一），校正後是 #130 的 13.20%。**不拿那個數字當線**：
它 75% 的分母來自 `main.ts` 與 `DevSession.vue`，兩支都是被 `dev-session-stripped`
**編譯**過、沒有被**執行**過（#130 §八.4）。把線畫在一個量測產物上，一年後沒有
人答得出「為什麼是這個數字」。這支 app 也沒有 usecase 層，所以 §二 那條 glob
不套用在它身上。

⚠️ `bff-routes.ts` 必須逐支列進射程 —— 它住在 package 根目錄不在 `src/`。#130
第一版漏了它，`apps/console` 低報成 2.12%。**射程寫錯不會報錯。**

#### 五、這棵樹上沒有一列會讓它開火 —— 所以絆線掛在產生器的輸出上

版控裡兩支示範切片都沒有 `src/usecases/`（C114 §六 刻意保留的狀態），所以
`src/usecases/**` 這條 glob 在這棵樹上**命中零個檔案**。而覆蓋率門檻的 glob
沒有命中任何檔案時，**它靜默通過、exit 0**（實測）—— 與打錯字完全同形。
這是第三次記下同一句話（#130 §七 射程寫錯不會報錯、C119 glob 打錯不會報錯），
所以這次給它機制而不只是註解：`tools/slice-gen/tests/coverage-gate.test.ts`

1. **靜態那半**：產生器真的產出 `vite.config.ts`；它的 glob 與數字**從契約取**
   （`USECASE_COVERAGE_GLOB`／`USECASE_COVERAGE_MIN`，不是字面值）；`enabled: true`
   在；provider 與 vue plugin 列在切片自己的相依裡；**而且產出的切片真的有
   usecase 檔** —— 最後這一條擋的就是「門檻永遠命中零個檔案」。
2. **執行那半**：把**產生器產出的那一份 `vite.config.ts` 原文**寫進一個暫存專案
   （建在 package 底下，node 才解析得到 `node_modules`，同 `tools/vue-typecheck`
   的 fixture），配一個兩分支的 usecase 跑兩次 —— 只走一支 → **紅，而且訊息
   指名那條 glob**；兩支都走 → 綠。重寫一份等價的設定的話，那支測試證明的
   是它自己。
...
- **切法：逐 package，不是逐類，也不是 `platform/*` 一組 `tools/*` 一組。**
  #130 的表上同一類內部差 17 倍（`platform/pii` 100% vs `tools/api-surface`
  5.91%），而 `tools/*` 九支的性質差很多 —— 一組數字要嘛擋死一半，要嘛對另
  一半形同不存在。逐 package 也是唯一能編碼 §七 那四種狀態的切法。
- **數字：先不設，因為分母現在是壞的。** #130 §八.1 的七支 tools 走子行程跑
  自己的 CLI（`api-surface`、`conformance`、`scope-check`、`doc-facts`、
  `promise-check`、`spec-report`、`slice-gen`），v8 只看得到 in-process 的執行。
  在那件事解決之前，任何數字要嘛逼人把 CLI 測試改寫成 in-process，要嘛把門檻
  設在 5% 以下 —— 兩個都不是想要的。⚠️ 值得先查的第三條路是用
  `NODE_V8_COVERAGE` 收子行程的覆蓋率再合併，**但那是要實測的一格，不是可以
  直接寫進裁決的斷言**。
- **四種狀態要編碼進去，不能都寫成 0%**（#130 §五）：`platform/bff-contract` 是
  **結構性無測試**（0/36，任何 > 0 的門檻都擋它）；`platform/tsconfig` 與
  `platform/eslint-config` 是**不適用**，而後者的報表寫 **100%**（那是 0/0）——
  **直接抄進設定就是一格假的滿分**。
- **軸：不能只設行覆蓋率。** 四個維度在同一個 package 裡會翻轉
  （`platform/config` 行 66.66% 但函式 **20%**；`tools/slice-gen` 行 62.5% 但函式
  **87.5%**）。**四個維度不互為代理**，與 C119 的複雜度四維是同一句話。
- **`platform/ui` 要單獨一格。** 它的 35.03% 是「27 支 `.vue` 裡只有 6 支被 mount 過」
...
- ⚠️ **`vp test --root <pkg>` 與 `vp run <pkg>#test` 的 cwd 不同**，而 #130 用的是
  ...
  `.feature`（`feature file specs/… does not exist`），產生器輸出的 20 條測試只跑
  得到 5 條 —— **usecases 那一層在那條路徑上讀出來是 0%**。#130 的數字沒有被污染
...
- **`vite.config.ts` 沒有進 `REQUIRED_FILES`。** 沿用 C114 的先例：`specs/` 與
  ...
  不該因為升級腳手架而紅。代價明講：**切片把這支檔案刪掉，門檻不會報錯，
  它會安靜地不存在** —— 產生器那一端有絆線（§五），手寫那一端沒有。
- **沒有碰 `features/order`／`features/shipment` 的測試。** 它們沒有 usecase 層，
  這一則不把它們遷過去 —— 那是 C114 §六 刻意保留的狀態，不是這張票的範疇。
- **收緊的時機沒有定。** 照〈校準〉，等它真的擋下東西、累積出 C 編號再說。

### C121 — 元層機械化：門檻是 `null` 而那是裁決，讓它跑得起來的那個旗標是安全性的（2026-08-24）

C117 §二 把這一格判成落 `release/v1`（「fork 之後 `platform/*` 的測試就是他們的
測試」）。#136 量出逐 package 的分佈，#145／`v1.10.1` 把它掉出來的三處補成測試。
這一則做剩下那一半：**把量測台留在樹上**，並寫下涵蓋範圍與第一次的門檻。
交付是 `stryker.config.mjs` ＋ `SCOPE.md` 一列（#150、`d67583e`、#152、`v1.11.0`），
怎麼跑與怎麼讀在 `TESTING.md` 元層那一節，**本則不重述**。

#### 一、涵蓋範圍：射程是一份逐 package 的清單，而唯一抓得到它寫錯的是**支數**

產品碼不在慣例位置的有三處（`tools/codemods` 的 `src/` 底下一支都沒有、
`tools/slice-gen` 多一個 `bin/`、`apps/console` 多一支根目錄的路由檔），所以射程
是列舉不是一行 glob。⚠️ **射程寫錯不會報錯，它會給出一個看起來合理的數字** ——
這棵樹為同一件事付過兩次學費（#130 的覆蓋率、#136 的突變量測），第二次湊出的是
「很低的總分配一個 100% 的覆蓋分」，兩個數字都不刺眼。

**唯一的對照是 `Found N of M file(s)` 的 N 對得上版控裡有幾支產品碼**，不是任何一個
分數。現在的 N 是 **78**＝86 − 3 − 1 − 4。⚠️ 那個對照寫成設定裡的一段註解，
**沒有任何測試在守它**（#150〈Testing Decisions〉：這個能力還沒被用過幾次，
現在替它決定「該守什麼」會固化一個還沒穩定的形狀）。

三個排除的理由**各不相同**，而「排除」不等於「這幾支沒有問題」——
三支的行覆蓋率分別是 58.25%／66.66%／70.58%：

| 排除的                                         | 理由                                                                                                               | 它是什麼性質                                                     |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------- |
| `tools/vue-typecheck`                          | 工具為了讓自己的 mutant 不製造型別錯誤而插 `// @ts-nocheck`，**正好關掉這支閘門存在的理由**，於是它在 dry run 就紅 | 結構性互斥（反方向試過：跑超過十分鐘沒跑完）                     |
| `platform/config`、`platform/security-headers` | 它們在射程裡會把一支需要 `process.chdir()` 的測試拉進 dry run，而 runner 把 `pool: 'threads'` 寫死在原始碼裡       | **dry run 一支紅就整趟中止** —— 留著它們等於整棵樹一個數字都沒有 |

⚠️ **`.vue` 不在射程裡，而那是這份清單的效果，不是驗過工具尊重那則裁決。**
設計系統那個 package 因此只被量到一小角（27 個 SFC 一個 mutant 都沒有）。
這與 C119 記的「`<script setup>` 在四條複雜度規則裡不存在」是**同一個位置的第二次**。

#### 二、第一次的門檻是 `null` —— 而那與 C120 §七 的「先不設」不是同一個東西

`thresholds.break` 是 `null`（`high`／`low` 只決定報表顏色），`scripts.gate` 與
`scripts.ready` 一個字都沒改。**理由不是資料不足，是這個量測的性質**：分數下降的
原因裡至少四種與程式碼品質無關 —— 新程式碼讓分母變大、等價變異、結構性量不到的
package、時間相依的 timeout 判定。**一道經常因為無關原因變紅的閘門，結局是被關掉，
而被關掉的閘門連清單都產不出來。**

⚠️ 對照 C120 §七：那裡的「先不設」是**分母是壞的**（七支 tools 走子行程，
instrumentation 看不見），資料補得起來就談得下去。這裡的 `null` 是**產出的形狀**
本身不是一個分數 —— 它是一份「哪些程式碼被改壞了而測試沒說話」的清單。
**兩則都寫「先不設」，但收緊的條件不一樣。**

收緊之前要全部成立（#150）：三個排除有處置、量得到的支數穩定；報告真的被看過至少
一次；**等價變異的比例逐 package 量過一次**（那是門檻的實際下限）；以及**門檻設在
`total` 還是 `covered` 上** —— 走子行程的命令列進入點對 instrumentation 一律不可見，
有的 package `total` 不到 6% 而 `covered` 近 77%，**`total` 對那些 package 是一個
不可能達成的門檻**。

#### 三、`ignoreStatic: true` 是**安全性的**，而它擋下的損失是零 diff、零紅燈的

⚠️ **這一格是本則最該被讀到的一段。** 靜態 mutant（模組載入時執行到的）歸屬不到
任何一條測試，所以工具對每一顆都**跑整套測試** —— 而整套裡有一支
（`tools/slice-gen/tests/e2e.test.ts`）用 `cwd: <repo 根>` spawn 真正的產生器 CLI。

實測（在一份 `git clone --no-hardlinks` 出去的拋棄式樹裡，**不是 worktree** ——
worktree 共用 `.git`，而這件事的症狀正好是往 `.git` 寫東西）：不開這一條跑一趟全樹，

1. 樹上多出一個 **`feat: initialized repo ✨`**，內容是整棵被注入過的樹（200 個檔），
   落在當時所在的分支上。⚠️ **沒有任何閘門會看到它 —— 它是一個合法的 commit。**
2. ⚠️ **本機 33 個 tag 全部被刪**（`git tag | wc -l` → 0），遠端的 33 個完好。
   出處是同一個函式的外層：`createInitialCommit` 的下一行就是 `clearLocalGitTags`，
   **那個 commit 存在就證明中間那行跑完了，所以前後兩行也跑了。**
   `git fetch --tags` 復原後與遠端逐項一致。
3. ✅ 沒有推上去（`push: !!locator`，`--offline` 且沒有 `--remote` 時 locator 是
   falsy；`origin/main` 的 reflog 只有 fast-forward）。⚠️ 但**「本機曾經有沒有沒推
   上去的 tag」證不了** —— 這條線的發版慣例是 GH Release 當場建（tag 來自遠端），
   所以風險低，**而那是慣例不是證明**。

**刪 tag 是零 diff、零紅燈的損失。** commit 至少在 `git log` 上看得見；tag 不見了要
到下一次發版才會發現，而那時沒有人會把它連回一次突變測試。⚠️ 這與這棵樹反覆記過的
形狀同一個（C116 的零行 diff、#130 §七 的射程塌掉）：**症狀不是紅燈，是什麼都沒有。**

⚠️ **`mutate` 排掉 `tools/slice-gen` 沒有用** —— dry run 兩種寫法都是 763 條測試，
而靜態 mutant「跑整套」不看射程。擋得住的只有 `ignoreStatic`。

**代價寫在設定裡**：827 顆靜態 mutant（全部的 14%）因此不被測試，其中 491 顆在關掉
這條時是真的被殺掉的 —— **那些訊號沒了**。⚠️ 這一格將來要收回來的話，前置條件是
先有一個「靜態 mutant 跑得起來又不會寫版控」的形狀（例如讓那支 e2e 在暫存目錄裡跑），
**不是把旗標關掉**。順帶：全樹 4 分 0 秒 → 1 分 29 秒。

#### 四、讀那份清單的兩條守則**方向相反**

- **第一條：存活是缺口的上界，不是缺口。** 沒有工具判得出等價變異，而它的比例逐
  package 不同（現有樣本：某個 package 的十個存活裡至少四個是等價的）。⚠️ 上一輪
  真的犯過 —— 一個存活被當成真缺口寫成頭條，而它是等價變異。
- **第二條：存活未必真的存活。** `platform/pii` 的
  `granularity: "grapheme"` → `""` 讓模組載入時丟 `RangeError`，手工套上去是
  **兩支測試檔整支載不起來、零條測試跑**，而工具印的是 `[Survived]` ＋
  「Ran all tests for this mutant.」。機制對得上：**工具數的是「有幾條測試失敗」，
  而一支載不起來的測試檔貢獻零條失敗、零條測試。**
  ⚠️ **`testsCompleted === 0` 不是它的過濾器** —— 這顆是 17；全樹 952 個存活裡
  （那一趟關著 `ignoreStatic`）`testsCompleted` 為 0 的只有 69 顆，這顆不在裡面。
  **沒有現成的欄位分得出它。**

#### 五、工作目錄換掉的是**問題本身**，不是效能

runner 預設只跑「與被改動的檔案相關」的測試。在 package 目錄跑，問的是**它自己測不測
自己**；在 repo 根跑，問的是**整棵樹裡所有執行到它的測試殺不殺得動它**。同一個
package 兩種跑法差 **21 個百分點**（`slice-kit` 57.07% vs 78.26%），而差額全部集中在
被跨 package 消費的那一支。**腳手架的程式碼本來就是給別的 package 用的**，所以主量法
是 repo 根 —— 這是裁決，不是預設值。

#### 六、代價與還沒做的

- ⚠️ **`inPlace` 還原時不保留檔案模式**：跑完九支命令列進入點的可執行位會掉，
  而**逐行 diff 是零**。三個候選（跑完加一步檢查／改用非 inPlace／接受並寫進文件）
  選了第三個：第二個被「範疇檢查的測試拿當前的樹當 fixture」擋死，第一個等於新增
  一道閘門而這一版明確不新增。**這一版沒有修那個缺口，只是不讓它是隱形的。**
  ⚠️ **這是同一件事的第三次**（C116 的零行 diff 溜進發出去的版本、#140／#141 的
  `pnpm install` 把模式翻成 100755、現在是這一條）——
  **檔案模式至今沒有任何閘門在看**，而那該是它自己的一張票。
- **沒有增量／since 模式可用。** `--incremental` 存在但它做的不是「只 mutate 改動的
  檔案」：mutant 照樣全數產生、dry run 照樣全跑，省的是重測；而它的增量檔內嵌每支
  產品碼與每支測試檔的**完整原始碼** ＋ 一個絕對路徑的 `projectRoot`
  —— **那是 CI 快取的對象，不是版控的對象**。刻意關著。
- ⚠️ **「什麼事件會讓人真的去看它」沒有答案** —— #150 六個待填格裡它自己標成最重要、
  也最容易空著的一個。候選是讓它在**新出現的存活**上說話（兩份 json 相減，
  而相減的鍵不能含絕對位置），但那要先有一個跑它的時機。**這一版沒有做，
  現在它是一個要有人手動打字才會跑的東西。**
- **692 個存活躺在報表裡等人逐條讀，而那是這個能力的用途，不是設定的交付。**
  #145 那三處是第一批，第二批要另開票。

#### 七、本則取代的兩處陳述

裁決是有日期的紀錄，所以下面兩處**不修改**，在這裡標明它們已經過期：

- **C118 §六 末行**「剩下三格（層 1 複雜度、層 2 覆蓋率下限、元層機械化）一個都還沒做」
  —— 三格都做了（C119／`v1.9.0`、C120／`v1.10.0`、本則／`v1.11.0`）。
- **C119 末行**「第一類那半與元層機械化還沒做」—— 元層機械化即本則；
  層 2 第一類仍等 #86（C120 §七 已有裁決）。

⚠️ #127 那張地圖的四格到此全部有東西在樹上並且各自發過版。**它剩下的不是工作，
是一個要回答的問題**：`TESTING.md` §六 那張表是按「補了之後改變什麼」排的，
四格清空之後它是不是還該存在。

---

### C122 — 檔案模式：樹自己有答案了，而守它的地方是既有那條規則旁邊、不是一支新工具（2026-08-25，`v1.12.0`、#156）

**問題**：版控的檔案模式（`100644` ↔ `100755`）變更**逐行 diff 是零** ——
`git diff --stat` 印 `| 0`、`git status --short` 印 ` M`，與任何一般修改長得一樣，
而 `vp check`、全套測試、每一道閘門、每一道 CI **一個都沒有在看那個位**。

發作過三次，三次的源頭不同：C116（`vp install`／`vp create` 動它，
⚠️ **那一次進了發出去的版本**，`v1.7.0`）、#140／#141（`pnpm install` 建 workspace
bin 連結時 `chmod +x` 連結目標）、C121 §六（Stryker 的 `inPlace` 還原時不保留模式）。
更早還有 C99 與 C114 發版前抓到的一次。**每一次都是靠人用 `--summary` 追出來的。**

#### 一、判準：**宣告了 `bin` 的目標一律 `100755`，其餘一律 `100644`**

⚠️ **不是「有 shebang 就要 755」。** 那會把測試 fixture 與範例掃進來 ——
一個示範腳本有 shebang 很正常，而它不是任何東西的進入點。`bin` 是**宣告**：
有人打算讓別人直接執行它，而那個意圖寫在 `package.json` 裡。

C116 §三 當時明文不補閘門，理由是判準不存在 ——「七支 `cli.ts` 全部有 shebang、
六支宣告了 `bin`，模式卻是五支 `100644`、兩支 `100755`，**所以『哪個模式才對』
這個問題，樹自己沒有答案**」。#141 把九支對齊成 `100755` 之後樹有答案了，
**這道檢查是讓那個答案保持為真的東西** —— 對齊一次是快照，不是控制。

事實來源是 `git ls-files -s`，不是檔案系統的 `stat`：git 只記兩個值，
而工作區的實際權限位比那多。問的是「**版控裡**是什麼」。

#### 二、住在 `tools/conformance` 的一條規則，不是一支新工具

`conformance` 已經不只守切片契約：`rules/action-pinning.ts` 守 CI 的 action 釘住、
`rules/ownership.ts` 守 CODEOWNERS —— 兩條都與切片無關。**這一條與它們同一類**，
而搭既有的 `Finding`／`collect`／報告與退出碼，比長一支新工具便宜一個數量級
（新工具要 `package.json`、`tsconfig`、README、`SCOPE.md` 登記、閘門鏈接線、
workflow 一步、README 那張表一格）。

順帶交代 tier：`conformance` 在 Tier 2，而「檔案模式對不對」**不會隨時間失效** ——
照 Tier 2 那三條規則的理由它該在 Tier 1。⚠️ **這是一個已知的不對齊，不是疏忽**：
`action-pinning` 也一樣，而它已經在那裡。要動的話該動的是整支 `conformance` 的層級，
不是為了一條規則把它切兩半。

#### 三、⚠️ 這條規則**不在 `--root` 底下跑**，而代價要寫清楚

`--root` 指到的是**複製到暫存目錄的切片副本**（`tools/promise-check` 與這支自己的
反向測試都這樣用）。**副本不是版控**，所以這條規則問的問題在那裡沒有答案 ——
不是「答案是綠的」。

兩條路都試不得：讓它照跑，`git ls-files` 非零退出，於是
`specs/promise-1-architecture.feature` 那條**期待綠燈的對照組**當場紅；
讓它安靜回零筆，就是這棵樹付過兩次學費的形狀（覆蓋率的預設射程、
ESLint 裝在 repo 外面）——**量不到被記成沒問題**。

⚠️ **代價是真的**：`--root` 正是 `conformance` 唯一的端對端反向測試機制，
所以 `negative.test.ts` 的每一條都證明不了這條規則有被接進 `cli.ts`。
補法是另外一條**接線測試**（見下），而不是假裝守得住。跳過這件事會印在成功訊息裡。

#### 四、第一版的缺陷：訊息與呼叫脫鉤，接線測試因此是**恆真的**

第一版把成功訊息寫成一個與呼叫無關的分支：

```
if (!SANDBOXED) findings.push(...checkFileMode(ROOT));
...
console.log(SANDBOXED ? "跳過" : "含版控檔案模式：bin 目標須 100755");
```

**把 `checkFileMode(...)` 那一行刪掉，訊息照印、八條判定測試照樣全綠。**
那條接線測試咬不到任何東西。

修法是讓訊息帶一個**只有真的跑過才產得出來的數字**（看了幾個版控檔案），
測試斷言那個數字而不是那句話。⚠️ 同一條教訓這棵樹寫過兩次：
`doc-facts/src/derive.ts` 的「唯一抓得到射程寫錯的對照是 `Found N` 對不對得上版控」，
以及覆蓋率那次的假滿分。**綠燈要附一個非零的對照，否則「零筆」與「一個都沒看到」
長得一樣。**
...
| 變異                                                | 紅了幾條 | 哪一條                                                                    |
| --------------------------------------------------- | -------- | ------------------------------------------------------------------------- |
| 拿掉 `cli.ts` 裡的 `checkFileMode` 呼叫（訊息留著） | 1        | 接線測試 —— **修完之後才咬得到，這就是 §四**                              |
| `bin` 只認物件形式（漏掉 `slice-gen` 的字串形式）   | 2        | 字串形式那條 ＋ 接線測試（`slice-gen/bin/index.ts` 變成違規，CLI 直接紅） |
| 只判「bin 目標掉了可執行位」一個方向                | 1        | 反方向那條                                                                |

反向測試跑在一個 `git init` 的 **fixture repo** 上，⚠️ **不對這個 repo 的 index 跑
`git update-index --chmod`** —— 翻掉與還原之間只要中斷一次，index 就留在髒的狀態，
而下一次 `vpr gate` 會為了一個無關的理由紅：正是這道閘門要治的那種零行變更。
fixture 用 `cwd:` 傳工作目錄，不用 `process.chdir()`（runner 把 `pool: 'threads'`
寫死在原始碼裡）。

#### 六、守不到的東西

- **`--root` 底下什麼都不說**（§三）。
- **只判 blob 的兩個模式**：symlink（`120000`）與 submodule（`160000`)不判 ——
  對它們要求 `100644` 是錯的。這棵樹目前一個都沒有，而「現在沒有」不是不處理的理由。
- **還沒 `git add` 的檔案看不到** —— 守的是版控，不是磁碟。`ls-files` 讀 index，
  所以 staged 的新檔當場看得見，這一點與 `scope-check` 同一條理由。
- ⚠️⚠️ **工作區單獨翻掉的模式，這道閘門看不到** —— 而本則第一版寫反了，見 §七。

#### 七、⚠️ 一則寫進去、發出去、然後被實測推翻的因果（2026-08-25 當日更正）

本則第一版在〈守不到的東西〉寫著：

> 「Stryker 那個缺口仍然存在，只是**不再隱形**：`inPlace` 跑完模式會掉、
> **這道閘門會紅**、人手動 `git checkout -- tools/` 回去。」

**那句話是錯的，而它跟著 `v1.12.0` 一起發出去了**（CHANGELOG、Release notes、
#156 的結案留言，共四處）。

`git ls-files -s` 讀的是 **index**；Stryker 的 `inPlace` 還原翻的是**工作區**。
實測：

```
$ chmod -x tools/conformance/src/cli.ts     # 只動工作區，不碰 index
$ node tools/conformance/src/cli.ts
✓ 一致性檢查通過（2 個切片）
  含版控檔案模式：284 個版控檔案（bin 目標須 100755，其餘 100644）
$ echo $?
0
$ git diff --summary
 mode change 100755 => 100644 tools/conformance/src/cli.ts
```

**閘門全綠，而舊機制（`git diff --summary`）看得見。**

⚠️ **測不出來的原因寫在測試裡**：三條 fixture 反向測試全部用
`git update-index --chmod`，那動的是 index —— **整套測試分不出這兩種情況**，
三個變異也沒有一個踩到工作區那條路。

**收窄後正確的說法：**

- 這道閘門守的是**進得了版控的**模式漂移（staged 或 committed）。
  C116 那一次是 commit 出去並發了版的，**這道閘門攔得下來** —— 那是它的價值。
- ⚠️ **工作區單獨翻掉的模式它看不到**，直到有人 `git add`。
- 所以 **C121 §六 的「跑完看一眼 `git diff --summary`」沒有被取代，仍然必要。**
  兩者**互補**，不是後者取代前者：`git diff` 看工作區 vs index，這道閘門看 index 本身。

⚠️ 沒有把事實來源改成檔案系統來「補」這個洞，而那是刻意的：工作區的權限位
比 git 記的多，改過去會讓同一棵樹在兩台機器上得到不同答案（§一）。
一個純本機、還沒進版控的狀態不該讓閘門紅。

**這正是這個 repo 自己命名的最嚴重的一種錯：聽起來合理、與事實吻合、
但沒被驗證過的因果。**寫下判斷的時候順手驗一次的成本是一行 `chmod`。

---

### C123 — 下一條承諾不寫：閘門忽略 `--root` 會全綠，而對照組那道保險守不了這個方向（2026-08-25，#155）

**問題**：#155 要答兩件事 —— 剩下四條承諾**下一條做哪一條**，以及
「**一支 vitest 算不算閘門**」。兩題的答案被同一件量出來的事實決定，
而那件事實不在票上：`--root` 不是一個旗標，是一個**沒有人在守的約定**。

#### 一、這條線的閘門靜默忽略不認得的旗標，而 `theme-verify` 因此對 `--root` 全綠

實測，兩邊只差一個旗標：

```
node tools/theme-verify/src/cli.ts --root <沙盒>
→ ✓ 靜態：27 個元件 ＋ 4 個切片／應用畫面、0 處原始顏色
  ✓ 引用：… ✓ 建置：…                                    exit 0
```

而那個沙盒的 `platform/ui/src/components` 底下只有 **26** 個 `.vue`
（量測前刻意刪掉 `UiSwitch.vue`）。它印的 27 是**真樹的**。

⚠️ **未知旗標沒有報錯。** `ROOT` 是 `resolvePath(fileURLToPath(import.meta.url),
"../../../..")` —— 從這支程式自己在磁碟上的位置往上四層算，`process.argv`
一個字都沒讀。這是同一顆釘子的第四次：`tools/spec-report` 的第四態 ❓ 未執行
（C115）、`.oxlintrc.json` 建了但 `--print-config` 證明完全沒被讀到（C119 §一）、
覆蓋率的預設射程（#130，#136 又踩一次）。**共同症狀都是「設定寫了但沒生效」
與全綠長得一模一樣。**

⚠️ **而這不是 `theme-verify` 一支的事。** `tools/conformance` —— 這條線上唯一
有 `--root` 的閘門 —— 把旗標打錯一個字母：

```
node tools/conformance/src/cli.ts --roo /tmp
→ ✓ 一致性檢查通過（2 個切片）                            exit 0
```

它認得的是 `--root` 這四個字，不是「旗標」這個概念（`argv.indexOf("--root")`，
其餘一律無視）。**這條線上的閘門一律靜默忽略不認得的旗標，`conformance` 只是
恰好認得那一個字。**

⚠️ **也不是新發現 —— `main` 上這件事已經被量到並修掉了。** 那條線的
`theme-verify` 註解逐字寫著「在此之前 `--whatever` 會安靜地跑完並回傳 0 ——
一個打錯字的 CI 步驟會頂著閘門的名字發綠燈」。修法是 `tools/gate-kit` 的
`parseFlags`（**不認得的旗標一律失敗**），而它的文件裡記著一則更貴的先例：
`--masking` 從 `pii-check` 拿掉之後，`tier2-security.yml` 那一步留了下來、
安靜地掃了別的東西、回傳 0 —— **一個叫「個資：畫面上必須隱碼」的綠燈，
而它宣稱守的那條早就沒有任何東西在守。**

⚠️ **`release/v1` 從來沒有 `gate-kit`**（`git log -- tools/gate-kit` 在這條線上
是 0 筆）。兩條線在 C70 之後各自往前走 —— 這條線的 C73 是另一件事，`main`
的 C73 才是抽 `gate-kit` 那一則。**不是判準否決了它，是這條線沒走那一步。**
處置見 §六。

#### 二、所以今天硬加一條承諾四的場景，紅燈會把人指向錯的地方

`checkPromises` 的順序是寫死的（對照組先跑）。推演一次：

| 場景                         | 破壞的 | 量到的 | 結果                  |
| ---------------------------- | ------ | ------ | --------------------- |
| 對照組「沒有人違規時不得紅」 | 沒破壞 | 真樹   | 綠 → **通過**         |
| 「切片裡寫了 `bg-white`」    | 副本   | 真樹   | 綠 → 〈承諾沒有牙齒〉 |

而〈承諾沒有牙齒〉的 fix 訊息寫的是「要嘛那道閘門真的漏了這一條（修閘門），
要嘛這條承諾不再為真（改規格）」—— **兩條路都不是真正的原因**，而第二條
是能讓它變綠的那一條。那則紅燈旁邊已經寫著「⚠️ 兩者差很多，不要為了讓它
變綠而改規格」，而這裡改規格的誘因特別強：表面的原因看起來與承諾無關。

⚠️ **對照組那道保險有方向性，這是 C118 §三 沒有寫下來的一半。**
它守的是：沙盒建壞掉的時候，每一條「必須紅」都會成功變紅 —— **偽陽性方向**。
這裡是反過來的：閘門根本沒看沙盒，於是每一條「必須綠」都會成功變綠 ——
**偽陰性方向，同一條對照組守不了它。**

#### 三、承諾一那個片段今天分辨不出這件事 —— 而它是唯一在做這件事的東西

`specs/promise-1-architecture.feature` 的對照組：
`那麼 它必須綠，訊息裡要出現 "2 個切片"`。訊息片段比對是這道閘門裡
唯一有機會問「你到底看的是哪一棵樹」的地方。實測真樹的輸出：

```
node tools/conformance/src/cli.ts        → ✓ 一致性檢查通過（2 個切片）
```

**逐字相同。** 因為 `SLICES_NEEDED = 2`，而沙盒是照真樹建的 ——
兩邊的數字**設計上就會相同**，這個片段永遠分辨不出來。

`tools/conformance` 今天確實讀 `--root`（`--root <空目錄>` → 「找不到
`features/` 目錄」，路徑印的是那個空目錄）。所以承諾一此刻是真的。
⚠️ **但沒有任何東西在守它繼續是真的**：旗標改名、被重構掉、或哪天有人
把它拿掉，症狀是三條「必須紅」全部報〈承諾沒有牙齒〉而對照組全綠 ——
與 §二 那張表**一模一樣**。⚠️ 而 §一 已經量到，那個「哪天」不需要有人動手
拿掉它：`--roo` 打錯一個字母就到了。

#### 四、「一支 vitest 算不算閘門」——判準不是形狀，是能不能被指向一份副本

`check.ts` 已經把「閘門」機械化成三個性質，判準用現成的就好：

| 性質                                          | `component-contract.test.ts` |
| --------------------------------------------- | ---------------------------- |
| 形狀：`<gate>/src/cli.ts` 存在                | ❌                           |
| 在 `scripts.gate` 裡                          | ❌                           |
| 在 `vpr ready`／CI 上被執行                   | ✅                           |
| **能對副本跑**：`--root <sandbox>` 量得到副本 | ❌                           |

中間那兩列在這裡最容易被答錯，所以先把事實擺開。這條線上有**三層**執行面：

- `scripts.gate` —— 8 支 CLI 閘門
- `scripts.ready` —— `vp check` → 測試 → 建置 → `vpr gate` → `spec-report`
- CI —— **不跑 `pnpm gate`**，把那 8 支逐步展開，另外還跑 `vp check`、
  `vp run -r test`、建置、`spec-report`、SAST、gitleaks

`platform/ui/tests/component-contract.test.ts` 在後兩層裡被執行，
而且 Tier 1 是必要 context —— 它**擋得下 PR**。承諾二／三 在 CI 上不是空的。

⚠️ **但把閘門身分放寬到「`vpr ready` 上跑得到就算」解決不了問題**，因為最後一列仍然敗：

```
component-contract.test.ts  PACKAGE_ROOT = join(import.meta.dirname, "..")
theme-verify/src/cli.ts     ROOT = resolvePath(fileURLToPath(import.meta.url), "../../../..")
```

**同一種寫死。** 放寬身分而不解決這一條，那條承諾會退化成「這支測試存在
且會被跑」的靜態比對 —— 正是 C118 §二 判別式要擋掉的手抄本。

> **答案：一支 vitest 不因為它是 vitest 而不算閘門。它與 `theme-verify`
> 卡在同一條線上 —— 今天沒有任何辦法把它指向一份副本。**

一條判準同時處理四條裡的三條（承諾二／三 卡 vitest ＋ `theme-verify`，
承諾四 整條就是 `theme-verify`），比對每一條各寫一個理由誠實。

#### 五、更正 C118 兩句話

**（a）§五「`theme-verify` 技術上接得上，代價是每個場景多幾十秒」——代價不是時間。**

把 `ROOT` 指向一個補齊了 `platform/ui` ＋ `apps` ＋ fixtures 的沙盒，實測：

| 半邊                       | 結果                                                    |
| -------------------------- | ------------------------------------------------------- |
| 靜態（掃元件、擋原始顏色） | ✅ 跑得起來，而且**真的讀副本**                         |
| 建置（真的建置兩次）       | ❌ **25 毫秒就爆** `Can't resolve '@org/ui/styles.css'` |

靜態那個「真的讀副本」有兩道對照：刪一個元件 → `27 個元件` 變 `26 個元件`；
往副本的切片 view 塞 `bg-white text-gray-900` → 兩條紅，訊息含「原始顏色」，
路徑是相對副本的。**承諾四的「擋原始顏色」那一格，驗法今天就接得上。**

建置那半失敗的原因不是慢：fixture 的 `@import "@org/ui/styles.css"` 要
workspace 解析，而副本不是 pnpm workspace、沒有 `node_modules`。
⚠️ 而 `runStatic(); await runBuilds();` 之間**沒有 early exit** ——
靜態那半今天單獨跑不了。

**（b）§二 拿 `tools/spec-report` 當「存在但不在閘門鏈上」的反例，說
「承諾綁到它，那條承諾在 CI 上是空的」——那句話對它為假。**

`tier1-quality.yml:98` 就是 `node tools/spec-report/src/cli.ts --check`。
**`scripts.gate` 的成員資格，對「會不會在 CI 上被執行」既不必要也不充分。**

⚠️ 而那個論證要的結論**仍然成立，只是要換一句話**：綁到不在 `scripts.gate`
上的東西，那條承諾在 **`vpr gate` 上**是空的 —— 而 `vpr gate` 才是 HANDOFF
叫採用團隊跑的那個東西（`vpr ready` 的第四步）。**他們不會跑這個 repo 的
workflow。對交付物而言 `scripts.gate` 仍是對的事實來源。**
`check.ts` 與 `negative.test.ts` 各一句措辭跟著這一則改掉。

⚠️ 附帶一件沒有人在守的事：**`scripts.gate` 與那兩份 workflow 是兩份手抄本。**
全樹查過，沒有任何機制在比對（`tools/doc-facts` 只數 `.github/workflows/*.yml`
裡 `uses:` 的行數，那是在守 Action 有沒有釘版本）。今天兩份一致。
**這裡不開工作項** —— 對承諾而言 `scripts.gate` 是對的那一份，而 CI 少跑一支
的症狀會直接是 PR 上缺一個必要 context，與承諾機械化不是同一格。
記在這裡，是因為下一個讀 `check.ts` 那句 fail 訊息的人會以為它們是同一件事。

#### 六、下一步：一條承諾都不寫，先做「能不能被指向一份副本」

#155 已經替這個答案留好位置（「答案可能是先做讓 `theme-verify` 跑得了副本
這件事，一條承諾都不寫」）。⚠️ **而它不是加一個旗標**，三件事各自獨立：

| 要解決的            | 形狀                                                                                                                                                                                                                                                                                   |
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--root` 語意不單一 | `ROOT` 底下同時掛著**被驗的對象**（`platform/ui/src/components`、`platform/ui/src/styles/index.css`、`features`／`apps`）與**工具自己的素材**（`tools/theme-verify/fixtures`）。前者該隨 `--root` 走，後者不該。                                                                       |
| 沙盒契約要擴        | `makeSandbox` 今天複製的是 `features/<2 片>` ＋ 一份寫出來的 `CODEOWNERS`。§五 那個綠是在**另外補了 `platform/ui` ＋ `apps` ＋ fixtures** 的沙盒上量到的。                                                                                                                             |
| 建置半在副本上失敗  | 要嘛讓副本能 install（那是全樹副本的量級，而這條線記過「驗這類事要用 clone 不是 worktree」），要嘛讓靜態半單獨跑得了。                                                                                                                                                                 |
| 旗標解析本身        | §一 那個病的解藥在 `main` 上已經存在（`tools/gate-kit` 的 `parseFlags`／`repoRoot`／`walk`），而這條線從來沒有它。⚠️ **它解的是「未知旗標靜默忽略」，不是「能被指向副本」** —— `main` 的 `repoRoot()` 同樣是寫死那四層，那條線的 `theme-verify` 也還是跑不了副本。別把它當成主線的解。 |

⚠️ **而三件事之前還有一件更便宜、也更該先做的**：`promise-check` 今天
**分辨不出「閘門讀了 `--root`」與「閘門忽略 `--root`」**（§一、§三）。
那個盲點在只有一支閘門的時候看不見，接第二支的那一刻就發作 ——
而它發作的樣子是一則**指向錯誤地方的紅燈**。

⚠️ **上面每一件都由 #167 own，而開票的順序是刻意的：先開它，再關 #155。**
#155 自己就是 #127 關掉時唯一失去追蹤處的東西 —— 一張票的「還差 N 件」寫在
裁決的表裡而沒有人 own，與那次的處境一模一樣。

**這一則刻意不動的東西**：`HANDOFF.md` 那句「守到一條，還差四條」不改
（C118 §六 刻意留的，補完一條才改）；`TESTING.md` §六 那一格不動；
沒有新檔案，`SCOPE.md` 根層表與 `CODEOWNERS` 不需要新列。

### C124 — 閘門看不看得見那份副本：兩趟 argv 對稱的差分探針，而唯一的非零列是一支假閘門（2026-08-25，#167）

C123 判了「下一條承諾一條都不寫」，理由是 `promise-check` **分辨不出
「閘門讀了 `--root`」與「閘門忽略 `--root`」**。這一則把那個盲點補上。
⚠️ **它不補承諾，也不動 `--root` 語意與沙盒契約** —— #167 到達條件的第 2、3
項原封不動留在票上（見§五）。

#### 一、判別法：跑兩趟，唯一的變數是那個路徑指向哪裡

`src/probe.ts`：一趟 `--root <一個空目錄>`、一趟 `--root <repo 自己>`，
輸出（含結束碼）逐字相同 → 它沒有看那個目錄。實測（`scripts.gate` 上那六支
純 CLI 閘門，兩趟合計約 260 毫秒）：

| 閘門                  | 兩趟   | 判定    |
| --------------------- | ------ | ------- |
| `tools/conformance`   | 不同   | 讀了    |
| `tools/api-surface`   | 逐字同 | ⚠️ 忽略 |
| `tools/vue-typecheck` | 逐字同 | ⚠️ 忽略 |
| `tools/theme-verify`  | 逐字同 | ⚠️ 忽略 |
| `tools/doc-facts`     | 逐字同 | ⚠️ 忽略 |
| `tools/scope-check`   | 逐字同 | ⚠️ 忽略 |

⚠️ **判定本身要穩定，否則它就是自己在治的那個病。** 這裡認的是「兩趟有差異」，
而差異不必然來自 `--root` —— 一個時間數字、一份沒有排序的檔案清單，都會讓一支
其實在量真樹的閘門被判成「讀了」，**而那種判錯不會有任何紅燈**。上面六支各判
三次，判定全部一致。⚠️ 綠燈那條路徑也把兩趟的輸出帶回來（`RootProbe.evidence`
兩邊都填），那一天才看得出差在哪裡。

#### 二、⚠️ 兩個設計決定，各自有一個對照組證明它不是裝飾

**a. 第二趟不是「完全不給 `--root`」。** 給 vs 不給的話兩趟 `argv` 形狀不同，
於是一支**只是把參數印出來、卻完全不使用它**的閘門會產生差異 —— 而探針會說
「它讀了」。那是**危險方向**的判錯：放行一支其實在量真樹的閘門。兩趟都給
`--root`、只換路徑，argv 就對稱了。

**b. 兩個路徑遮成同一個標記。** 遮成 `<PROBE>` 與 `<ROOT>` 兩個不同的標記，
等於沒遮 —— 上面那支閘門仍然會產生差異。

⚠️ **這兩句話都不是推論。** 各自臨時拿掉一次，`tests/probe.test.ts` 的假閘門
當場被判成「讀了 `--root`」，兩條斷言都紅。拿掉遮罩：`expected true to be false`。
兩趟不對稱：同一則。

#### 三、唯一的非零列是一支假閘門，而它必須存在

這條線上今天唯一被承諾規格指名的閘門（`tools/conformance`）**是**真的讀
`--root`。所以探針寫出來的那一刻就全綠 —— 而全綠與「探針沒有作用」長得
一模一樣。這個 repo 在假的零上付過好幾次學費（`doc-facts` 的 `unguarded`、
`spec-report` 的第四態、量測台裝在 repo 外面），每一次的解藥都是同一句：
**表裡至少要有一列非零**。

那一列是 `tests/fixtures/ignores-root/src/cli.ts` —— 一支印出 argv、
卻完全不使用它的假閘門。⚠️ 它**不在** `scripts.gate` 上、不是任何 package
的 bin、只被 `tests/probe.test.ts` 直接指名。正向那一列（`conformance` → 讀了）
也不可省：只有反向的話，一支「永遠回答讀不到」的探針會全綠。

⚠️ 整合層（探針有沒有真的接進 `checkPromises`）另有一格，用的是
**真的 `tools/scope-check`**：它存在、在 `scripts.gate` 上、而且忽略 `--root`。
**那天它讀了 `--root`，這一格會安靜地不再守任何東西** —— 那天是好消息，
換一支還沒改的閘門就好，但要記得換。這句話寫在 `negative.test.ts` 裡。

#### 四、⚠️ 素材是空目錄，不是一份沙盒副本

沙盒是照真樹複製的，兩邊的輸出**設計上就會相同** —— C123 §三 那個坑
（承諾一的對照組片段 `"2 個切片"`，`SLICES_NEEDED = 2`）。而「切片數」那類
素材在 fork 的樹上會變成樹相依的（`breakage.ts` 記著 C95／C97 同一條教訓）。
空目錄與樹上有幾片切片無關，換一棵樹仍然分辨得出來。

⚠️ 探針的判錯方向是**偽陽性**：一支讀了 `--root`、而對空目錄的輸出恰好與真樹
逐字相同的閘門，會被誤報。紅燈訊息裡明講了這一種，並指向 `src/probe.ts`
而不是閘門。
...
| #167 到達條件                                      | 這一則         |
| -------------------------------------------------- | -------------- |
| ① `promise-check` 分辨得出閘門看沒看沙盒           | ✅ 交付        |
| ② `gate-kit`（或等價物）進不進 `release/v1` 的裁決 | 未動，仍在票上 |
| ③ `--root` 語意與沙盒契約                          | 未動，仍在票上 |

`HANDOFF.md` 那句「守到一條，還差四條」不改（C118 §六 刻意留的，補完一條
才改）；`TESTING.md` §六 那一格不動；`specs/` 一個字都沒動 —— **承諾的條數
沒有變，變的是「已經在守的那一條，現在證明得出它量的是副本」**。

---

### C125 — `parseFlags` 進 `release/v1`，而理由不是「`main` 有它」：`spec-report` 打錯一個字母會把過期的報表抹掉然後回綠（2026-08-25，#167）

#167 到達條件的**第二項**：`gate-kit`（或等價物）進不進這條線的裁決。
**裁決：`parseFlags` 進，`repoRoot()` 不進，`walk` 不在這個問題的範圍。**
⚠️ **這一則只有裁決，不含實作** —— 理由見§六。

#### 一、決定性的一趟：`--chec` 不只是被忽略，它會抹掉證據

`tools/spec-report` 的 `--check` 是「不寫檔、只比對報表與現況，不一致就紅」；
**沒有** `--check` 的那條分支是 `writeFileSync(absoluteReport, content)`。
把報表故意改到過期，然後跑兩趟：

```
node tools/spec-report/src/cli.ts --check   → exit 1   ✓ 它在守
node tools/spec-report/src/cli.ts --chec    → exit 0   ⚠️ 而且 git status 是乾淨的
```

第二趟打錯**一個字母**。那道閘門於是從「報表過期就紅」變成
**「把報表覆寫成當下現況，然後回綠」**。⚠️ 兩次 append 的過期標記都被抹掉了，
所以 `git status` 乾淨 —— 這不是推論，是量出來的。

⚠️ **這比 C52 的 `--masking` 更糟。** 那一次是掃了別的東西然後回綠
（「一個檢查不存在，比一個檢查失敗糟得多」）；這一次是**主動消滅它本來要
報告的那個差異**。

#### 二、判準怎麼套：看它守的東西給誰看

- `tier1-quality.yml:98` 是 `node tools/spec-report/src/cli.ts --check` ——
  **整棵樹的 workflow 裡唯一一行帶旗標的閘門呼叫**，而採用團隊拉走的就是它。
- `SPEC-REPORT.md` 是**業務功能完成率**，那份文件會被貼進工單與週報（C114／C115）。
- 所以這件事守的東西**給採用團隊看**，判準的必要條件成立。

⚠️ 而判準是**必要不充分**，所以還要問形狀（§四）與爆炸半徑（§五）。

#### 三、⚠️ 事實表：吃旗標的是四支，不是六支也不是九支

| CLI                   | 吃的旗標                                               | 打錯字會怎樣                   |
| --------------------- | ------------------------------------------------------ | ------------------------------ |
| `tools/spec-report`   | `--check`／`--report`／`--results`／`--root`／`--help` | ⚠️ 抹掉過期的報表，exit 0      |
| `tools/api-surface`   | `--baseline`／`--platform`／`--reference`／`--update`  | 掃預設位置／不更新基準，exit 0 |
| `tools/conformance`   | `--root`                                               | 掃真樹，exit 0（C123 §一）     |
| `tools/promise-check` | `--root`／`--spec`                                     | 掃版控全部規格，exit 0         |

⚠️ **量的時候踩了一次自己記過的坑**：`ls -d tools/*/` 給出 20 個工具，
而 `git ls-files` 給出 10 個 —— 差的那 10 個是**切分支留在磁碟上的殘骸**
（`gate-kit`／`pii-check`／`sast`… 都在磁碟上，一個都不在這條線的版控裡）。
C73／C98 講的就是這件事，而這一則差點把它寫進表裡。
⚠️ 同一趟還誤把 `theme-verify` 算成吃旗標的 —— 那個 `--color-` 是 CSS 自訂屬性
的前綴，它一個 `process.argv` 都沒讀；`scope-check` 那一處也只出現在**註解**裡。

#### 四、形狀：搬 `flags.ts` 進 `tools/gate-kit/`，同路徑同名 —— 而併回 `main` 實測過

⚠️ **這一格不能用推論決定。** [[merge-silently-undoes-v1-scope-cut]] 記著
`release/v1` 併回 `main` 零衝突刪掉 65 個檔**與 13 個 CI 步驟**，而目錄支數與
集合差**兩道核對都看不見**它（#159）。一個「v1 上只有 `flags.ts` 的 gate-kit」
併進「main 上有三個模組的 gate-kit」正是那個形狀。

所以用 `git merge-tree` 實測了一趟（臨時分支，沒有推、沒有動 `main`）：

| 檔案                                            | 併回 `main` 的結果                        |
| ----------------------------------------------- | ----------------------------------------- |
| `src/root.ts`／`src/walk.ts`／`tests/`          | **完整保留**（v1 從沒有它們，不是刪除）   |
| `src/flags.ts`／`package.json`／`tsconfig.json` | 逐字相同 → **零衝突**                     |
| `src/index.ts`                                  | ⚠️ **add/add 衝突**（v1 一行、main 三行） |

**那個衝突是保護，不是成本**：唯一的分歧點是一行 `export`，而它會**停下來要人看**
—— 恰好是 #159 那個病沒有給過的東西。

⚠️ 對照過的另一條路（把 `parseFlags` 放在 v1 自己的地方，`main` 的 `gate-kit`
完全不受影響）**不採用**：它換到的是「同一個想法有兩份實作」，而這條線一再栽在
「同一件事有兩份寫法然後它們漂開」上（`flags.ts` 自己的檔頭就寫著這句）。

#### 五、⚠️ 不進的兩塊，與「四支全部」

**`repoRoot()` 不進。** 它是 `resolve(fileURLToPath(import.meta.url), "../../../..")`
—— **就是 C124 剛量到的那個病**。搬進來等於把〈閘門指不到副本〉制度化。
⚠️ C123 §六 已經警告過別把 `gate-kit` 當成「能跑副本」的解：`main` 的 `repoRoot()`
同樣寫死那四層，那條線的 `theme-verify` 也還是跑不了副本。

**`walk` 不進。** 它不在這個問題的範圍，而這條線沒有量過需不需要它。

**四支全部採用，不做子集。** 沒有一支的靜默忽略是安全的（§三 那張表），而
「先改三支」會長成 C118 那句「守到一條，還差四條」的第二個版本 —— 一個做完的
子集、一個沒有追蹤處的餘數、其餘地方全綠。

#### 六、⚠️ `parseFlags` 不取代 C124 的探針，兩者關的是不同的門

| 病                                                       | 關它的               |
| -------------------------------------------------------- | -------------------- |
| **不認得的旗標 → 沉默**（`--chec`）                      | `parseFlags`（本則） |
| **認得的旗標 → 收下了但不用它**（`--root` 傳了卻掃真樹） | C124 的差分探針      |

兩者互不涵蓋。`spec-report` 有 `--root` 而它同樣寫死 `repoRoot()` 當 fallback ——
`parseFlags` 讓它不再吞掉打錯的旗標，**不會**讓它看得見副本。

#### 七、這一則刻意沒做的事，以及 #86

**實作是另一批。** 它會改**已交付閘門的退出碼**（fork 的 CI 上一個打錯字的旗標
會從綠變紅），照這條線的版號判準那自成一次 minor —— 裁決與那次發版不該綁在一起。
`specs/`、`HANDOFF.md`、`TESTING.md` 都不動。

⚠️ **#86 的凍結不涵蓋這件事。** 那條凍結管的是「往 `main` 投入」（backport、
補條目、為 `main` 開 PR）；這裡的方向相反 —— 把 `main` 上**已經存在**的東西搬進
交付線。§四 那趟 `merge-tree` 是本機模擬，沒有推、沒有動 `main`。
這句話寫在這裡，是因為下一個人會先問這個問題。

---

### C126 — `parseFlags` 接上了，而範圍從四支擴到**八支**：一條只守四支的絆線，與沒有絆線長得一樣（2026-08-25，#167）

C125 判了「`parseFlags` 進」。這一則把它接上去。
⚠️ **範圍比 C125 寫的大**：那則說「吃旗標的四支全部採用」，實際接的是
**八支**（這條線自己寫的每一支 CLI）。理由在§一，而它是實作逼出來的，
不是順手擴的 —— **裁決與程式碼不一致的話，兩邊都會爛掉**。

#### 一、為什麼四支不夠：絆線的形狀決定了範圍

C125 §五 判「不做子集」，理由是先改一部分會長成 C118 那句「守到一條，
還差四條」的第二個版本。把同一句話套在絆線上：

> **一條只守四支的絆線，與沒有絆線，在第九支加進來的那天長得一模一樣。**

而 `main` 上 `flags.test.ts` 早就把答案寫成一條測試，逐字是：
「★ 空 spec 是 `theme-verify` 這種完全不讀 argv 的工具的情況 ——
『沒有旗標』很容易被實作成『什麼都放行』，而那正好讓**最不設防的那幾支
繼續不設防**」。空 spec 在 `parseFlags` 底下的意思是**拒絕所有旗標**。

所以不吃旗標的四支（`doc-facts`／`scope-check`／`vue-typecheck`／`theme-verify`）
各接三行空 spec。**「不吃旗標」現在是一句話，不是一片沉默。**

#### 二、C125 §一 那個病，關掉了

```
報表故意改到過期 →  --chec  exit 1，而且沒有覆寫（git diff 顯示那兩行還在）
                    --check exit 1（它本來就在守）
還原之後       →   --check exit 0
```

八支各餵一個不認得的旗標，全部非零；不帶旗標全部照舊。

#### 三、絆線：名冊是推導的，而⚠️ 事實來源是**兩個** script

`tools/gate-kit/tests/adoption.test.ts`。名冊從 `package.json` 抽
`tools/<某支>/src/cli.ts` 這個形狀 —— 寫死清單的話，第九支加進來不會有人知道。

⚠️ **只讀 `scripts.gate` 會漏掉 `tools/spec-report`** —— 它不在閘門鏈上，
它是 `scripts.ready` 的最後一步（也是 `tier1-quality.yml` 那一行）。
**而它正是讓 C125 成立的那一支。** 這是 C118 §二 的教訓反過來打在自己身上：
閘門鏈的成員資格，對「會不會被執行」既不必要也不充分。
所以名冊讀 `gate` ＋ `ready`，而**第一條斷言就是「名冊裡必須有 spec-report」**。

⚠️ `eslint` 不在名冊裡（第三方 CLI，旗標解析不歸這條線管）。**八支 node CLI**
＝ 閘門鏈上七支 ＋ `spec-report`。

#### 四、三個已知非零，因為全綠證明不了任何事

| 拿掉什麼                                  | 結果                                 |
| ----------------------------------------- | ------------------------------------ |
| 任一支的 `parseFlags`（試了 `doc-facts`） | 絆線紅：〈這一支靜靜地跑完了一趟〉   |
| `SCOPE.md` 的 `tools/gate-kit` 那列       | `scope-check` 紅：〈樹上有、沒登記〉 |
| （C124 那兩個對照組仍然成立）             | 探針的遮罩與兩趟對稱各拿掉一次都會翻 |

#### 五、⚠️ 更正 C125 §四：新增的衝突是**三個**，不是一個

C125 §四 用 `git merge-tree` 量到「唯一的分歧點是一行 `export`」。實作之後
重量了一次（`origin/main` vs 這一批，比對**衝突檔案的集合**）：

| 檔案                                                                     | 併回 `main`                 |
| ------------------------------------------------------------------------ | --------------------------- |
| `src/flags.ts`／`tests/flags.test.ts`／`tsconfig.json`                   | 逐字相同 → 零衝突           |
| `src/root.ts`／`src/walk.ts`／`tests/root.test.ts`／`tests/walk.test.ts` | 完整保留                    |
| `tests/adoption.test.ts`                                                 | v1 獨有 → 保留              |
| `src/index.ts`／`package.json`／`README.md`                              | ⚠️ **add/add 衝突（三個）** |

多出來的兩個是**這一則自己造成的**：`package.json` 的 `description` 改成
「只帶 `parseFlags`」，`README.md` 整份重寫（`main` 那份說「三個 export」）。
⚠️ **兩個都是刻意的** —— 一份說自己有三個 export 而實際只有一個的 README，
比一個看得見的衝突貴得多。衝突檔總數 23 → 25。

⚠️ 順帶消掉一個既有衝突（`tools/spec-report/package.json`）。**原因沒有查清楚**：
`main` 上沒有這個路徑，而 merge-tree 對它給出三個 stage，看起來是改名偵測把它
配到別的檔案上、加一行相依讓相似度掉到閾值以下。**這是推測，不是量到的。**

#### 六、⚠️ 兩處訊息會變成錯的，跟著改

C124 的探針判「閘門看不見那份副本」，判定沒變 —— 但**機制變了**：

| 時間      | 閘門收到沒宣告的 `--root`    | 探針的判定 |
| --------- | ---------------------------- | ---------- |
| C126 之前 | 靜默忽略                     | 看不見     |
| C126 之後 | **拒絕**（exit 1，兩趟相同） | 看不見     |

所以 `check.ts` 那句「這道閘門忽略了 `--root`」與 `negative.test.ts` 對
`scope-check` 那格的註解（「哪天它讀了 `--root`」）都改掉了。⚠️ 後者的失效
條件也變了：不是「讀了」，是「**宣告**了」。

#### 七、⚠️ `parseFlags` 有一個能力落差：可重複的旗標

`--spec`（`promise-check`）與 `--results`（`spec-report`）可以重複，而
`parseFlags` 只留最後一個。**沒有為此改 `flags.ts`** —— 改一個字就失去
「逐字相同 → 零衝突」，而那是 C125 §四 的立論基礎。

兩支的做法是把職責分開：`parseFlags` 負責**不認得的旗標一律失敗**（它存在的
理由），取值仍走各自原本的收集函式。⚠️ 不要為了統一把 `--spec` 改成只吃一個
—— 那是拿一道防線去換一次能力縮減，而縮減掉的正是反向測試餵多份改壞規格那條路。

#### 八、量測台自己回報過一次假的綠

驗「逐字相同」時用的是 `diff <(git show …) 檔案 && echo ✓` —— 而 `git show`
對一個**不存在的路徑**會失敗、給出空輸入，那一行仍然印了 ✓。
改用 `cmp` 之後三個檔案的逐字相同才是量到的。**`&&` 接在 `diff` 後面，
與 `| head` 吞掉退出碼是同一族的錯。**

#### 九、附帶

`README.md` 的「23 個 workspace 套件」改成 24 —— 是 `doc-facts` 抓到的，
不是人記得的。**那道閘門在做它該做的事。**

⚠️ **這一則的爆炸半徑比 C125 描述的更寬**：採用團隊的 CI 或包裝腳本，只要對
**八支裡任何一支**傳任何一個沒宣告的旗標，現在會硬失敗 —— 包含人手動跑的
`--update`。照版號判準那是 minor。

---

### C127 — 閘門指得到副本了，而擋路的不是「建置會失敗」，是**建置會成功並報出真樹的數字**（2026-08-26，#167）

**問題**：#167 的第三項 —— C123 §六 那張表的三件事（`--root` 語意不單一、
沙盒契約要擴、建置半在副本上失敗）。① 與 ② 交出來了，③ **整條理由被量測推翻**，
而推翻它的正是 ① 自己。

#### 一、`--root` 指的是「被驗的對象在哪」，不是「這支工具跑在哪」

`tools/theme-verify` 的 `ROOT` 底下同時掛著兩種東西，而它們該往相反方向走：

| 基準點 | 掛著什麼                                                                             | 隨 `--root` 走？ |
| ------ | ------------------------------------------------------------------------------------ | ---------------- |
| `ROOT` | `platform/ui/src/components`、`platform/ui/src/styles/index.css`、`features`／`apps` | ✅               |
| `SELF` | `tools/theme-verify/fixtures`（這支工具自己的證據來源）                              | ❌ 釘在磁碟位置  |

分家之後多出三個守衛，而它們補的不是防禦性程式碼 —— `runStatic` 底下
〈元件目錄是空的〉與〈切片與應用底下找不到任何 .vue〉那兩則紅燈**本來就是
為這一格寫的**（「這條檢查掃不到東西時會全綠 —— 那正是綠燈代表沒有人看」），
而在 `ROOT` 寫死的年代它們**永遠到不了**。`--root` 一接上，`readdirSync` 會
先丟 ENOENT，人看到的是一段 stack trace 而不是那句話。第三個守衛是
`THEME_CSS` 不存在 —— 它與「`@theme` 是空的」刻意分成兩則訊息，因為前者的
修法是補齊副本，後者是找回代幣。

⚠️ **`tools/conformance` 沒有這個病**：它的 `ROOT` 底下只有被驗的對象
（`features`、`platform`、`apps`、`.github/workflows`、`CODEOWNERS`）。
所以這是 `theme-verify` 一支的事，不是這條線的通病 —— 而下一支接 `--root` 的
工具要先問自己這個問題。

#### 二、沙盒契約：`platform` ＋ `apps` ＋ **`.github`**，而且改由 `git ls-files` 驅動

`makeSandbox` 在此之前只複製 `features/<2 片>` ＋ 一份寫出來的 `CODEOWNERS`。
副本裡因此沒有 `platform/ui`，`theme-verify` 指過去就 ENOENT；`conformance`
的 CSP 那條與幽靈依賴那條在副本上**掃 0 個目錄** —— ⚠️ 而那不是「那裡沒有
違規」，是**沒有人看**。

⚠️ **第三層是覆盤時才找到的，而它藏在一個 early return 裡。**
`conformance` 的 `checkActionPinning(ROOT)` **無條件執行**（不像檔案模式那條有
`SANDBOXED` 擋著），而它開頭是 `if (!existsSync(dir)) return;` —— 副本裡沒有
`.github` 的時候，它掃 0 個檔、**什麼都不說**、然後那道閘門全綠。
⚠️ 這一格特別值得記，因為 §一 才剛把 `conformance` 舉成「沒有語意混用問題」的
乾淨例子 —— **乾淨的是它的 `ROOT` 語意，不是它在副本上的每一條規則。**
處置是把 `.github` 放進契約（而不是像檔案模式那樣跳過）：workflow 檔在副本裡
**有答案**，跳過會白白關掉一條將來接得上的承諾。補完之後副本是 136 個檔。

⚠️ **`tools/` 刻意不在契約裡** —— §一 已經把素材釘回工具自己身上。**① 縮小了 ②**。

⚠️ **複製改成走 `git ls-files`，不是 `cpSync` 遞迴。** 這是 C73／C98 那條規矩
（`trackedSlices` 早就照著做，複製那一半沒有）。差別在切分支留下的殘骸：
磁碟上多一個 `platform/ui/src/components/Foo.vue`，`theme-verify` 在副本上就會
數到一個真樹沒有的元件 —— 而那個數字正是承諾要比對的東西。
這一輪自己就踩過同一顆釘子（C125 §三：磁碟 20 支工具、版控 10 支）。
順帶把 `node_modules`／`dist` 濾掉了：它們本來就不在版控裡。

⚠️ **而「複製到 0 個檔就丟錯」這個念頭是錯的，寫過一版之後拿掉了。**
複製既然走 `git ls-files`，0 的意思就是「這棵樹上本來就沒有那一層」—— 副本仍然
忠實反映真樹，而閘門在真樹上看到的也是同一個「沒有」。對一個把 `apps/` 換掉的
fork 丟錯，是拿一則**關於他們沒做錯的事**的錯誤訊息，去換一個這裡本來就守不住
的東西（C95／C97 記過這條：閘門的紅燈會被拉 v1 的團隊讀到）。
守這份契約的責任改放在測試裡：`tests/sandbox.test.ts` **逐層比對副本與真樹的
檔數**，層名打錯一個字母（`platfrom`）在那裡是紅的。

#### 三、⚠️ 更正 C123 §五：建置半在副本上**不會**失敗，它會成功並量真樹

C123 §五 量到的是「25 毫秒就爆 `Can't resolve '@org/ui/styles.css'`」，
而那個爆炸的原因是**當時 `fixtures/` 也跟著副本走**（副本不是 pnpm workspace）。
§一 把素材釘回去之後重量一次，整條理由翻過來：

| 被驗的樹                                     | 靜態半        | 建置半                                     |
| -------------------------------------------- | ------------- | ------------------------------------------ |
| 真樹                                         | 27 個元件     | 200 格代幣宣告；覆寫 3 格、連動 1 格…      |
| 副本（刻意刪掉 `UiSwitch.vue`）              | **26 個元件** | **200 格；覆寫 3 格、連動 1 格… 逐字相同** |
| 副本（`index.css` 掏成一行註解，代幣全沒了） | 紅（該紅）    | **仍然 200 格、0 處懸空引用**              |

第三列是決定性的那一列：**副本的設計系統是空的，而建置半報了 200 格。**
`@import "@org/ui/styles.css"` 從 `fixtures/` 經 `node_modules` 解析到**真樹**的
`platform/ui`，而那份 CSS 的 `@source` 是一條從**真樹根部**往下的 glob
（`runReferences` 的檔頭寫著這件事，只是沒有人把它與 `--root` 放在一起想）。
副本從頭到尾沒有參與。

⚠️ **這比「會失敗」危險得多，而且 `promise-check` 的探針接不住它。**
`probeRootSupport`（C124）比的是兩趟輸出有沒有差別 —— 而靜態半確實有差別。
於是它會判「讀了 `--root`」，然後建置半頂著閘門的名字報真樹的數字。
**那正是 C124 建來抓的形狀，只是低了一層：探針問的是「這支閘門」，
而答案可以逐段不同。**

處置：`--root` 之下**不跑建置半**，並且**印一行說自己沒跑**。
先例在同一條線上 —— `conformance` 在 `--root` 之下不檢查版控檔案模式，
它也是印一行說明，不是安靜跳過。

⚠️ **這不是「跳過一段慢的檢查」，是一句範疇聲明**：配色可換性那條軸在副本上
**沒有辦法驗**（要驗就得讓副本 install 得起來，那是全樹副本的量級 ——
而這條線記過「驗這類事要用 clone 不是 worktree」）。
⚠️ **不准有任何承諾把「那麼」綁在建置半的字串上而用 `--root` 跑** ——
`check.ts` 比對的是 `output.includes(fragment)`，所以那一行說明與最後的總結
都刻意與建置半的三行 ✓ 用不同的字，讓那種接法接不上。

#### 四、絆線，以及五次敲掉

⚠️ **`platform` 與 `apps` 補進沙盒的那一刻，沒有任何場景用得到它們** ——
今天唯一被規格指名的閘門是 `conformance`，而承諾一問的是切片之間的邊界。
一段「為了以後」而寫、沒有人在跑的擴充，就是這個 repo 記過好幾次的形狀
（`.oxlintrc.json` 建了但完全沒被讀到；`spec-report` 的第四態）。
所以 `tools/promise-check/tests/sandbox.test.ts` 用**真的** `makeSandbox` 建副本、
真的跑 `theme-verify --root` 與 `conformance --root`，而每一條都是**差分**：

| 敲掉的                    | 哪一條紅                    | 紅成什麼樣子                       |
| ------------------------- | --------------------------- | ---------------------------------- |
| `ROOT` 改回寫死           | 「刪一個元件數字要少一個」  | 印 27（真樹），副本是 26           |
| `--root` 之下照樣跑建置半 | 「不印建置半的任何一行」    | `✓ 建置：…` 與真樹**逐字相同**     |
| 沙盒契約拿掉 `platform`   | 三條                        | `✗ 元件目錄是空的`（§一 那則紅燈） |
| 沙盒契約拿掉 `.github`    | 「塞一行沒釘 SHA 的 uses:」 | 副本裡一個 workflow 都沒有         |
| 層名打錯成 `platfrom`     | 四條                        | 「版控裡沒有 `platfrom/`」         |

第三次順帶驗掉了 §一 的守衛：紅的是**設計好的那則訊息**，不是 ENOENT stack。

#### 五、現在探針說什麼：八支裡兩支讀 `--root`

`conformance` ✅、`theme-verify` ✅（本則之後）、其餘六支 ❌ ——
而 ❌ 的機制是 C126 之後的那一種：**它們拒絕 `--root`**（沒宣告），
兩趟都失敗且訊息相同，探針判定一樣是「看不見」。
那正是設計行為：哪天有場景指名那六支的其中一支，`promise-check` 會當場紅。

#### 六、刻意不做的

- **一條承諾都不寫。** C123 判的「下一條承諾不寫」到這裡**還沒有被撤銷** ——
  ③ 交的是能力，不是承諾。⚠️ `HANDOFF.md` 那句「守到一條，還差四條」不動。
- **不給其餘六支 `--root`。** 沒有人要指它們，而給了就要為每一支答一次
  §一 那個問題（哪些是被驗的對象、哪些是它自己的素材）。
- **不讓副本 install 得起來。** 那是全樹副本的量級，而它買到的只有建置那半軸。
- ⚠️ **`makeSandbox` 的成本上升了，而每一個 fork 在 `vpr gate` 裡都要付。**
  實測（各三趟取最小值，4 個場景）：`0.78s` → `1.12s`，約每份副本 **+85 毫秒**。
  記在這裡是因為場景數會長 —— 這個數字是逐場景的，不是一次性的。

---

### C128 — 設計系統的第二條軸只做在一個元件上，而檢查寫死了那個元件的檔名（2026-08-17）

> ⚠️ **這一則原本是 `main` 的 C70，併線時改號**（見 C133）。日期保留原始的那一天 —— 它記的是那天做的決定，不是改號的那天。

> 這一條是在 `release/v1` 上發現並修好的，回頭 cherry-pick 進 `main`。
> 起點是縮版時把五條產品承諾與它們的閘門逐條對照 —— 對照本身就是那次的產出。

#### 一、`UiDialog` 從落地那天就缺「形狀」那條軸

三軸的分工是：**值**走代幣、**形狀**走可覆寫的 class 字串、**結構**走 slot。

| 軸   | `UiButton`            | `UiDialog`      |
| ---- | --------------------- | --------------- |
| 值   | ✅                    | ✅ 全走語意代幣 |
| 形狀 | ✅ `inject(UI_THEME)` | ❌ **完全沒有** |
| 結構 | ✅ 1 格               | ✅ 3 格         |

`w-[min(32rem,92vw)] top-1/2 left-1/2 -translate-*` 寫死在模板裡。一個要把
對話框改成手機版底部滑出的案子，代幣換不掉（那不是值）、slot 換不掉
（那不是結構），只能去改 `platform/ui` —— 也就是 C62 決定要集中的那一半。

**沒有東西說話的原因很具體。** 當時的檢查是
`readFileSync("src/components/UiButton.vue")` —— **寫死一個檔名**。
它守的不是一條規則，是一個檔案。而 C65／C67 的驗收也都是拿 `UiButton` 做的，
所以「接縫是通的」這個結論從頭到尾只被一個元件證明過。

這是 A1 的反例：**列舉出來的清單只有一項，而沒有人注意到它只有一項。**

#### 二、決定

**（a）`createUiTheme` 改成「元件 → 具名槽」**（breaking，附 codemod
`flatten-ui-theme-to-components`）：
...
舊形狀 `{ variants, sizes }` 是**按鈕的概念長在全域 API 上**，第二個元件
沒有地方可去。改的時機是判準本身：**tag 之前的代價是一個呼叫端，之後是每一個
fork、永遠。** 同一個判準 C65 用過一次（趁沒有 fork 之前把 `--color-muted` 改名）。

**（b）槽名照 reka-ui 的基元名**（`overlay`／`content`／`title`／`description`）。
那也是 shadcn-vue 的 part 名與 shadcn Figma kit 的圖層名。設計師說
「overlay 要更淡」，前端要改的那一格就叫 `overlay` —— **成本零**，
因為那些名字本來就在元件的 import 裡。

⚠️ 但 **variant 的名字不跟** shadcn（`default`／`destructive`）：那要動 prop union，
＝ api-surface 的破壞性變更加上每個使用端。**免費的對齊做，要付錢的不做。**

**（c）檢查器改成掃 `src/components/*.vue`**，五條條文，每條配反向測試
（見 `platform/ui/tests/contract.ts`）。

#### 三、五條裡有一條是後來補的，而它才是最像真實錯誤的那一條

前四條（有沒有匯出／有沒有宣告槽／宣告與預設表一致／有沒有 `inject`）
抓得到「**接縫不見了**」，抓不到「**接縫還在、只是沒接上**」：

```
:class="parts.overlay"  →  :class="DEFAULT_PARTS.overlay"
```

打錯一個名字之後，`parts` 仍然被算出來、`DEFAULT_PARTS` 仍然有全部的鍵、
`UiThemeOverride` 仍然宣告著那個槽 —— **四條全綠，而各案的覆寫一個字都不生效。**

第五條因此是「模板不得直接引用預設表」。兩張表的型別註記一模一樣，
靠**有沒有讀 `theme.`** 分辨，不靠命名慣例 —— 靠命名慣例分辨等於沒有分辨。

規則寫成通則而不是 `UiDialog` 專屬：`UiButton` 有同樣的間接（`classes`
這個中間名字），只是它剛好沒寫錯。

#### 四、刻意不守的東西

**「接縫夠不夠」不是靜態事實。** 一個元件該開幾個槽是設計判斷，交給
`CODEOWNERS` 與 PR。做成規則的話它會長成「每一塊 class 都要有槽」，
然後 `UiDialog` 那兩個排版用的 `<div>`（`mt-4`／`mt-6 flex …`）會被逼出
沒有人會覆寫的槽名 —— C41：會誤報的閘門第一天就會被加例外，
而例外永遠不會拿掉。

守得住的只有兩件事：**有沒有接縫**、**接縫有沒有漂**。三種真實壞法都證明過會紅
（拿掉 `UiThemeOverride` 那一格 → ②③；拿掉 `inject` 只留預設表 → ③；
模板綁回 `DEFAULT_PARTS` → ⑤）。

#### 五、shadcn 沒有安裝

被問到「可不可以用腳手架已安裝的 shadcn」時查的：`node_modules` 裡有
`reka-ui`／`clsx`／`tailwind-merge`，**沒有 `shadcn-vue`、沒有 `components.json`、
沒有 CLI**。D15 的標題會讓人以為裝了，已更正。
...
| 命名與解剖 | **照 shadcn**，成本零，而且詞彙有外部事實來源                               |
| 樣式層     | **不照** —— 理由見下面那條更正                                              |
| 元件來源   | 用它的 CLI 抄元件進來是補元件最快的路，但那是**內容**不是接縫，不在本條範圍 |

> ### ⚠️ 更正（2026-08-17，讀完官方原始碼、CLI 文件與 skill 之後）
>
> 這一段被寫錯過**兩次**，兩次的形狀不同，都值得留著。
>
> **第一版**（進了 v1.0.0 的 tag 與 PR #43）：「它的答案是**你擁有原始碼，
> 直接改它**，接案公司照做 ＝ 20 個案子 20 份 UiButton」。
>
> **第二版**（PR #45 初稿）讀了 `registry` 之後把整段收回，理由是：cva 表裡
> 只有語意 class 名（`cn-button-variant-default`），樣式住在
> `registry/styles/style-*.css`，換 preset 就換掉整套外觀、元件原始碼不動；
> 而官方 `customization.md` 的順序是「內建 variant → `class` →
> 改原始碼加 variant → wrapper」，改原始碼排第三。
>
> **第二版收回得太多。** 讀完 `cli.md` 與 `SKILL.md` 之後，準確的說法是
> 三件事**同時**成立：
>
> 1. `add` 把元件**原始碼複製進專案**（`--all` 一次全部），可隨意改。
>    **這是它的定義性特徵** —— 第一版對這一點的描述沒有錯。
> 2. 同時有一層 preset CSS，所以「換整套外觀」**通常不需要動元件原始碼**。
>    第二版對這一點的描述也沒有錯。
> 3. 而對一間同時跑 N 個案子的公司，關鍵是第三件 ——
>    **上游修了 bug 之後怎麼傳到那 N 個案子。**
>
> 第三件的官方答案在 `SKILL.md`〈Updating Components〉：`add --dry-run`
> 看影響、逐檔 `--diff`、而**有本地改動的檔案要「讀本地檔、分析 diff、
> 手動套用上游變更同時保留本地修改」**。逐檔、逐專案、需要人判斷的合併。
>
> **所以第一版那個「20 個案子 20 份」的結論是對的，第二版不該連它一起收回。**
> 錯的只有理由：不是「官方叫你改原始碼」，是**散佈模型** —— shadcn 是每個
> 專案的起點，不是共用函式庫，它沒有收斂機制**因為它不需要有**。
>
> ⚠️ 而這一句要寫下來：**對單一專案，shadcn 的模型比這裡的好。**
> 元件多得多、維護得好得多、限制少得多。`platform/ui` 這條路只有在
> 「多個案子共用一條線、而且要能一起升級」時才划算 —— 那正好是接案公司的
> 情況，所以它在這裡合適。**不是它比較差，是位置不同。**
>
> 樣式層不照抄 preset 的理由則獨立於上面全部：**preset 沒有任何閘門在守。**
> `.cn-button-variant-defualt` 打錯一個字產生一個永遠不匹配的 class，
> 畫面安靜地少一塊樣式；具名槽打錯字是編譯失敗。
>
> ── 這一連串錯誤的共同形狀 ──────────────────────────────────────────
>
> 第一版：**沒讀原始碼就引用一個上游專案的設計決定。**
> 第二版：**讀了原始碼，但只讀了一部分就推翻整個結論。**
>
> 兩次都是拿手上剛好有的證據推到最遠。而第二版更值得記，因為它
> **看起來像在修正錯誤**，實際上是用一個新的過強結論換掉舊的。
> 這是本輪第四、第五次（前三次記在 C65／C68／C69）。
>
> 操作上的教訓：**收回一個結論之前，先分開問「前提錯了嗎」與「結論錯了嗎」。**
> 這次是前提錯、結論對，而第二版把兩者一起丟掉。
>
> ── 另外兩件對補元件有直接影響的事 ──────────────────────────────────
>
> 1. **抄 `.vue` 只會拿到沒有樣式的元件。** 樣式在 `style-*.css`，
>    而且用它自己的代幣詞彙（`--primary`／`--muted-foreground`／
>    `--border`／`--ring`），與本 repo 的語意代幣需要一張對照表。
>    漏翻的會被 `theme-verify` 擋下來。
> 2. **它的多檔解剖對應到我們的單檔＋具名槽。** `Card` 是七個 `.vue`
>    （Card／Header／Title／Description／Content／Footer／Action），
>    每個帶一個 `data-slot="card-header"`。我們的 `UiDialog` 已經是這個形狀
>    ——一個檔案包七個 reka 基元、開四個具名槽。**槽名直接用它的 `data-slot` 值。**

#### 六、順帶修掉的一個誤判

第一版把「宣告的槽 ＝ 預設表的鍵」當成主要買點。**它大半是白買的** ——
`Record<X, string>` 是滿的，表少一個鍵 TypeScript 自己就會擋。真正買到的是
**跨檔案的那一段**：`UiThemeOverride` 宣告的槽 union 與元件裡真的有表的槽
對不上時，型別完全合法而新加的槽靜靜地什麼都不做。

條文留著，理由重寫過。**買點寫錯的閘門會在下一次重構時被當成冗餘刪掉。**

---

### C129 — 註解規範進 AGENTS.md：一條明知**不可驗證**所以不設閘門的規範（2026-08-18）

> ⚠️ **這一則原本是 `main` 的 C71，併線時改號**（見 C133）。日期保留原始的那一天 —— 它記的是那天做的決定，不是改號的那天。

規範本身由使用者提供（"Code explains HOW; comments explain WHY"），完整條文寫在
[AGENTS.md](AGENTS.md) 的〈Commenting Standards〉。這裡只記三個決定。

#### 一、放 AGENTS.md，不放 CLAUDE.md

`vp` 會重寫 AGENTS.md，但只重寫 `<!--VITE PLUS START-->`…`<!--VITE PLUS END-->`
之間 —— 這點是讀 `vite-plus/dist/agent-*.js` 的 `getMarkedRange()` 確認的，不是
從標記名稱推的。手寫段落因此加在 END 標記**之後**。

CLAUDE.md 只放一行 `@AGENTS.md`，規則不存在第二份。理由是這 repo 會交接給別的
工具（見 HANDOFF.md）：規範抄兩份，兩份就會開始漂移。

#### 二、刻意不設閘門 —— 而這與 README 的論點相反

README 的立場是「把架構決策寫成閘門，而不是寫成規範文件」。這一條**違反那個立場**，
而理由不是懶：註解的好壞沒有可判定的形狀。能寫成閘門的只有代理指標 —— 註解密度、
註解／程式行數比、禁字清單。

⚠️ **以下是推論，沒有量**：那三個指標看起來都會獎勵錯的行為 —— 密度閘門讓人補廢話
填額度，禁字清單擋不掉 `// 取得使用者餘額` 這種語法完全正確的廢話。這條推論沒有
fixture、沒有實測，寫下來的用途是「日後有人想裝這道閘門時，先量這件事」，
不是結論。C69 在這裡只當**「量完決定不裝」的先例**引用，不主張它的量測結果重現。

#### 三、沒被守到的部分寫在這裡

這條規範**只在 review 時生效，也就是靠人**。日後若要收緊，可行方向只有一個：
變成 review 的檢查項（PR template 或 CODEOWNERS 指到的人），不是變成 CI 步驟。

---

### C130 — 收回一條寫在程式碼裡的規則：「空形狀等於沒有守」守錯了東西（2026-08-17）

> ⚠️ **這一則原本是 `main` 的 C72，併線時改號**（見 C133）。日期保留原始的那一天 —— 它記的是那天做的決定，不是改號的那天。

#### 一、被收回的那一句

`tools/api-surface` 的 SFC 解析原本在最後一步無條件丟例外：

> `SampleWidget.vue` 沒有解析出任何公開面 —— 空形狀等於沒有守。

那句話本身是對的。**但它擋的不是空形狀，是純版型元件。** 一個
`Separator`／`Skeleton`（沒有 prop、沒有 slot、沒有 emit、沒有 model）
在 `platform/ui` 裡連 build 都過不了，而它們正是元件庫「不封頂」之後
下一批要抄進來的東西。

這與 C128 之前那道「原始碼裡出現 `defineEmits` 就丟例外」是**同一種錯**：
絆線綁在**宣告的形式**上，而不是綁在它要保護的那件事上。同一個形狀，
一個月內第二次。

#### 二、真正要擋的是什麼

不是「沒有公開面」，是「**有公開面但這支解析讀不出來**」。四個巨集裡
只有 `defineProps` 從第一天就有那條絆線，另外三個沒有 —— 而
`defineEmits(["picked"])`（執行期陣列形式）會讓**所有事件安靜消失**，
基準檔看起來仍然記著一份完整的元件形狀。少算比不記更難發現。

所以順序是：**先把四個巨集的絆線補齊，才放行空形狀。**
⚠️ 顛倒過來的話，中間那段時間裡任何寫錯形式的巨集都會安靜地變成一個
「合法的零公開面元件」—— 一個比原本的例外更糟的洞，而且是在專門補
這種洞的改動裡加進去的。

#### 三、兩條互斥的正規式之間一定有縫

`defineModel` 原本用兩條樣式分擔具名與不具名，靠 `\(\s*[^"'\s]` 這種前瞻
互相排除。review 在那兩條之間找到三個縫：
...
三者的症狀一模一樣：那個 model 的 prop 與 `update:` 事件不進 API 表面。
**沒有紅燈，沒有訊息，基準檔看起來很完整。**
...
#### 四、量出來的：只斷言「會紅」的反向測試是假的

補完之後做變異測試：**7 處絆線各自拿掉再跑，8 條測試各自紅在該紅的那一次**
（7 條新的，加一條既有的「寫了 `defineProps` 但用執行期物件形式」）。

第一輪同時拿掉三處，五條測試全紅 —— **但看失敗訊息才發現它們差一點就是
裝飾品**：拿掉絆線之後閘門仍然是紅的，因為那個元件的成員少了一堆，
於是判成「破壞性變更」。

也就是說，`expect(result.red).toBe(true)` 這一句在絆線消失之後**仍然會過**。
那一輪五條裡有五條。

> **反向測試要斷言的是「為什麼紅」，不是「有沒有紅」。**
> 一個檔案裡通常只有少數幾條路會通往紅燈，而它們彼此會互相掩護。
> 這一條之後對所有 `🔴` 測試都適用：斷言要挑**只有這條路會印出來**的字串。

（改法：把 `toContain("defineEmits")` 換成 `toContain("不是型別參數形式")`
—— 前者在 `assertDeclared` 的「請補進 `defineEmits<{…}>()`」裡也有。）

⚠️ **這一節的數字第一版是憑印象寫的，而且寫錯了**（記成「七次變異、七條
新測試」，實際是六次、六條，第五條紅的是既有那條）。一個講「關於測試的
未經查證的宣稱就是失敗模式」的段落，自己犯了同一件事 —— 補上第七條反向
測試（重複巨集）之後數字才對得起來。**C53(c) 適用於自己**：
沒有事實來源的計數不要寫，包括寫在 DECISIONS 裡的。

#### 五、`members: []` 在這個 repo 裡有兩個意思

放行空形狀之後記下來的是 `{ kind: "component", members: [] }`。而
`shape.ts` 自己的慣例是**空成員清單代表「改比對型別字串」**
（見 `typeShape` 與 `objectMembers`）—— 元件的形狀沒有 `type` 欄位，
照那個慣例讀的話兩半都比不到，這個元件之後長出來的每一個 prop 都不會漂移。

實測 `compare.ts` 判的是 `members !== undefined`，所以它是對的。
但那是**兩個檔案剛好對上**，不是一條寫下來的約定 —— 所以把它變成一條
反向測試（種一份空基準，再加一個 prop，斷言會紅），並且用變異驗過：
把 `compare.ts` 改成 `members.length > 0` 那條測試就紅。

#### 六、代價

`api-surface` 的 SFC 解析變嚴了：具名型別別名形式的 `defineProps<Props>()`
現在會紅。三個既有元件都不受影響（實測基準逐字相同），但下一個從別處
抄元件進來的人可能要改寫一次宣告。訊息裡寫了為什麼。

---

### C131 — 閘門底下那一層抽成 `tools/gate-kit`：三個 export，而不是四個（2026-08-18）

> ⚠️ **這一則原本是 `main` 的 C73，併線時改號**（見 C133）。日期保留原始的那一天 —— 它記的是那天做的決定，不是改號的那天。

11 支閘門之間沒有任何共用 module。「repo 根在哪、走哪些目錄、旗標怎麼讀」
各自實作了一次 —— 而它們已經互相矛盾到會說謊：

```
$ node tools/conformance/src/cli.ts --roots /tmp/does-not-exist
exit 0          ← 旗標打錯字，最大的那道閘門掃了真的 repo 然後回報綠燈
```

`--roots` 不是 `--root`。`conformance`、`api-surface`、`compliance`、`doc-facts`
對任何不認得的旗標都 exit 0。11 支裡只有 `pii-check` 擋得住，而它那道防線是
**被一次真實事故逼出來的**（C52 拿掉 `--masking` 之後，`tier2-security.yml`
那個步驟被留下來，頂著「個資：畫面上必須隱碼」的名字回傳綠燈，而 ⑨ 早就沒有
東西在守）。教訓學到了，只套用在 11 支裡的 1 支 —— 因為沒有地方放。

#### 一、第一版只出三個 export，`report()` 與 `Finding` 不在裡面

原本的提案是四個。刪掉第四個的理由不是保守，是**現在沒有任何一支工具產出
`Finding[]`**：`theme-verify` 用的是 module-level 的可變計數器、`fail()` 當場
列印；`pii-check` 成功時印的三行散文講的是它**抓不到什麼**（「姓名抓不到 ——
見 src/detect.ts」），沒有泛用格式生得出來。

先定義一個零實作的型別，就是「一個 adapter 等於一個假想的接縫」。而一個只做
`n > 0 ? 1 : 0` 的 `report()` 過不了刪除測試 —— 拿掉它重新出現的是 11 行算術，
複雜度沒有集中，只是搬家。`Finding` 該由「把判定從 `cli.ts` 拆出來」那一步引入，
那時它才有第一個生產者。

**代價**：`gate-kit` 先做的理由因此縮成「它今天就在說謊」這一條，不再是
「其他兩件事的地基」。那條理由仍然夠，但它比原本弱，寫下來以免日後被追認成
更宏大的計畫。

#### 二、跳過清單是參數，不是政策

八份跳過清單互相矛盾，`conformance` 最短（只有 `node_modules`／`dist`，
而且內聯了兩次，連常數名字都沒有）。直覺是順手統一成一份。

**那會讓閘門變弱。** 給 `conformance` 一份聯集清單，它會開始跳過 `coverage/`、
`.git/`、`.scan/` —— 不再讀今天讀得到的檔案，**紅可能變綠**。一道閘門悄悄變弱
比誤報難發現得多。而按 C41：一份統一清單只要在 `conformance` 上誤報一次，
第一天就會拿到例外，例外不會再拿掉。

所以 `skip` 是 `string[]` 參數。分歧從八個檔案收攏進同一個型別上看得見，
行為一格都不動。調和是下一步，要帶自己的反向測試。

同理，`skip` 與 `extensions` 都是**資料而不是述詞**。八份清單裡只有兩份用了
非字面規則（同一條 `startsWith(".")`，收成 `skipDotDirs`），七份副檔名過濾裡
六份已經是後綴清單。用 `(name) => boolean` 會讓兩份清單沒辦法機械比對，
而那正是這次唯一的收穫。

#### 三、`parseFlags` 回傳判別聯集，不在函式裡 `process.exit`

被取代的實作是在 `parseRoot()` 裡直接離開，於是那道防線只能靠起行程來測 ——
`pii-check` 有四條 `spawnSync` 測試，其中一條在斷言**訊息的措辭**
（`expect(result.stderr).toContain("綠燈")`）。那段措辭搬進 `gate-kit` 之後
變成所有閘門共用，卻只能起行程驗。

改成 `{ ok: true; flags } | { ok: false; message }`，那整套政策與措辭現在是
純函式測試。「忘了處理 `ok: false`」不需要另外守：沒先收窄 `.ok` 就碰 `.flags`
是型別錯誤 —— 寫這一版時我自己的測試就是這樣紅的，那條後來留成一條
`@ts-expect-error` 的測試。

#### 四、驗收條件換過一次，因為原本那條是空的

原本寫「`api-surface` 的基準檔一個位元組都不動」。**那對只動 `tools/` 的改動
會自動成立** —— `tools/api-surface/src/cli.ts:38` 是
`const PLATFORM_DIR = join(ROOT, "platform")`，它根本不掃 `tools/`。

改成：乾淨樹上先錄下 11 支 CLI 的 exit code 與 stdout／stderr，改完逐支比對。
結果 **10 支一字不差**；第 11 支 `pii-check` 從「掃了 67 個檔案」變成 70，
而那 3 個是 `gate-kit` 自己的測試檔（`SCAN_RULES` 第一條吃 `/tests/`）——
把 `tools/gate-kit/tests` 移開再跑一次是 67，移回來是 70，實測歸因不是推理。

兩個例外寫進錄製腳本：`exit-drill` 不加 `--full`（那會真的建置），
`csp-verify` 改錄 `--print-probe` —— 它的預設模式是一支長駐在 `:4173` 的
伺服器，給人開瀏覽器點對話框用的，**不是一道檢查**，永遠不會結束。

#### 五、錄製當場撞到「`vpr ready` 是假的」

只加了一個空 package、還沒改任何一支工具的時候，`vpr gate` 全綠 ——
而 `doc-facts` 紅了：README 寫著 27 個 workspace 套件，加一個變成 28。

`doc-facts` 不在 `scripts.gate` 裡，也就不在 `ready` 裡，只在 CI 跑。
所以本機 `vpr ready` 全綠、推上去 CI 紅，而 README 兩處都把 `vpr ready`
講成推送前的完整檢查。**這件事這次是撞到的，不是推論出來的**，
但修它要重排三份手抄的閘門名冊，不在這個 PR 的範圍。

#### 六、pilot 選 `pii-check` ＋ `theme-verify`，不是只選一支

C55 的先例是「先做一個、量成本、再決定批量」。照抄有問題：`pii-check` 是唯一
已經做對的那支（它就是 `parseFlags` 的來源），單獨拿它當 pilot 等於照著它的
形狀刻一個 module 然後發現它完美貼合 —— 證明不了任何事。C55 的價值來自
`UiInput` 暴露的兩個缺口，不是來自它順利落地。

第二支挑在每一項上都相反的：`theme-verify` 的跳過規則是述詞不是清單、
完全不讀 argv、走完當場把內容讀進 `Map`。兩支加起來把 `skip` 的型別、
空 spec 的語意、`walk` 該不該讀檔全部逼出來。

**空 spec 的語意是新判定**：`node tools/theme-verify/src/cli.ts --anything`
以前靜靜 exit 0，現在紅。「沒有旗標」很容易被實作成「什麼都放行」，
而那正好讓最不設防的那幾支繼續不設防 —— 有一條測試釘住這件事。

#### 七、一個更正

架構檢視的報告裡寫「14 支閘門」。實際是 `tools/` 底下 15 個目錄、**11 個有
`src/cli.ts`**，14 兩邊都不是。同一份報告裡「28 份 `repoRoot` 複本」也是錯的
—— 那是 `grep -rl fileURLToPath` 的**檔案數**，其中有 `const HERE`／`FIXTURE`／
`CONFIG` 這些套件相對路徑，`repoRoot()` 取代不了。機械推導後是 26 處、5 種寫法。

兩個都是 C53(c) 講的那件事：**沒有事實來源的數字不要寫**。寫在這裡是因為
那份報告的三張卡有兩張在罵這個毛病。

---

### C132 — 閘門名冊接到 `main`：同一個缺陷的**第三次**發作，以及這裡多出來的三種閘門形狀（2026-08-18）

> ⚠️ **這一則原本是 `main` 的 C74，併線時改號**（見 C133）。日期保留原始的那一天 —— 它記的是那天做的決定，不是改號的那天。

`tools/gate-roster` 原本寫在 `release/v1`（那條線的 C129）。依 v1 的範疇判準
（**看它守的東西給誰看**）它不屬於 v1 —— 閘門清單漂移傷的是維護者，不是拉 v1
去做案子的團隊 —— 所以 `v1.0.3` 把它移除，搬到 `main`。**而 `main` 才是它更需要
被裝上的地方**：這裡的 `tools/` 底下有十五個套件，清單更長、更容易漂。

#### 第三次發作，在同一格

把名冊原封不動接上來，當場 **26 個問題**。其中最該被記住的一條：

> **README〈兩層檢查〉的 Tier 2 那格寫著「一致性檢查 + ESLint 安全規則」，
> 而 Tier 2 實際上跑十道。** Tier 1 那格也漏了 `.vue` 型別檢查。兩格加起來
> **漏了九道**。

這個缺陷 C129 抓到過（那是第二次），而 `v1.0.2` **只修了 `release/v1` 上的那一份**
—— `main` 這一份從來沒被碰過。同一個位置、同一種錯、第三次。

值得記下來的不是「又錯一次」，是**它每次都錯在最會被讀的那一格**：
`README`〈兩層檢查〉正是讀者判斷「我的 PR 會被什麼擋下來」的地方。
`doc-facts` 的檔頭記過同一個形狀（摘要表最先被讀、最後被登記）。
**被單獨拿出去用的那一格，守備等級反而最低。**

#### 同一個 `tools/sast/` 幽靈，兩條線上都有

順手發現的第四件事，這道閘門**抓不到**：README 的目錄結構那棵樹列著
`tools/sast/`，而那個目錄**在兩條線上都不存在**。自寫的汙點傳遞規則住在
`.semgrep/`，由 tier2 用釘住 digest 的容器跑 —— 而 `.semgrep/` 在那棵樹裡
根本沒出現。一個假的項目、一個缺席的真項目。

`release/v1` 的 `v1.0.4` 才剛修掉自己那一份，`main` 這一份是同一個字。
名冊守的是〈兩層檢查〉那張表，不是目錄樹；`doc-facts` 守的是數字，不是清單。
**兩道閘門中間有一條縫，而這棵樹正好掉在縫裡。**

#### 兩支閘門在 CI 跑，而不在 `scripts.gate` 裡

`doc-facts` 與 `pii-check` 都在 tier2 跑，兩支都不在本機的 `vpr gate` 鏈上。
**那是 PR #51 修過的那個毛病，一字不差** —— 本機 `vpr ready` 全綠、推上去 CI 紅，
而 README 有一節就叫〈一次跑完所有檢查〉。v1 那邊修好了，`main` 這邊沒有人發現。

加上 `vue-typecheck` 沒有單獨跑的別名（`scripts.gate` 跑它，但 `vpr vue-typecheck`
不存在）。這三件都不是「接線工作」，是這道閘門一上線就抓到的真漏洞。

#### `main` 上多出來的三種形狀

那份資料原本的世界很整齊：每一支 `tools/*` 都是一支 CLI、每一道閘門在 CI 剛好
跑一次、而且每一道都在 `scripts.gate` 裡。**`main` 三件都不成立。**

| 形狀                        | 誰                                                                               | 怎麼表達                                                     |
| --------------------------- | -------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| 同一支工具在 CI 跑不只一次  | `exit-drill --require-fresh`、`compliance --evidence`、`supply-chain` 的兩個模式 | `variants`，**每一項理由必填**                               |
| 閘門刻意不進 `scripts.gate` | `bff-check`（本機由 `vp run -r test` 跑到，放進去等於同一批測試跑兩次）          | `notInGateScript`，**它就是那句理由**                        |
| 閘門沒有 `src/cli.ts`       | `bff-check` 是一包測試，CI 直接對它跑 vitest                                     | `ciCommand` ＋ `GATE_SHAPED` 認得 `vitest run --root tools/` |

⚠️ **三個欄位都是字串不是布林，那是刻意的。** 判準只有一條：
**一個擴充如果強迫寫下一句下一個人可以反駁的理由，它是合法的；
如果它只是一個讓檢查閉嘴的開關，它就是 C41 說的那種例外。**
`skipCheck: true` 會被第二個人照抄，`why` 不會。

`supply-chain --split-lockfile .scan` 是這條判準的試金石：**它根本不是判定**，
是替 Trivy 把 lockfile 拆成讀得懂的形狀。它長得像閘門只因為它也是那支 CLI 的
一個模式。理由欄裡就這樣寫著 —— 誠實地留在散文裡，而不是把一個非閘門藏進
閘門的紀錄。

#### `exit-drill` 的 id 不去搶一個已經有意思的名字

`vpr exit-drill` 現在的意思是「跑完整演練」（`--full`，數分鐘、連網、每季一次），
README、`tools/exit-drill/README.md` 與 cli.ts 的錯誤訊息都這樣引用它。
而閘門跑的是**另一件事**：只驗退出面、不建置。

把別名改成 bare 會讓那四處變成假話。所以閘門的 id 取作 `exit-surface`，
`exit-drill` 原封不動。**給原本沒有名字的那一半一個名字，比搶走一個有意義的
名字再去改四處文件好。**（`pii` 也是同一手：`id: "pii"`／`pkg: "pii-check"` ——
③ 比對 `scripts[gate.id]`、① 用 `gate.pkg` 登記，兩者本來就是分開的欄位。）

#### 八支工具，八次判斷

| 工具           | 判斷              | 憑什麼                                                                 |
| -------------- | ----------------- | ---------------------------------------------------------------------- |
| `bff-check`    | **閘門**（tier2） | R6 處置的驗收器；沒有 cli.ts 是形狀問題，不是資格問題                  |
| `compliance`   | **閘門**（tier2） | 對照表是產生的，人改了原始碼沒重產就紅                                 |
| `exit-drill`   | **閘門**（tier2） | D2 保單。PR 只驗退出面，完整演練每季一次（C64）                        |
| `supply-chain` | **閘門**（tier2） | 盤點與來源綁定進版控，漂移會紅                                         |
| `pii-check`    | **閘門**（tier2） | §11 II ⑥。CI 早就在跑，只是本機沒接上                                  |
| `gate-kit`     | `UNGATED`         | **它是 library**（C131），沒有 cli.ts，壞了會讓消費它的閘門紅          |
| `csp-verify`   | `UNGATED`         | **要人開瀏覽器**。`--record`／`--verify` 已因每次成本最高而移除（C52） |
| `ui-survey`    | `UNGATED`         | **選型市調，不是判定**（D15 / #49）。別人改授權不代表這個 repo 壞了    |

`UNGATED` 的理由必填，這樣「漏接」與「刻意不接」才不會長得一樣（C41）。
`csp-verify` 那一格特別值得讀：它記的是**失去了什麼** ——
「有沒有人真的驗過 CSP」這個問題現在沒有機器答案。

#### 守備範圍的第二個洞

除了 semgrep／gitleaks 那些 docker 與 `uses:` 步驟，`main` 還有**第三個 workflow**：
`exit-drill.yml`（每季排程的完整演練）。這道閘門只讀 tier1 與 tier2 兩個檔案，
**對它什麼都不說**。

沒有納進來，是因為它跑 `--full` —— 那不是 PR 上的閘門，而名冊的 tier 模型只有
兩層。硬塞會讓 `Tier` 變成 `"tier1" | "tier2" | "每季"`，而第三個值只有一個成員。
一道只守半個檔案的閘門，必須自己講明白守的是哪半個 —— 兩個洞都寫在
`gates.ts` 的檔頭與 CLI 的輸出裡。

#### 三個新欄位各自被突變測試驗過

反向測試 32 條（原本 23 條）。三個模型改動各還原一次：
`GATE_SHAPED` 拿掉 vitest → 4 條紅；`deriveGateScript` 不過濾 `notInGateScript`
→ 1 條紅；`deriveTierCommands` 不看 `variants` → 2 條紅。**還原後 32 條全綠。**

新測試裡有一條是反方向的：**登記了 variant 之後它就必須真的在**。
`variants` 讓那些行不再被誤報成「多一道」，但代價是「少一道」也要能看見 ——
否則 `--evidence` 那一步被誰刪掉，名冊會說一切正常。

#### ⚠️ 這個 C132 不是 `release/v1` 的 C132

兩條線的 C 編號從 C128 起就分岔了（`main` 的 C128 是〈第二條軸只做在一個元件上〉，
v1 的 C128 是〈承諾有五條，檢查只有四條〉）。v1 那邊的 C129 是**這件事的前身**、
C130 是**範疇判準**；`main` 這邊 C129 起是另一串。
**跨分支引用 C 編號之前先確認自己在哪一條線上。**

---

---

### C133 — `release/v1` 併回 `main`：而天真的合併零衝突刪掉 71 個檔與 13 個 CI 步驟（2026-08-26，#86／#159）

> ⚠️ **這一則同時是編號改號的對照表**（§三）。讀舊 PR、舊註解、或別人的筆記
> 讀到「`main` 的 C73」時，來這裡查。

#86 那條凍結的解除條件有兩件：`release/v1` 併回 `main`，以及該分支被刪除。
地圖是 #159（那張票只畫地圖、不做合併）。這一則記的是**執行時發生的事**，
其中有三處推翻了地圖自己的判定。

## 一、為什麼解決規則不是「取 `release/v1`」

`49b36da` 的 commit 訊息是「縮到五條承諾，**其餘留在 `main`**」。
`release/v1` 在那一版刪了 65 個檔，`main` 之後沒有再動過它們 ——
一邊刪、另一邊沒動，**合併照做，不問**。所以那句話會被逐字撤銷，
而且**不出現在任何一條衝突裡**。

實測（合併樹 `c94cc1d`）：

```bash
git diff --diff-filter=D --name-only origin/main "$TREE"        # 65
git diff --diff-filter=D --name-only origin/release/v1 "$TREE"  # 0
```

⚠️ **而 65 是低估的**：`--find-renames` 把「刪七支工具＋新增三支」配成六個改名
（`package.json`／`tsconfig.json` 是幾乎一樣的樣板），那六個檔在 `D` 那一欄
看不見。**實際不存在的是 71 個。** 正確的量法是逐檔問「它在不在工作區」：

```bash
for f in $(git ls-tree -r --name-only origin/main); do [ -e "$f" ] || echo "$f"; done | wc -l
```

七支工具（`bff-check`／`compliance`／`csp-verify`／`exit-drill`／`supply-chain`／
`ui-survey` 整支，`pii-check` 的五個檔）、無障礙閘門的實作四個檔、
兩個排程 workflow、`UI-SURVEY.md` —— **全部復原**。逐工具的裁決照 #159 §四。

## 二、⚠️ 而「整檔消失」是比較好抓的那一類 —— 活檔案裡掉行才是大宗

`--diff-filter=D` 看不見兩個 workflow 裡消失的 **13 個 CI 步驟**（檔案都活著），
其中四步是整條 SCA／SBOM 管線，而 `gate-roster` 檔頭自己聲明過對 `uses:` 步驟
什麼都不說。處置：兩個 workflow **以 `main` 為底重建**，再套上 `release/v1`
真正新增的三件事（`spec-report --check`、`promise-check`、`set -o pipefail`）。

復原七支工具之後，`git diff --numstat` 還剩 58 個「掉行候選」。
**判準是「被移除的行有沒有對應的替代」，不是「有沒有掉行」**（#159 §六）。
逐檔判完的結果：

| 檔案                                                                                                                                                                              | 判定                                             |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| `doc-facts` 的六個事實（`packages`／`native`／`families`／`no-slsa`／`slsa`／`handoff-items`）＋ `handoffItemCount()` ＋ `GUARDED` 裡的 `UI-SURVEY.md` ＋ 對應的三個測試 describe | **真流失**，復原                                 |
| `CODEOWNERS` 五列安全共管擁有者 ＋ 它們的理由註解                                                                                                                                 | **真流失**，復原                                 |
| `pnpm-workspace.yaml` 的 `eslint-plugin-vuejs-accessibility` ＋ `platform/eslint-config` 的 `./a11y` export 與相依                                                                | **真流失**，復原（兩者一起，少一個就是裝不起來） |
| `vue-typecheck`／`bff-contract` README 各一節、`renovate.json` 的 `needs-exit-drill`、`apps/console/vite.config.ts` 的 `DRILL_PLUGINS` 註解、`platform/pii` 測試檔頭那句          | **真流失**，復原 —— 它們都在說一支被復原的工具   |
| `.semgrep/rules.yml` 的前導斜線錨定、`theme-verify` README 的 ASCII 例子、`slice-gen` 的 `VITE_CONFIG`、`platform/ui/theme.ts` 對 shadcn 模型的更正                               | **升級**，留 `release/v1` 的                     |

⚠️ **這張表的價值在最後一列**：掉行大多是改寫。把「掉行」當成「流失」會把
一批升級退回去。

## 三、⚠️ 編號撞號：地圖說「沒有重號」，而它有五格

#159 §三 寫著「✅ **沒有重號**：`main` 獨有 C90／C113，`release/v1` 兩則都刻意
跳號」。前半是錯的：**C70–C74 兩條線各用了一次，五則內容完全不同。**
（`release/v1` 跳過的是 90 與 113 這兩個號，不是 70–74。）

判準是**哪一邊的編號已經被送出去了**：

|            | `main` 的五則                                                        | `release/v1` 的五則                                                               |
| ---------- | -------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| 樹上的引用 | 17 處，**全部是內部**（`gates.ts`、`gate-kit` README、一個測試標題） | 48 處，含 `CHANGELOG.md`（9）、`SCOPE.md`、`HANDOFF.md`、`TESTING.md`、元件原始碼 |
| 樹外       | 無                                                                   | **已發佈的 GitHub Release notes** 引用 C125／C126                                 |

所以**改號的是 `main` 的五則**。對照表：

| 原本（`main`） | 現在     | 標題                                                           |
| -------------- | -------- | -------------------------------------------------------------- |
| C70            | **C128** | 設計系統的第二條軸只做在一個元件上，而檢查寫死了那個元件的檔名 |
| C71            | **C129** | 註解規範進 AGENTS.md                                           |
| C72            | **C130** | 收回一條寫在程式碼裡的規則：「空形狀等於沒有守」守錯了東西     |
| C73            | **C131** | 閘門底下那一層抽成 `tools/gate-kit`                            |
| C74            | **C132** | 閘門名冊接到 `main`                                            |

⚠️ **五則的日期保留原本那一天。** 它們記的是那天做的決定，不是改號的那天 ——
所以檔尾的日期會往回跳，那是對的，不要「修好」它。

⚠️ **舊條目裡那些跨線警告一個字都沒有改**（例如 C74 §開頭「`main` 的 C72 是這一條」）。
它們是**有日期的歷史陳述**，而這份日誌不守歷史（同一條理由讓 `doc-facts` 不守
`DECISIONS.md`）。讀到它們時對照上面那張表。**活的東西才改**：
`gates.ts`、`gate-kit/README.md`、`tier1-quality.yml`、`conformance/src/finding.ts`、
`scope-check/src/check.ts` 與它的測試，都已指向新號。

⚠️ 順帶：`scope-check` 的訊息原本寫「`release/v1` 的 C72」（C96 加的限定詞，
理由正是這次的撞號）。**併線之後那個限定詞比裸寫更糟** —— 它指向一條
即將不存在的分支。改回 `C72`，測試跟著換成「指得到判準」＋「不得再提那條分支」。

## 四、地圖過期的另外兩處

- **`theme-verify` 不必「換回 `@org/gate-kit`」**：C126 之後兩條線都有 `gate-kit`，
  它們是 `add/add`。`src/flags.ts` 與 `package.json` **逐位元組相同**（`cmp` 驗過），
  `src/index.ts` 取 `main` 的（三個 export 是超集），README 併雙方。
- **`conformance` 的 `add/add` 是七個檔不是四個**，另加 `gate-kit` 三個 ——
  衝突路徑實際是 28 條不是 23 條。處置不變（同一次拆分之後的漂移，取 `release/v1`）。

## 五、閘門名冊的目標狀態，以及它逼出來的四件真修正

`promise-check` → tier2；`spec-report` → tier1；`scope-check` → `UNGATED`。
`scripts.gate` 由 `deriveGateScript()` 重產，不手抄。三件連帶：

1. **`notInGateScript` 長出第二種成立情況。** 原註解寫著「只有一種情況成立」
   （本機已有別的東西涵蓋它）。`spec-report` 是第二種：**它吃測試留下的
   `.vitest-results.json`，而 `vpr gate` 不跑測試。** 註解改成兩種，並寫明
   「加第三種就改這段話」。
2. **`ciCommand` 也長出第二種。** 原本只有 eslint 需要它；`spec-report` 的
   CI 那一行帶 `--check`（判定模式），而別名不能是判定模式 —— 否則
   `vpr spec-report` 就再也產不出報表。
3. **`SCOPE.md` 一個字都不改**，工具進 `UNGATED` 並寫明「`main` 的範疇清單等
   #90／#93」。⚠️ 對這棵超集樹跑它會列出 10 項，而**那 10 項全部是同一個方向**
   （樹上有、沒登記）—— `scope.test.ts` 的斷言因此從「零問題」換成
   「每一項都必須是那個方向」，反方向出現一項仍然當場紅。

**併線逼出來的四件真修正**（它們都不是「合併沒合好」，是兩半從來沒有見過面）：

- **不認得的旗標一律紅**擴到 `gate-roster`／`exit-drill`／`supply-chain`／`compliance`。
  `gate-kit/tests/adoption.test.ts` 的名冊是從 `scripts.gate` ＋ `scripts.ready`
  **推導**的，這四支一進閘門鏈就紅。⚠️ 蒐旗標要掃**四個**地方，不是三個 ——
  多的那個是**排程 workflow**（`--require-fresh`、`--recapture-safe` 只出現在那裡，
  而名冊測試看不見它們，漏了會在下一次排程才炸）。
- **退出演練會在下一次排程壞掉。** 切片自 C120 起各帶一支 `vite.config.ts`
  （覆蓋率門檻只收得進 package 自己的設定），而它們 `import ... from "vite-plus"`。
  演練複製套件目錄時沒有把它們拿掉 —— 與 `dependencies.ts` 檔頭記的 PR #15
  **完全同一個形狀**：壞掉、每季才跑、19 個 PR 沒人知道。
  ⚠️ 同一批：`@vitest/coverage-v8` 要進「明示不裝」那張表，理由不是「用不到」。
- **`D2 退出面`的判準從寫死路徑換成形狀。** 三個檔被報成「退出面擴大」：
  兩支切片的 `vite.config.ts`（**它們就是設定檔**，規則本來就允許）與
  `apps/console/tests/dev-session-stripped.test.ts`（它**用建置器問一個關於建置器
  的問題**）。前者改判形狀（`*/vite.config.ts`）——⚠️ 寫死路徑的話，
  `slice-gen` 產出的每一片新切片都會在第一天撞到這道紅燈，而「加第一片切片」
  正是採用指南教的第一件事（C95／C97 的形狀）。後者進一張**逐條寫理由**的
  具名例外表，與 `EXIT_SURFACE` 分開 —— 那一份是「演練要重建哪幾份設定」，
  混在一起會讓演練去重建一支測試檔。
- **六支 CLI 的檔案模式**在版控裡是 100644，而 `package.json` 宣告它們是 bin。
  #141 只修在 `release/v1`，而守它的閘門（C122）也只在那裡 —— 兩者一相遇就紅。

## 六、`HANDOFF.md`：兩份不同的文件共用一個檔名

`main` 的是〈交接清單〉（1652 行，26 個 `## N.` 項次），`release/v1` 的是
〈採用指南〉（418 行）。**兩份都是載重的**：前者被 `doc-facts` 的
`handoffItemCount()` 數著、被 README 引用；後者被 `AGENTS.md`、`TESTING.md`、
`promise-check/README.md` 按檔名指過來。

處置是**併進同一個檔案**，採用指南在前、交接清單在後，中間放一段說明兩半
是給誰看的。⚠️ **採用指南的標題整體降一級**，因為 `handoffItemCount()` 用
`^## N.` 數項次 —— 交接清單那半的 `## N.` 不得再降級。拆成兩個檔案的方案被
否決：那會讓上面每一處按檔名的引用一次全斷，而且 `doc-facts` 的 `GUARDED`
要多守一份沒有人指名的文件。

## 七、⚠️ 無障礙閘門在這一天**第一次真的被執行到**

`main` 的 a11y 閘門（C60／C69）跑在 3 個元件上、一個 `<label>` 都沒有；
`release/v1` 的 24 個元件從來沒有被它量過。兩半相遇，4 個紅燈：

- 三個是 `label-has-for`。它的預設是 `{ every: ["nesting", "id"] }` ——
  **同時**要求 label 包住控制項**而且**帶 `for`。WCAG 兩種關聯任一種就成立，
  所以那個預設比它宣稱在守的標準更嚴，而且**它否決掉的正是 `for` 這一種**。
  改成 `some`。一個既沒 `for` 也沒包住控制項的 `<label>` 仍然會紅。
- 一個是 `form-control-has-label` 打在 `UiTextarea` 的 `<textarea>` 上。
  那條規則問的是「控制項自己身上有沒有 `id`」（它自己的註解說掃全檔找 `for`
  太慢），而這一層的基元**刻意不宣告 `id`** —— 由使用端經 `$attrs` 落下來，
  `UiField` 產一組再一次綁上（C84）。關掉，**範圍只有
  `platform/ui/src/components/**`**：切片與應用畫面裡的控制項仍然被它守著。
  ⚠️ 同一組基元裡 `<input>` 沒紅，而那是**巧合不是差別** —— 規則對沒有 `type`
  的 `<input>` 直接提早 return。

⚠️ **這一節不是「調鬆門檻換綠燈」，而分辨的方法是問綠燈以前代表什麼**：
併線前那道閘門是綠的，因為**沒有東西被檢查**。這正是 `a11y.js` 檔頭
自己寫下的警告（「綠燈不代表頁面可用」）第一次應驗在它自己身上。

## 八、層 1 複雜度的門檻重新校準

C119 的四個數字（185／5／6／36，測試碼 455／3／4／11）是
**`release/v1` 那棵樹的觀測最大值**，而那棵樹是這棵的子集。七支工具併回來之後
三處超出：`supply-chain` 的 `captureOne`（循環複雜度 39）、`exit-drill` 的
`runFull`（239 行）、`bff-check` 測試裡那個 mock 伺服器 handler（複雜度 15）。

⚠️ **重新量、重新設，而那正是 C119 定下的方法** ——「先設寬到不擋任何既有
程式碼，每擋下一件事就記一則 C 編號，收緊時附上那些 C 編號當論證」。
舊值從來不是「這棵樹的最大值」，它是**另一棵樹**的最大值；照抄它等於在第一天
擋下七支一行都沒改過的工具（C108 已經付過那筆學費）。新值：**239／5／6／39**，
測試碼 **455／3／4／15**。

## 九、`pii-check` × json reporter：一個只在「跑過測試之後」才出現的紅燈

`vp run -r test --reporter=json` 每個 package 留一份 `.vitest-results.json`，
內容含**毫秒時間戳**，而 13～16 位的時間戳有相當比例通過 Luhn 校驗 ——
於是 `pii-check` 報出三個「信用卡號的形狀」，而那些數字是時鐘。

⚠️ **症狀最值得記**：先跑閘門是綠的，跑過測試再跑才紅。`vpr ready` 的順序
（測試在前、閘門在後）保證每次都會踩到，而單獨跑 `vpr gate` 不會 ——
一個看起來像幽靈的紅燈。處置：`SKIP_FILES` 濾掉那個檔名，
...
- ⚠️ **完整的退出演練沒有實跑。** 它要連網、二十分鐘、每季一次。
  §五 那個修法（刪掉複製過去的切片設定）是**讀出來的**，不是量出來的 ——
  下一次排程是它第一次真的被驗證。
- **`docs/adr/` 遷移（#87）不動。** #159 §三 的排序：先合併再遷移，
  在兩份日誌上做同一次遷移等於做兩次。
- **`main` 的範疇邊界不定義。** 那是 #90（清單與判準）與 #93（閘門設計）的產出。
  這次只保證 `scope-check` 與 `SCOPE.md` 沒有被弄丟。
- **`csp-verify` 沒有實跑**（要人開瀏覽器），它在 `UNGATED` 裡，理由沒有變。

---

### C134 — 收回 `SCOPE.md:34` 的准入效力：判準還會分類，但它判出的「在外」已經沒有地方可去（2026-08-27，#87／#90／#93）

C72 定下的判準寫在 `SCOPE.md:34`：

> 判定時**必須寫得出「受益者是拉 v1 的團隊」那一句**。寫不出來就不准進。

`release/v1` 併回 `main`、分支刪除（C133）之後，這句話還算不算准入門檻，
擋著 #87 開工。裁決是**不算**。但「不算」有精確的範圍，而下面三節的順序
就是那個範圍 —— **只有第二節是裁決，另外兩節是它的邊界。**

## 一、問法還活著

`SCOPE.md:36` 的問法是「**這東西紅了，是誰的問題被擋下來？**」

它問的是受益者的**角色**，不是分支的名字。「交付物的使用者 vs 維護者」
這條線在只剩一棵樹的時候照樣分得出來 —— `SCOPE.md:8-11` 早就預演過：
fork 去做案子的團隊讀同一份文件、填同一張表，那幾格寫自己的理由。

**這一節沒有被收回。** 判準作為一個分類問題，一個字都沒有失效。

## 二、後果死了 —— 這一節才是裁決

判準之所以是門檻，不是因為它會問問題，是因為它有**兩個出口**：
准進的往 `release/v1`，不准進的送 `main`（`SCOPE.md:34`、`36-37`）。

`release/v1` 沒了之後，「送 `main`」等於「留在原地」。兩個出口通往同一棵樹。

**它從准入門檻退化成標註** —— 還會蓋章，但蓋哪個章都不影響東西走哪扇門。
這不是措辭過期（把「拉 v1 的團隊」改讀成「採用腳手架的團隊」救不了），
是機制少了另一端。

⚠️ 順帶：`scope-check` 的紅燈訊息現在逐字印著「寫不出來就送 `main`」。
那是 C133 §三 才剛改掉限定詞的結果（原本寫「`release/v1` 的 C72」，理由是
撞號；併線後那個限定詞指向一條即將不存在的分支，所以改回裸寫）。
**改對了指涉，而句子本身在同一天失去了指涉對象** —— 兩件事只差幾行。

## 三、⚠️ 這一則不是准許進入的執照

「沒有被治理」不等於「准許進入」—— 那是 C93 的話，這裡原封不動地適用。

C134 收回的是**這條判準的准入效力**，不是「`main` 的樹上什麼都可以放」。
`main` 的範疇邊界**還沒有定義**，票在 #90（清單與判準）與 #93（閘門設計）。
在那之前，判斷一個新目錄該不該在樹上，靠的是人，不是機制。

同一段話管住另一件事：`scope-check` 現在的 **10 項違規**（`tools/` 少 8 支：
`bff-check`／`compliance`／`csp-verify`／`exit-drill`／`gate-roster`／`pii-check`／
`supply-chain`／`ui-survey`；根層少 `CLAUDE.md`／`UI-SURVEY.md`）**不因這一則而合法**。
那 10 列仍然是 #90／#93 的產出，**在判準待裁時補它們是讓閘門轉綠，不是回答問題**
（AGENTS.md 規則二）。這一則做完之後也一樣 —— 收回門檻不會產生清單。

## 四、判準的示範案例反轉了，而那是一句假陳述

`SCOPE.md`〈刻意在外的〉整節講的是 `tools/gate-roster`：

> `v1.0.2` 曾短暫帶著它，`v1.0.3` 移除。**這是這條判準第一次真的擋下東西**

實測（2026-08-27，`865948f`）：

- `git ls-files tools/gate-roster` —— **在版控裡**
- `scripts.gate` 的**第一支** —— 它天天在跑
- `scope-check` 把它列為 10 項違規之一

也就是說，判準拿來證明自己有效的那個標本，現在在樹上而且在守門。
跟 C72〈人工跑一遍判準〉那一節抓到的 `tools/sast/`（README 目錄樹列著
一個從來不存在的目錄）是**同一種病、方向相反**：這次是文件宣告已經移除、而東西在樹上。

## 五、`SCOPE.md` 一個字都不改

沿用 C133 §五.3 的處置：工具與文件都留著、不動。理由沒有變 ——
`main` 的清單等 #90／#93，而在清單寫出來之前改 `SCOPE.md`，改出來的是
第五份手抄本，不是一份正確的文件。

⚠️ **代價寫在這裡，因為沒有東西會說話**：`SCOPE.md` 從今天起有兩處與現況
不符 —— 標題那句「什麼准許出現在 `release/v1` 的樹上」指向一條刪掉的分支，
以及 §四 那句假陳述。**沒有任何閘門看得出來**（`scope-check` 在 `UNGATED`
裡，而且它比對的是目錄清單、不是敘述）。發現它們的唯一途徑是人讀 `SCOPE.md`，
而人讀到的時候會先相信它。

## 六、這一則本身也沒有機制在守

`doc-facts` **刻意不守 `DECISIONS.md`**（`facts.ts:23-33`：守一份有日期的日誌
等於要求改寫歷史）。所以這一則的正確性沒有機器答案 —— 驗證方式是人讀 diff。

⚠️ 而**那正是這一則記的東西**：一條靠人讀才守得住的規則。**這不是第一條** ——
C109 §七（AGENTS.md 規則四「不得改 `specs/`」沒有機制在守）與 C129
（註解規範，標題就寫著「明知不可驗證所以不設閘門」）都是。⚠️ 這裡刻意不寫
「第幾條」：一個沒有事實來源的計數，寫在一則講「沒有機制在守」的條目裡，
正是 C130 §四／C53(c) 那個形狀。

## 七、對 #87 的作用：那一軸不是清掉了，是**換了一個還沒定義的**

⚠️ **這一則沒有讓 #87 可以開工。** 它只是把擋路的東西從一道死掉的門檻
換成一條還沒畫出來的線。

#87 必然會造出 `docs/adr/` 與 `CONTEXT.md` —— **兩個新的根層項目**。
按 §三：`main` 的範疇邊界還沒定義（#90／#93），而「沒有被治理」不等於
「准許進入」。所以准不准它們進樹，今天沒有機器答得出來，**要人判**。

⚠️ 這一段寫成這樣是刻意的。寫成「SCOPE 那一軸清掉了」的話，這一則就會
變成有人拿來加 `docs/` 的依據 —— 而 §三 存在的理由正是關掉那條路。

**同時實測（2026-08-27，`865948f`）：票面已經比它自己以為的更舊。**

| 票上寫的                                                                           | 樹上的事實                                                                                                                                                                                                                                                                                                                      |
| ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 決策與完整理由見 `docs/adr/0001-migrate-decisions-to-adr.md`                       | ⚠️ **`git ls-files docs/` ＝ 0 個檔** —— 那份 ADR 不在樹上。**票引用的理由來源已經不存在**                                                                                                                                                                                                                                      |
| 驗收條件二：刪掉 `docs/agents/domain.md`／`CONTEXT.md`／`AGENTS.md` 三處過渡期文字 | 三處**都不存在**（骨架被 #98／C93 撤掉）。⚠️ 這條**滿足於缺席** —— 與「真的做到了」長得一模一樣                                                                                                                                                                                                                                 |
| `### C<n>` 條目 89                                                                 | **133**（含這一則；⚠️ 最大編號是 C134 —— **C51 缺號**，編號不等於條目數）。⚠️ **而 89 對不上任何一條線**：票建立當天（2026-08-20）`main` 是 85、`release/v1` 是 74。**那個數字的口徑不明** —— 這比「數字過期」嚴重                                                                                                              |
| 交叉引用 745 次                                                                    | 745 不成立。這次粗掃（`grep -o 'C[0-9]\{1,3\}'`）得到 **約 2168**（內部 847 ＋ 其他版控檔 1321）—— ⚠️ **那是粗掃，不是議定的口徑**，它會把非引用的字串一起數進去。**目前沒有一個議定的計數口徑，所以這裡不下定論** —— 遷移成本要算之前得先定義「什麼算一次引用」（C130 §四：沒有事實來源的計數不要寫，包括寫在 DECISIONS 裡的） |

所以順序是：**先還原骨架、再遷移、最後刪** —— 而第一步就撞上 #90／#93。
在那之前 #87 動不了，動它就是在替一個還沒定義的邊界自己下定義。

### C135 — 正典佈局的清單：32 列、四個桶，而判準刻意不帶准入後果（2026-08-27，#90／#88／#91）

#### 一、這一則是 #90 的產出

`#90`（正典佈局的清單）問的是：把樹上與專案紀錄有關的東西逐一歸類，並定出
判準，「否則第七、第八份文件出現時沒有東西可以套」。

⚠️ **產出寫在這裡，不寫成 `docs/adr/` 的 ADR** —— 那會造一個新的根層項目，
而准不准它進樹正是 C134 §三 說「今天沒有機器答得出來、要人判」的那件事。
`#93` 的票面把自己的產出釘在 `docs/adr/`，於是「`#87` 擋在 `#90`／`#93`」
那條線是**繞回來的**：`#93` 的交付物要造的東西，正是它要解的問題。
這一則走 `DECISIONS.md`，把那個圈剪開，`#93` 可循同路。

⚠️ **代價**：這往一個因為體積而要搬家的檔案裡再搬一箱東西（`#87`）。

#### 二、判準

**正典由這個 repo 自己定義**，不採用 `/setup-matt-pocock-skills` 的佈局當權威；
外部佈局降為「沒有既有意見時的預設」。理由是 `#90` 自己寫的那個陷阱：
一個不認識你載重檔案的外部標準，會把 `README`／`HANDOFF`／`SCOPE` 判成殘留物
要求刪除。**標準不認識載重物，就不能拿它當尺。**

判準的形狀是混合的：

- **表是事實** —— 事實來源是 `git ls-files`，⚠️ **不是 `SCOPE.md`**（理由見 §四）。
- **判準只回答一個分類問題** —— 「它承載的是專案紀錄，還是別的軸？」
- ⚠️ **判準不帶准入後果。** 它只說落哪一桶，**不說准不准進樹。**

最後那一句是這一則最重要的一行。C134 §二 剛拆解過相反的做法：`SCOPE.md:34`
那句舉證責任（「寫得出受益者是拉 v1 的團隊」）在兩條線變一條線之後，
**問法活著、後果死了** —— 因為舉證責任式判準的效力**依附在它的後果上**，
而後果是環境給的。把准入後果綁進這裡，就是重造一次 `SCOPE.md:34`。

⚠️ **這個判準機器判不了，而這是實測出來的。** 起草時試過用「被幾個工具或
workflow 提到」當代理指標：`LICENSE` 被提 6 次、`.gitignore` 4 次、
`.git-blame-ignore-revs` 0 次。**檔名被提到不等於有一條治理規則在管它的內容。**
下面每一列的桶都是判斷，不是量測。

#### 三、四個桶

- **正典** —— 這個 repo 自己的紀錄體系。人讀它們來知道「這個專案是什麼、為什麼」。
- **正交** —— 各有各的軸與閘門。⚠️ **機制不准改，但要報**（理由見 §五）。
- **過渡豁免** —— 今天存在、遷移完成後該消失的。帶到期條件。
- **無關** —— 既不是紀錄、也沒有自己的治理軸。
  ⚠️ **這是唯一不要求寫理由的桶。** 替 `LICENSE` 寫那一句是儀式不是判斷
  （`SCOPE.md:120` 已經這樣裁過）。**寫明它不要求理由，是為了讓它是一個
  有名字的逃生門，而不是一個沒人注意的。**

⚠️ **實測：無關桶只有 4 列。** 它不是預期中的磁鐵，反而小到可以合理質疑
第四個桶值不值得。留著，因為那 4 列在三桶制底下確實無處可去。

#### 四、32 列

母體：`git ls-files` 的 24 個根層檔 ＋ 8 個頂層目錄。

**正典（6）**

| 名字           | 承載什麼體系                            | 備註                                                                             |
| -------------- | --------------------------------------- | -------------------------------------------------------------------------------- |
| `README.md`    | 入口敘述；數字由 `doc-facts` 守         | ⚠️ `#90` 票面判它正交，這一則反轉（見 §六）                                      |
| `HANDOFF.md`   | 交接 —— 拉 v1 做案子的團隊讀的那一份    | ⚠️ 同上，反轉                                                                    |
| `AGENTS.md`    | 下發給採用團隊的 agent 契約（四條規則） | ⚠️ **只有 END 標記之後那半是正典**；標記之間由 `vp` 產生，是另一條軸，機制不准碰 |
| `CLAUDE.md`    | Claude Code 入口，指向 `AGENTS.md`      | ⚠️ `SCOPE.md` 根層表**沒有這一列**（`scope-check` 十項違規之一）                 |
| `TESTING.md`   | 測試層級模型 —— 正確性靠哪幾層守        | 存在由 `scope-check` 守（該支在 `UNGATED`）                                      |
| `UI-SURVEY.md` | UI 選型的評估證據（D15／#49）           | ⚠️ 起草時判正交，查證後翻成正典（見 §六）。⚠️ `SCOPE.md` 根層表沒有這一列        |

**過渡豁免（1）**

| 名字           | 承載什麼體系               | 到期條件                         |
| -------------- | -------------------------- | -------------------------------- |
| `DECISIONS.md` | C／D／R 三串編號的決策日誌 | `git ls-files DECISIONS.md` 為空 |

⚠️ **到期條件不會自己響。** 它只在有人**跑**這個機制的時候才被問到 ——
**是溫度計，不是鬧鐘。** 寫明這一句，是因為 `#87` 的驗收條件二示範過相反的
下場：那三處過渡期文字今天都不存在（骨架被 C93 撤掉），而**「滿足於缺席」
跟「真的做到了」長得一模一樣**（C134 §七）。

**正交（21）**

| 名字                  | 它的軸                                            | 已知與現況不符（2026-08-27 快照）                                                                                                     |
| --------------------- | ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| `SCOPE.md`            | 交付樹准入；`scope-check`（在 `UNGATED`）         | ⚠️⚠️ **兩處**：標題指向已刪的 `release/v1`；〈刻意在外的〉整節為假陳述（C134 §四、§五）。**外加根層表缺 `CLAUDE.md`／`UI-SURVEY.md`** |
| `API.md`              | `platform/*` 形狀，由 `api-surface` **產生**      | 不要手改                                                                                                                              |
| `SPEC-REPORT.md`      | 業務完成率，由 `spec-report` **產生**             | 不要手改                                                                                                                              |
| `CHANGELOG.md`        | 版本沿革時間軸；SemVer 對 `platform/*` 的承諾     | —                                                                                                                                     |
| `CODEOWNERS`          | 擁有權（D12），一致性檢查驗它                     | —                                                                                                                                     |
| `package.json`        | 根 workspace ＋ `scripts.gate` 閘門鏈             | —                                                                                                                                     |
| `pnpm-workspace.yaml` | workspace 成員 ＋ catalog（D6）                   | —                                                                                                                                     |
| `pnpm-lock.yaml`      | 鎖定檔；SCA／catalog 的對象（D6）                 | —                                                                                                                                     |
| `.npmrc`              | 供應鏈（D1／D6）：內部 registry、`ignore-scripts` | —                                                                                                                                     |
| `tsconfig.json`       | 根 TypeScript 設定                                | —                                                                                                                                     |
| `eslint.config.js`    | Tier 2 安全閘門進入點（D10），刻意不經 `vp`       | —                                                                                                                                     |
| `vite.config.ts`      | Tier 1 快軌 ＋ 模板註冊 ＋ 複雜度門檻（C119）     | —                                                                                                                                     |
| `renovate.json`       | 升級提案（D16）—— 答「該升了」那一半              | —                                                                                                                                     |
| `stryker.config.mjs`  | 突變測試（元層）。⚠️ 不擋任何東西                 | —                                                                                                                                     |
| `tools/`              | 閘門本體。底下有自己的清單                        | —                                                                                                                                     |
| `platform/`           | 交付物本體。底下有自己的清單                      | —                                                                                                                                     |
| `specs/`              | 框架承諾；`promise-check` **真的執行**            | ⚠️ AGENTS.md 規則四守它，而那一條**沒有機制在守**（C109 §七）                                                                         |
| `features/`           | 示範切片；conformance 守契約                      | —                                                                                                                                     |
| `apps/`               | 示範應用外殼。⚠️ 目錄准許存在，內容不治理         | —                                                                                                                                     |
| `.github/`            | 兩支 CI workflow（Tier 1／Tier 2）                | —                                                                                                                                     |
| `.semgrep/`           | 承諾五的自寫 SAST 汙點傳遞規則                    | —                                                                                                                                     |

**無關（4）** —— ⚠️ 這一桶刻意沒有「它的軸」欄。
...
| `.claude/`               | ⚠️ 版控裡只有 `launch.json`，`worktrees/` 被 `.gitignore:33` 忽略。**生效條件**：`#88` 第一階段把 skill 放進 `.claude/skills/` 的那天，這一列要改成「底下有自己的清單」（`tools/`／`platform/` 的先例）—— 理由見 §六 |

合計 6 ＋ 1 ＋ 21 ＋ 4 ＝ **32**。

#### 五、平行體系的定義 —— 這一則答掉了 #91 的一半

母體選成「全部根層項目」之後，這張表跟 `SCOPE.md`〈根層 —— 准許存在的〉
**四條軸重合了三條**：同樣的母體、同樣的事實來源（`git ls-files`，兩份文件
都明文寫著）、而 `SCOPE.md` 那張表的「這是什麼」欄已經在記「誰守它」
（`API.md`「由 `api-surface` 產生」、`SCOPE.md`「由 `scope-check` 守著」）——
那就是這張表的「承載什麼體系」欄。

**按 `#88` 自己的定義，這是機制上線前自己先長出來的第一個平行體系候選。**
所以判準必須在這裡定死：

> **平行的特徵不是母體重疊，是「同一個問題有兩個答案，而沒有規則決定誰贏」。**

`SCOPE.md` 問「**准不准在樹上**」，這張表問「**它承載哪一條軸**」——
兩個問題，答案不會互相矛盾。⚠️ 而要讓這句話站得住，這張表的事實來源
必須是 `git ls-files` 而**不是 `SCOPE.md`**：一旦寫成「以 `SCOPE.md` 為準」，
它就繼承了那張表的洞（今天缺 `CLAUDE.md`／`UI-SURVEY.md` 兩列），
而 §三 說了它不准改 —— 那就變成一張你修不了、卻要為它負責的表。

⚠️ **對 `#91` 的作用：這個定義換掉了它的題目。** `#91` 原本問「新長出的平行
體系偵測得到嗎」，預設偵測對象是「多了一份根層 `.md`」。現在偵測對象是
「兩張表對同一個問題給出不同答案」—— 而那**一半是可枚舉的**。
⚠️ **`#91` 不關。** 剩下那一半（「同一個問題」是語意判斷）原封不動，
而它自己寫的警告一個字沒失效：「**一個分不出好壞的偵測器比沒有偵測器更糟**
—— 它會給出綠燈，而綠燈會被當成保證。」

#### 六、起草時翻掉的三件事

**一、`README.md`／`HANDOFF.md` 反轉。** `#90` 票面把它們舉為**正交**的例子，
理由是「被 `doc-facts` 守」。那個分類是**外部標準假設**的產物 —— 怕天真的
機制把它們當殘留物砍掉。正典改成自訂之後這個危險消失了，而「有閘門守」
從來不是正交的判準（正交是**不同的軸**，不是「有人守」）。
⚠️ 而正典比正交**更**保護它們：正交說「不干我事」，正典說「它本來就該在這裡」。

**二、`UI-SURVEY.md` 從正交翻成正典。** 起草時判正交，理由是「它是產生物、
是證據不是紀錄」。兩半都錯：

- `tools/ui-survey/src/cli.ts:167` 那個 `writeFileSync` 寫的是 `mkdtempSync`
  開出來的暫存 `package.json`（拿去跑 `npm install --package-lock-only`），
  **與 `UI-SURVEY.md` 無關**。這份文件是人寫的。
- 它進 `doc-facts` 的 `GUARDED` 的理由，跟 `README`／`HANDOFF` 是**同一條**：
  `cli.ts:51-52` 寫著「README 與 HANDOFF 第 14 項都直接把讀者指過去」，
  而 `facts.ts` 的入場判準是「被 README 或 HANDOFF 指過去、而且用現在式寫的」。
- 外部證據同向：主流 ADR 變體（Nygard／MADR／Y-Statement）**沒有一種**把
  評估證據放在決策記錄之外；MADR 的〈Pros and Cons of the Options〉是 ADR
  **內部**的欄位（`adr.github.io/adr-templates/`）。

⚠️ **代價**：正典桶從此包含一份**會合理過期**的文件（檔頭「調查日 2026-08-15」）。
所以**「正典」不等於「現在式」** —— 而那條區分正是 `doc-facts` 的入場判準在用的。
兩者從今天起不對齊，這裡先記著。

**三、`.claude/` 維持無關，但備註換成有來源的。** Claude Code 官方的
`.claude` 目錄文件（`code.claude.com/docs/en/claude-directory`）把它定義為
**多用途容器**，其記載的子路徑版控處置**不一致** —— 該頁的檔案樹裡
`committed` 12 個、`gitignored` 1 個、`local` 7 個。
⚠️ 兩件實測：**`CLAUDE.md` 在專案根層，不在 `.claude/` 底下**（官方樹與本樹
一致）；而 **`launch.json` 在那一頁出現 0 次** —— 本樹 `.claude/` 裡唯一的檔案，
官方那張目錄圖沒有收錄。

所以「整個目錄一個桶」在它長出 `skills/` 的那天就不成立，屆時照
`tools/`／`platform/` 的先例改成「底下有自己的清單」。
⚠️ 而**不採納「現在就逐檔分類」** —— `settings.json`／`skills/`／
`settings.local.json` 都不在這棵樹上，替假想檔案分類是繞過 §二 那條
「事實來源是 `git ls-files`」。

⚠️ **順帶記一件沒有採納的事**：外部框架**答不了第一列**。Diátaxis
（`diataxis.fr`）分的是使用者需求象限、不處理檔案分類；arc42（`arc42.org`）
只涵蓋架構紀錄；OpenSSF Scorecard 講的是開源安全姿態（SECURITY.md 要有
揭露流程、LICENSE 要在頂層），那是合規檢查不是文件分類法。
**它們沒有回答，只是沒有反對。** 記下來，是因為一個灌水的引用比沒有引用更糟
—— 下一個人會以為那裡查過了。

#### 七、⚠️ 這張表沒有機制在守，而這是刻意的

技術上做得出一道只查完整性的閘門 —— `scope-check` 就是那個形狀，而且這張表
有 `SCOPE.md` 沒有的優勢：**它第一天就是完整的**（母體是全樹，照今天的樹寫的）。

不做，理由是：一張要被閘門守的清單必須是**活的**，而這一則是**有日期的
過去式陳述**。`doc-facts` 不守 `DECISIONS.md` 的理由正好反過來咬這件事
（`facts.ts:23-36`）：

> 守它等於要求每次相依變動都回頭改寫歷史記錄……**一份被持續改寫的決策日誌，
> 就不再是決策日誌了。**

而把表搬出去要造一個新的根層檔 —— **又撞上那條沒畫的線**，這是這一輪的
第三次（前兩次見 §一 與 §六 的 `docs/adr/`）。

⚠️ **代價，明知故犯：這張表會用 `SCOPE.md` 那張表一模一樣的方式腐爛。**
有人加一個根層檔，不會有任何東西紅。差別只有兩個字：**明知**。

⚠️ **這個處置可以被推翻**：等 `#93` 那道閘門存在、且清單有一個活著的家之後，
要重新裁。寫明「暫時」，沿用 `UNGATED` 裡 `scope-check` 那條的做法
（「⚠️『暫時』要說得出口，所以它在這裡而不是被悄悄刪掉」）——
不寫，「暫時」就會變成永久。

⚠️ **這一則本身也一樣沒有機器答案**，驗證方式是人讀 diff（同 C134 §六）。

#### 八、母票的射程已經死了兩句，原文存檔於此

`#88` 的 Notes 有兩句話會被下一個讀它的人（或 agent）**照做**，而它們已經失效。
⚠️ GitHub 票面不受版控、改了沒有 diff 可讀，所以原文逐字抄在這裡再去改：

> 「**審查對象只有 `release/v1`。** `main` 的文件樹刻意落後（停在 C74、沒有
> C75–C89、卻有 C90），維護兩份預期佈局就是 `SCOPE.md` 開頭警告的手抄本。
> `main` 解凍併線後自然收斂。」

實況：`release/v1` 已併回 `main` 並刪除（C133、#176）；`main` 的舊 C70–C74
已改號成 C128–C132，編號分岔**已經解決**，不是搬家。

⚠️ **那兩句不是過期的數字，是還在生效的指令** ——「排除 `main`」照做的結果
是繞開整棵樹。這是改票面的理由；C134 §七 剛示範過同一件事：票面比它自己
以為的更舊，而沒有任何機制會說話。

**對 `#87` 的處置不同，刻意的。** `#87` 票面也有四處過期（C134 §七 已量出並
貼成留言），但那些是**陳述**（會被相信，不會被執行），與 `#88` 的**指令**不同類，
所以不套同一個處置。⚠️ **唯一的例外**是它開頭那句「決策本身與完整理由見
`docs/adr/0001-migrate-decisions-to-adr.md`」—— 那是**指路**，而路沒了
（`f240484` 加入、`7b0b4a1`／#98／C93 撤回），與「排除 `main`」同類，照改。

⚠️ **票面數字一個都不動。** C134 已裁定 `89` 對不上任何一條線、`745` 不成立，
而**目前沒有議定的計數口徑**。在沒有口徑的情況下換一個新數字上去，
就是再造一個沒有事實來源的計數（C130 §四／C53(c)）。

### C136 — 範疇判準的後果隨環境消失，所以那張表退回登記表（2026-08-27，#180）

#### 一、這一則是 #180 的產出，而那張票的前提在查證後翻了一半

`#180` 問的是：`tools/scope-check` 在 `UNGATED` 裡等一張沒有人在寫的清單，
而「暫時」到期了。它把問題設成「上游這棵樹還需不需要一張准入清單」，並把
那 10 項違規設成「是違規、還是清單錯了」的二選一。

**兩個都不是。** 查下去掉出來的是第三種，而它決定了下面每一條裁決。

⚠️ **產出寫在這裡，不寫進 `SCOPE.md`。** C133 §五.3 與 C134 §五 兩次都選了
「留著不動」，理由是「清單寫出來之前改它，改出來的是第五份手抄本」——
那個條件今天才到期（C135 §四 寫出了根層 32 列）。順序仍然是先裁決、後改文件，
沿用 C72 立的那條（「反過來做，補文件的動機會變成讓閘門轉綠」）。

#### 二、那 10 項的出身有四種，而砍掉其中 8 項的判準隔天就被推翻了

| 項目                                                                                                                      | 誰砍的                                                                  | 用的是哪條判準                |
| ------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- | ----------------------------- |
| `bff-check`／`csp-verify`／`compliance`／`exit-drill`／`pii-check`／`supply-chain`／`ui-survey` ＋ `UI-SURVEY.md`（8 項） | `49b36da`（2026-08-17）「release(v1.0.0): 縮到五條承諾，其餘留在 main」 | **「不服務五條承諾就出局」**  |
| `gate-roster`（1 項）                                                                                                     | `cfb9668`／`v1.0.3`（2026-08-18）                                       | **C72**「看它守的東西給誰看」 |
| `CLAUDE.md`（1 項）                                                                                                       | 沒有人 —— 它 2026-08-18 生於 `main`（#48），`release/v1` 上從未存在     | 無                            |

⚠️ **`49b36da` 是 8 月 17 日，C72 是 8 月 18 日** —— 而 C72 明文推翻的第一條，
正是 `49b36da` 用的那一條：

> | 「不服務五條承諾就出局」 | 連 `tools/doc-facts` 一起砍 —— 而它是 Tier 2 的閘門之一 |

**C72 推翻它之後，只重判了 `gate-roster` 一支**（舊判準放行、新判準擋下的那一支），
**沒有回頭問前一天被砍的那一批該怎麼辦**。併線（#176）把那一批原封帶回來 ——
連同 `platform/eslint-config` 的 `./a11y` 匯出與 Tier 2 的 SCA／SBOM ——
而**沒有任何東西說話**。

所以那 10 項的性質是：**一次從未執行過的判斷，不是一次修復。**
⚠️ 而這**不等於**「它們通過了判準」。沒有被治理不等於准許進入（C93）。
它們今天在樹上的理由只有一個：沒有人裁過要移走它們。

#### 三、判準真的重跑一次，產出的是分類 —— 因為後果沒有地方可掛

那就跑。但跑出來的結果掛得上什麼？

- **「判在外的就從樹上刪掉」** —— 那 8 支裡有 5 支（`gate-roster`／`exit-drill`／
  `supply-chain`／`compliance`／`pii-check`）**今天在 `scripts.gate` 上**，
  `gate-roster` 還是第一支。這個後果一裁下去，沒有人會執行它。
- **找過別的後果，這棵樹上一個都沒有**：「不下發」不成立（fork 拿的是整棵樹，
  沒有打包邊界）；「不進閘門鏈」已經是 `gate-roster` 的 `GATES`／`UNGATED`
  在答的問題；「能力上不承諾」是 HANDOFF〈不涵蓋〉那條**另一條軸**。

**裁決：判準重跑只產出分類，不帶准入後果。**

這正是 C134 §二 預告過的形狀 ——「**舉證責任式判準的效力依附在它的後果上，
而後果是環境給的**」。C134 對 `SCOPE.md:34` 裁過一次，這裡是同一件事在
`tools/` 那張表的第三欄（「為什麼受益者是拉 v1 的團隊」）再發生一次。

⚠️ **必須同時寫下的推論：不准拿這個分類論證誰可以留在樹上、誰該被刪。**
與 C135 §二 同一條線 —— 一個不帶後果的判準被拿去當准入依據，就是重造一次
`SCOPE.md:34`。

⚠️ **順帶解掉一件票面禁止的事**：`#180` 寫「不得補那 10 列來讓閘門轉綠」，
理由是「替一條沒畫的線自己下定義」。**根層那張表沒有線要畫** ——
`SCOPE.md` 自己寫著它「刻意只有兩欄，沒有『受益者是拉 v1 的團隊』那一欄」，
它問的只有「這東西在不在版控裡、有沒有人寫過一句它是什麼」。所以補根層那兩列
在構造上不可能是綠燈交易（補完之後 `tools/` 那 8 項照樣紅）。

#### 四、三層各自處置，因為三層的處境沒有一個是共通的

「一支 CLI」是實作的單位，不是判斷的單位。

| 層          | 母體（2026-08-27 快照） | `SCOPE.md`                           | 有沒有別的機制                                                                                         | 處置                                                |
| ----------- | ----------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------ | --------------------------------------------------- |
| `tools/`    | 19                      | 11 列（＝已刪分支 `ea3ad1d` 的快照） | **有，而且完整**：`gate-roster` 的 `GATES ∪ UNGATED`，`why` 必填，機制在守（`check.ts:198`），今天全綠 | **交棒**：`GOVERNED` 拿掉 `tools`，那一節降級為散文 |
| `platform/` | 10                      | 10 列                                | **沒有**                                                                                               | **保留**，一個字不動                                |
| 根層        | 24 檔 ＋ 8 目錄         | 缺 `CLAUDE.md`、`UI-SURVEY.md`       | C135 §四 的 32 列，但 §七 **刻意**不帶閘門                                                             | **補那兩列**                                        |

⚠️ **`platform/` 那一半是這場處置的下限，寫在正文不寫在備註。** 它是三層裡
唯一「今天正確、而且沒有第二個機制在做同一件事」的一半，也是唯一**會安靜消失**
的一半 —— 綠的東西被順手刪掉，不會有任何紅燈說話。

##### 為什麼 `tools/` 那一節不是「改欄補齊 19 列」

拿掉判準欄之後，那張表剩下的欄位（路徑／守什麼）**正是 `gate-roster` 的 `why`
已經在寫的東西**，而母體完全重合。`SCOPE.md` 自己命名過這個病：

> **同一件事抄成兩份、而沒有東西斷言它們一致** —— 那正是 `v1.0.3` 才剛付過
> 代價的病……加一份清單而不加守它的機制，是在製造第五份手抄本。

⚠️ 這裡更糟一點：**兩份都有機制，而兩道機制互不斷言** —— 漂移的時候兩邊都是綠的。

⚠️ **而它們今天不是平行體系**（C135 §五 的判準：同一個問題有兩個答案而沒有規則
決定誰贏）。`codemods`／`slice-gen`／`gate-kit`／`scope-check` **四支同時**在
`SCOPE.md` 那張表裡、又在 `UNGATED` 裡 —— 兩張表同時說「該在樹上」與
「刻意不是閘門」，不矛盾。是補齊 19 列**才會**把它變成平行體系。

##### 窄化不是改一個常數

`parse.ts` 的 `declaredSections` 撈的是 `SCOPE.md` 裡**每一個含「准許存在的」
的標題**，`GOVERNED` 沒對應到的就報「這一節沒有人在檢查」。所以把 `tools`
從 `GOVERNED` 拿掉、而那一節留著 → **閘門照樣紅，只是換一種紅法**。
窄化必然連帶處置那一節本身。

##### 交棒交過去的不只是一張清單

`gate-roster` 的 `why` 今天回答的是「它是／不是閘門，為什麼」。交棒之後它要
按**兩個讀者**分開寫，而這件事要寫在 `gates.ts` 的檔頭：

- **對上游** —— 「該不該在樹上」已經沒有後果（§三），所以那一欄**不假裝**在
  回答它。不寫明的話，下一個人會以為交棒把准入也接過去了。
- **對 fork 團隊** —— 那一欄**就是**那個判斷。他們加 `tools/their-thing` 撞到
  「工具沒登記」、被逼著寫一句理由，那正是 `SCOPE.md:8-11` 承諾的
  「這道閘門要的是**有人判斷過**」。

⚠️ **所以 `SCOPE.md` 的雙讀者性質也一起交過去了。** `#180` 提醒過「刪掉
`scope-check` 不是乾淨的選項，它會從採用團隊手上拿走一個還在運作的東西」——
交棒不觸犯那一條的原因正是這個：fork 團隊加 `tools/` 目錄仍然會被擋，
只是擋他們的換成了 `gate-roster`。⚠️ **失效的只有 `SCOPE.md:8-11` 說「你會被
送到哪裡」那一句**，承諾的內容一個字沒變。

#### 五、`SCOPE.md` 改完之後是什麼

三張表變兩張、判準欄一個都不剩、「在外」那個桶不存在了。剩下的是：
`platform/` 與根層兩張**登記表**、`tools/` 一段指向 `gate-roster` 的散文、
事實來源那一節、以及守它的那一節。

**標題與開頭定位要改寫，檔名不動。**

⚠️ **標題假的不只是分支名，是「准許」兩個字。** 只把 `release/v1` 換掉，
會產出一份**看起來仍在守門、實際上只在登記**的文件 —— 那正是 §六 要從
〈刻意在外的〉手上拿掉的病。

⚠️ **不改檔名，而這是成本判斷不是偏好**：改檔名的成本落在別人身上
（README／HANDOFF／CODEOWNERS 的指路、C135 §四 那一列的名字），
收益只是名字更貼切。而 `SCOPE`（範疇）這個詞**沒有變假** —— 這份文件仍然回答
「這棵樹上有哪些東西」，那就是範疇的描述義。

##### 〈判準〉那一節改寫，不撤除 —— 與〈刻意在外的〉刻意不同處置

| 節             | 性質                                                             | 處置                               |
| -------------- | ---------------------------------------------------------------- | ---------------------------------- |
| 〈刻意在外的〉 | **對現況的假陳述** —— 它說「在外」的東西是 `scripts.gate` 第一支 | **撤除**，原文存 §六               |
| 〈判準〉       | **真陳述，但後果死了** —— 問法仍問得出角色（C134）               | **改寫**：留問法，明寫它不決定准入 |

⚠️ 兩節都撤的話，這份文件就一點都不記得自己曾經判過東西 —— **而它判過，
而且判對過一次**（C72 擋下 `gate-roster`）。只剩兩張登記表的文件，
讀的人不會知道那兩張表為什麼長這樣。

##### 紅燈訊息的兩個讀者合併成一條

C95 §二 把訊息拆成 `forFork` 與 `forUpstream`，並**刻意不去偵測這棵樹是不是
上游**（「一個猜錯的偵測會給出看起來很確定的錯訊息」，那是 #91）。
分岔的**唯一內容**是上游那條要求六個特定的字、而 fork 團隊依定義寫不出來。
§三 之後那個要求沒了，兩個讀者要做的事一模一樣：加一列，寫一句它是什麼。

**合併。** ⚠️ 留著會變成一個沒有差別的分類，還會讓讀者以為得先判斷「我是哪一種」
才知道要做什麼。

⚠️ **合併之後必須補一句**：這道閘門綠了**不代表**那東西可以在這裡，它只保證
有人寫過一句。這是 C93 在使用現場的那句話，而它現在特別需要說出口 ——
文件裡已經沒有判準節在暗示還有第二關。

⚠️ **對 #91 的作用**：C95 那句「刻意不偵測上游」的**前提消失了** —— 這道閘門
從此不需要那個答案（不是「不必知道就能對兩種人說話」，是**根本沒有那個分岔**）。
⚠️ **#91 不因此可關**：它問的是平行體系偵測，C135 §五 已經換過一次它的題目，
這裡只是又拿掉一個依賴它的地方。

##### `UPSTREAM_CRITERION` 是閘門正在說的一句假話

`check.ts` 的那句「寫得出『受益者是拉 v1 的團隊』才可以進 `release/v1`，
寫不出來就送 `main`」，C134 已裁定它的後果死了（「送 `main`」＝「留在原地」）。
⚠️ **它今天對 `platform/` 與根層兩層的紅燈都還在說** —— 而 `platform/` 那一半
正是這場處置決定要保住的。這一項不做完，§四 保住的東西會帶著一句錯的指示上線。

#### 六、〈刻意在外的〉整節撤除，原文逐字存檔於此
...
第一段講的是 `tools/gate-roster`，而它今天在版控裡、是 `scripts.gate` 的
**第一支**、也是那 10 項違規之一。⚠️ **不能只加標註留在原地**：一個標題叫
〈刻意在外的〉、而內容講的東西是閘門鏈第一支，讀者會先相信標題。

第二段是活的、真的指路，但它指的東西〈三份清單，三條軸〉那張表已經指過去了 ——
整節撤除不掉任何東西。

⚠️ **撤掉之後 `SCOPE.md` 沒有任何地方記載「這棵樹上有東西曾經被判出去過」。**
那段歷史的唯一去處是這一則與 CHANGELOG —— 而那正是 §八 那條書寫規則要接住的。

#### 七、第三處假陳述：一句自己預言過的話，在另一條分支上被兌現

查 §五 的連帶範圍時掉出來的，C135 §四 只記到兩處。`SCOPE.md` 與 `TESTING.md`
兩處都寫著：

> `doc-facts` 的 `GUARDED` **只有 `README` 與 `HANDOFF` 兩份**，加第三份的那一刻，
> 四個地方寫著「只守那兩份」的句子會同時變成假的 —— 所以這份文件不進 `GUARDED`。

實測（2026-08-27，`e43f660`）：`tools/doc-facts/src/cli.ts` 的 `GUARDED` 是
**三份**（多 `UI-SURVEY.md`），**2026-08-16 起**。

**形成方式是這棵樹的老病**：`ea3ad1d`（`release/v1` 最後一版）的 `GUARDED`
**確實只有兩份** —— 因為 `UI-SURVEY.md` 被 `49b36da` 一起移出去了（§二 那一批）。
**那句話在 v1 上是真的、在 `main` 上是假的**，併線把兩半放在一起，
兩句話當場變假，而**沒有任何東西說話**。

⚠️ **而程式碼那一半做對了。** `cli.ts` 逐字寫著「檔名從 `GUARDED` 印出來，
不手寫 —— 加第三份的那天……」，紅燈訊息裡的檔名清單是算出來的。
**寫下預言的人防住了程式碼，沒防住散文。**

**兩處一起改**（`SCOPE.md` 與 `TESTING.md`）—— 那是同一句話的兩份手抄本，
只修一份，留下的那份就是唯一倖存的假陳述，而它會繼續被讀成真的。
⚠️ **射程到此為止**：只改這一句的兩個實例。`CHANGELOG.md` 也有一句，
但那是有日期的歷史紀錄，不是活的假陳述，不動。

#### 八、推翻一則 C-條時，要寫明它產出過什麼、重跑了沒有

這一整則的內容，本質是一次遲到十天的複查：一條判準被推翻，而**它產出的東西
沒有人回頭重跑**。看見它的不是閘門 —— 是 C134／C135 逼著人去讀 `SCOPE.md`。

**裁決：沒有機制，而是一條書寫規則。** 一則 C-條推翻另一則的時候，
必須寫明**被推翻的那則產出過什麼，以及那些產出重跑了沒有**。

⚠️ **這條規則沒有任何東西在守，而那是查證過的結論不是省事**：
「某則裁決的產出沒被重跑」不是靜態可驗證的 —— `DECISIONS.md` 裡沒有任何一條
從 C-編號連到它產生的 commit。到期條件也不成立：條件要說得出口，
而這裡的條件是「某一天有人推翻它」，事前不可知。

⚠️ 所以它與 **AGENTS.md 規則四**是同一種形狀：一條沒有機制在守、只靠人讀的規則。
處理方式沿用 C109 §七 與 C135 §七 —— **寫明它是敞著的**，不要讓下一個人
以為有東西在守。

⚠️ **C72 自己已經做對過一半**：它文末有一句「⚠️ 那句『沒有任何機制在守這份
文件』已於 `v1.0.5` 不再成立，見 C73」。它漏的正是另一半 ——
**它沒回頭問前一天那批被砍的東西該怎麼辦。**

**C72 原文不改，加一行 ⚠️ 指向這一則。** 沿用 C-條慣例（留原文、加標註、
用後續版本修正），而 C72 自己就是這個形狀的先例。
⚠️ **不收回 C72**：它擋下 `gate-roster` 是「這條判準第一次真的擋下東西」，
那件事發生過而且是對的 —— 收回它等於把一次正確的判斷從紀錄裡拿掉。

#### 九、三支 PR，而中間那個狀態是刻意的

|        | 內容                                                                                                                | 走完之後 `scope-check`      |
| ------ | ------------------------------------------------------------------------------------------------------------------- | --------------------------- |
| **一** | 本則 ＋ 根層補兩列 ＋ 標題與開頭改寫 ＋〈判準〉改寫 ＋〈刻意在外的〉撤除 ＋ 兩處 `GUARDED` ＋ C72 標註              | 10 → **8**（純文件）        |
| **二** | 窄化 `GOVERNED`、`tools/` 那一節降級、判斷紀錄搬進 `why`、`UPSTREAM_CRITERION` 改寫、訊息合併、`SCOPE.md:8-11` 改寫 | 8 → **0**，但仍在 `UNGATED` |
| **三** | 出 `UNGATED`、進 `GATES`（`tier1`，排在 `doc-facts` 之後）＋ `scripts.gate`／README／workflow                       | **在閘門鏈上**              |

⚠️ **PR 二結束時那個中間狀態是刻意的**：「它綠了」要單獨被看見一次，
再談「所以它可以上鏈」。合成一支的話，綠燈與上鏈同時發生，而**沒有任何一刻
可以檢查「它是因為問題解決了才綠，不是因為被調鬆了」**（AGENTS.md 規則二）。

⚠️ **PR 二會被誤讀成調鬆門檻，而它合法的唯一理由要寫在 commit 正文裡**：
那一半的保證由 `gate-roster` **完整承接**（§四），不是因為它太吵。

⚠️ **`pnpm ready` 的綠證明不了前兩支**：`scope-check` 在 `UNGATED`，不在 `ready`
裡。三支都要另外手跑 `node tools/scope-check/src/cli.ts` 並記下 10 → 8 → 0。
⚠️ 而 PR 三的連帶改動不必靠人記得：`gate-roster` 斷言 `scripts.gate` 與 `GATES`
推導出來的逐字相同，並比對 README 那張表與 workflow，**漏一處就紅**。

#### 十、對 `#87` 的作用 —— 解鎖的是機制那一半

`#87` 要造 `CONTEXT.md` 與 `docs/adr/` 兩個新的根層項目，而 C134 §七 說
「准不准它們進樹今天沒有機器答得出來」。這場處置之後，根層是一張**純登記表**
（兩欄、無判準），`scope-check` 全綠且在閘門鏈上 —— 加兩列寫一句它們是什麼，
**閘門就答得出來了**。

⚠️ **判斷那一半沒有解，而且是刻意的。** §三 已裁分類不帶准入後果，C93 說
沒有被治理 ≠ 准許進入。**「閘門會答」不等於「答案是可以」** —— 要不要造那兩個
東西仍然是人的決定，而 C135 §一 已經對其中一個示範了**不造也走得通**
（產出寫 `DECISIONS.md`，把 `#93` 的迴圈剪開）。

⚠️ **`#87` 動起來的第一件事不是建目錄**：它的驗收條件二今天「滿足於缺席」
（骨架已被 `7b0b4a1`／#98／C93 撤掉，三處過渡期文字都不存在），
而票面數字 `745`／`89` 沒有一個成立過（C134、C135 §八）。**它要先重問自己的前提。**

⚠️ **這一則的收尾不是「`#87` 解鎖了」** —— 那句話會被讀成 `#180` 批准了
`docs/adr/`，而那正是 C135 §二 明文拒絕回答的問題。

### C137 — 「過度設計」在這棵樹上沒有分母，所以那次研究產出的是觀察不是判準（2026-08-29）

#### 一、問法要先被裁一次，而不是先被回答

起於一次研究請求：「`main` 的腳手架足夠還是過度設計」。查下去第一件掉出來的
不是答案，是**這個問法在這棵樹上量不了**。

最好引用的那個數字是「工具層 vs 產品碼」的比值。而 `AGENTS.md` 明講這棵樹是
**交給採用團隊的腳手架**，`apps/console` 與 `features/*` 是示範切片 ——
**這個 repo 的產品就是 `tools/`**。真正的分母（採用團隊自己的業務碼）
不在這棵樹上，也不會在。

**裁決：那次研究產出分類與觀察，不帶處置後果。**

這是 C134 §二那句話的第三次應驗 ——「**判準的效力依附在它的後果上，而後果是
環境給的**」。C134 對 `SCOPE.md:34` 裁過一次、C136 §三 對 `tools/` 那張表裁過
一次，這裡是同一件事發生在一個**從外面帶進來的**判準上。

⚠️ **必須同時寫下的推論：不准拿 C137 論證刪掉哪一支閘門、或論證再加一支。**
與 C135 §二、C136 §三 同一條線 —— 一個不帶後果的判準被拿去當准入依據，
就是重造一次 `SCOPE.md:34`。
...
| 觀察                                       | 有沒有處置                                                            |
| ------------------------------------------ | --------------------------------------------------------------------- |
| 閘門回綠燈的方式，五層與兩類都量不到       | **有** —— `TESTING.md` §五 補一節（`#185` —— 本則寫下時已開、未合併） |
| 驗收面薄於閘門面（承諾一有規格、還有四條） | **無需** —— `TESTING.md` §六 第二列**早就寫著**，研究只是重推了一次   |
| `CODEOWNERS` 存在而條目全 `Unknown owner`  | **無** —— `CODEOWNERS` 檔頭自己寫著，且 C63 已裁刻意不做成閘門        |
| release 流程未自動化                       | **無** —— 沒查到它是刻意還是遺漏，所以它是**未決問題**不是發現        |
| Nx／SLSA／CycloneDX 的外部校準             | **無** —— 外部脈絡，這棵樹上沒有東西掛得上去                          |

⚠️ **第二列是這一則最該記住的一格。** 研究的「頭條發現」是這份文件**自己已經
寫過的話**，而 `TESTING.md` §七 三次寫「這裡不重抄」。**一份研究最容易產出的
東西，是把既有文件重新推導一次然後宣稱它是新的。**

#### 三、治理層的自我修復率：這個數字兩邊讀都成立，所以它不分辨

⚠️ **凍結量測（2026-08-28，於 `84c5089`）**：近 50 筆提交有 10 筆在修治理層
自己的缺陷 —— C122、C124、C125／C126、`9c13193`、`b05d22d`、`d1357be`、
`0904d79`、`e43f660`。**這是那一天那個 commit 的量測，不是現在的比率。**

兩種讀法，而現有證據**兩邊都支持**：

- **「閘門太貴」** —— 五分之一的力氣花在維護量尺本身，沒有落到產品上。
- **「回饋在運作」** —— 那十件**每一件都被找到而且修掉了**，其中兩件是這一層
  替自己造的機制找出來的（C124 的差分探針、C126 的旗標拒絕）。**自我修復率為零
  才是該警覺的那一種** —— 那代表沒有東西在看。

**裁決：這個數字不作為刪除或新增任何閘門的依據。** 要讓它能分辨，缺的是
**別的樹的同一個量測**，而那不在這棵樹上（回到 §一 的同一個分母問題）。

#### 四、研究自己錯了三次，而三次是同一個形狀

三條結論在覆核時翻掉，值得記的是**它們錯得一模一樣**：

| 錯誤結論                             | 判準套在什麼上                  | 實際                                                                      |
| ------------------------------------ | ------------------------------- | ------------------------------------------------------------------------- |
| 「缺 `CODEOWNERS`／`renovate`」      | 只看了 `.github/` 底下          | 兩者都在**根層**；GitHub 接受根層、`.github/`、`docs/` 三處               |
| 「`gate-kit` 是失效的閘門」          | 它的**標籤**（在 `UNGATED` 裡） | 它是 **library**，沒有 `cli.ts` —— C131 裁過，`UNGATED` 的 `why` 寫著     |
| 「Nx 用 1 條規則做完這裡 13 支的事」 | 拿邊界規則對比**全部**閘門      | 可比的只有 `conformance` ＋ `scope-check`；供應鏈／個資／法遵不在 Nx 射程 |

**同一個形狀：判準套在東西的「描述」上，而不是它的「實質」上。** 這在這棵樹上
已經記過數次（C105 一輪三次、C106 第四次），這裡是又一次 —— 而這次犯它的是
**一個為了審視這棵樹而設的外部視角**。

**處置：兩條書寫規則，不設閘門**（與 C129 同一條理由 —— 明知不可驗證）。

1. **判「有沒有」之前先確認容器。** 「`.github/` 底下沒有」不等於「這棵樹沒有」。
2. **量測要標基準 commit，寫出來的時候重讀一次。** 見 §五。

#### 五、量測基準在研究進行期間往前移動了

研究以 `84c5089` 為基準，寫到一半 `main` 前進到 `b745d6e`（`#184`）——
而那支 commit 正是研究列為 P0 的兩件之一（`scope-check` 上閘門鏈）。
**報告的一條建議在它還在被寫的時候就落地了**，同時「13 上鏈／6 未上鏈」
變成「14 上鏈／5 未上鏈」。

⚠️ **這不是意外，是這份日誌的數字為什麼一律要凍結的理由。**
`doc-facts` **刻意不守 `DECISIONS.md`**（守它等於要求改寫歷史），
所以這裡的每一個數字都**沒有機制在守**，只有「標明那一天那個 commit」
這個寫法在守 —— 與 `CODEOWNERS` 檔頭那個「22 條」用的是同一招。

⚠️ **本則寫下時 `#185` 尚未合併，所以 §二 第一列指的是那張票、不是那一節。**
直接斷言「`TESTING.md` §五 有這一節」的話，票被關掉的那一刻它就變成假的 ——
`TESTING.md` §七 記過那種句子怎麼死的（同一句話在兩條線上一真一假）。

### C138 — `DECISIONS.md` 凍結於本則，C139 起寫進 `DECISIONS-2.md`（2026-08-29，#195）

⚠️ **量測基準 `1c39132`。** 本則所有數字都凍結在那一天那個 commit（C137 §五）。

#### 一、路線 A 照 `#87` 現在的寫法否決，而理由不是成本

`#87` 的條件一是把 C／D／R 三串編號的交叉引用全部重接成 ADR 的一決策一檔格式。

- **C 系列 1,851 處**（三串合計約 2,700 處，扣掉定義處的口徑）——
  ⚠️ **凍結於 `dcc65f9` 的量測**，不是今天的數。
- 今天散佈在 **230 個檔**（`grep -rlE` 粗量，**上界**）—— 其中包含
  `tools/scope-check/src/tree.ts`、`tools/doc-facts/src/cli.ts`、
  `tools/api-surface/tests/negative.test.ts` 這些**程式與測試的註解**。

⚠️ **而沒有任何閘門守得到那些引用**，理由寫在 `tools/doc-facts/src/cli.ts:25`：

> DECISIONS.md 是有日期的決策日誌，守它等於要求回頭改寫歷史。

所以路線 A 的第一步是**一次跨兩百多個檔、約 2,700 處、零機械驗證的大量編輯**。
在這棵樹上這不是成本問題：光是做本則的量測就踩到三種「假的綠」——
`git grep` 的 pattern 對不上而回報 0 個檔（**對照組同樣回報 0，是它接住的**）、
`git show` 的輸出被重排讓 `^@@` 全部失效、多命令區塊吞掉後段輸出。
**無人守的 2,700 處編輯會安靜地出錯，而出錯的樣子就是全綠。**

#### 二、但「什麼都不做」也不是答案 —— 這一格是量出來的

| 日期（commit）          | `DECISIONS.md`                         |
| ----------------------- | -------------------------------------- |
| 2026-08-15（`ba985ee`） | 74,105 bytes（72 KB）                  |
| 2026-08-21（`7cd0e5c`） | 465,121 bytes（454 KB）                |
| 2026-08-25（`0543fd4`） | 696,036 bytes（680 KB）                |
| 2026-08-29（`1c39132`） | **856,275 bytes（836 KB）、13,172 行** |

**14 天 11.6 倍，線性，沒有平緩過。** 內容是 C1–C137（**C51 從未使用**）、
D1–D16、R1–R9。

而 `#88`（2026-08-20 建票）自己寫過：

> 機制在 `DECISIONS.md` 還有 439KB 的時候不可能通過

那個數字對得上（08-21 實測 454 KB）。**今天是它的 1.9 倍，而它在一半大小時
就已經被判定不可能。**

⚠️ **這組數字不推向搬遷，它推翻的是「什麼都不做」** —— 而那正是 `#195`
開票時的隱含預設。**否決路線 A 與維持現狀是兩件事，`#195` 原本把它們綁在一起。**

#### 三、裁決：凍結，不搬遷

**`DECISIONS.md` 到本則為止（C1–C138），C139 起寫進 `DECISIONS-2.md`。**

|                     |                                                                                                |
| ------------------- | ---------------------------------------------------------------------------------------------- |
| 約 2,700 處交叉引用 | **一處都不動** —— 它們全部指向 C1–C138，而那些留在原地                                         |
| 檔名 `DECISIONS.md` | **不改名** —— **44 個檔**提到這個檔名（`grep -rlF`，排除兩卷自己），改名會讓每一處指向錯的一卷 |
| 成長                | 當天停止                                                                                       |
| 搬遷成本            | 永遠不必付；要付也可以往後再決定                                                               |

形狀上有現成前例：**C133 停止維護 `release/v1` 改走 `main`，沒有回頭改寫歷史。**

⚠️ **這是 C135 §一 那一手的反向操作，而那正是「新位置」是真決定不是形式的證據。**
C135 §一 把 `#87` 要的 `CONTEXT.md` **溶掉**了 —— 產出寫進 `DECISIONS.md`，
不造新檔。本則反過來：**造一個新檔，好讓舊檔停下來。** 同一個問題兩個方向，
兩次都是人判的，沒有一次是機器答的。

#### 四、「凍結」這個前提站不站得住 —— 量過了

凍結的全部好處建立在一件事上：**舊則不會被回頭改**。實測最近 **25 支**
動到 `DECISIONS.md` 的 commit（`1c39132` 起回溯），看增刪行數與最早 hunk 位置：

|                         | 支數                                    |
| ----------------------- | --------------------------------------- |
| 刪 0 行（純插入）       | **20 / 25**                             |
| 最早 hunk 落在檔尾 ≥94% | **22 / 25**                             |
| 真正就地註記舊則        | **1 / 25**（`d04d780`／C136，落在 37%） |
| 落在 78%                | 1（`0904d79`／C133）                    |
| 分支合併，不是日常編輯  | 1（`1633550`／`#86` 併線，7275+/377-）  |

⚠️ **連那唯一一支就地的（C136）也是 292+/0-** —— 它在舊則裡**插入**一段警語，
沒有改寫任何一行既有文字。

而更關鍵的是體例本身：**這棵樹推翻舊裁決的主流做法就是開新則** ——
C134「收回」、C136「退回登記表」、C137「沒有分母」都是新則推翻舊則。
**凍結需要的紀律，這棵樹已經在用了。**

**那 1/25 的去處寫死**：C139 之後要註記 C1–C138 的某一則，
**寫在新一卷裡，舊卷最多加一行指標。**

⚠️ **推翻條件寫明白**：如果之後發現舊則其實常被就地改寫，本則失效，
**「留在原地、承受體積」就是預設答案。** 這一句是給推翻本則的人用的 ——
它要的證據是**重跑一次上面那張表**，不是換一把尺（D16 記過三次翻案，
每一次證據沒變、變的只有尺）。

#### 五、位置是新的根層檔，不是 `docs/adr/` —— 而這一格是人填的

⚠️ **這是本則最該被讀懂的一節。**

C135 §二 **明文拒絕回答** `docs/adr/` 能不能進樹；C136 §十 又特別警告
「這一則的收尾不是『`#87` 解鎖了』—— 那句話會被讀成 `#180` 批准了 `docs/adr/`」。
而 C93 的裁決是**沉默是舉證責任沒履行，不是預設通過**。

**所以本則選一個位置的時候，不能靜靜地選。** 選項被明白列出來問了人：

| 選項                   | 代價                                                                                 |
| ---------------------- | ------------------------------------------------------------------------------------ |
| **新的根層檔（採用）** | 體例、編號、C-條格式全部沿用；`SCOPE.md` 根層表加一列兩欄                            |
| `docs/adr/`            | 決定了 C135 §二／C136 §十 刻意留白的那一格；**而且換格式** —— `#87` 的重接成本回來了 |
| 先不凍結，只裁路線 A   | 不決定任何被拒答過的問題；代價是這個檔繼續以每天約 +55 KB 長                         |

**採用第一項的理由**：本則主張的是**凍結不搬遷**，而搬遷才要換格式，凍結不用。
`docs/adr/` 會把一個已被否決的成本（重接約 2,700 處）從後門帶回來。

⚠️ **所以 C135 §二 與 C136 §十 留白的那一格，本則沒有填，也沒有動到它。**
`docs/adr/` 今天仍然是**沒有人裁過**的。要造它，仍然要走 C93 那一關。

#### 六、本則沒有推翻任何 C-條 —— 而 C136 §八 仍然適用一半

依 C136 §八，推翻一則 C-條要寫明**它產出過什麼、重跑了沒有**。
**本則不推翻任何 C-條**，所以那一半不觸發。

但 §八 的另一半在這裡成立：本則**決定了一件別的則刻意留白的事**（`DECISIONS.md`
的續卷位置），所以照同一條精神寫出來 —— §五 就是那一節。

⚠️ 而本則**確實讓一句話變成假的**，見 §八。

#### 七、連帶改動，以及守它們的是什麼

| 改動                                               | 守它的                                         |
| -------------------------------------------------- | ---------------------------------------------- |
| `DECISIONS-2.md` 建檔（新的根層檔）                | `tools/scope-check` —— 不登記就紅              |
| `SCOPE.md` 根層表加一列 ＋ `DECISIONS.md` 那列改寫 | 同上（兩個方向都紅：樹上有沒列、列了樹上沒有） |
| `DECISIONS.md` 開頭加凍結告示                      | **沒有東西在守** —— 它是散文                   |

⚠️ **`DECISIONS-2.md` 不進 `doc-facts` 的 `GUARDED`**，理由與 `DECISIONS.md`
一字不差（`cli.ts:25`：守有日期的日誌等於要求改寫歷史）。
⚠️ 而 `cli.ts:28` 警告「`GUARDED` 加第三份檔案的那一刻，四個地方寫著
『只守 README 與 HANDOFF』的句子同時變成假的」—— **本則不加第四份，所以不觸發。**

⚠️ **在根層加東西被送到 `SCOPE.md` 登記，不是 AGENTS.md 規則二說的「改門檻」**——
`SCOPE.md` 開頭自己寫著「你在根層加東西的時候會被送到這裡」，那是這道閘門
**設計好的**路徑。它要的是「有人寫過一句它是什麼」，而不是准入。

#### 八、本則自己會讓什麼變成假的

⚠️ **`#195` 上那則留言說「裁決仍然沒有人做過」—— 本則一落地，那句話就是假的。**
所以本則要求在 `#195` 補一則指向 C138 的留言。這正是 C136 §七 記過的形狀
（同一句話在兩條線上一真一假，而沒有任何東西說話），差別只在**這一次是事前
就看見的**，所以寫成一項義務而不是一則事後檢討。

⚠️ **`#88` 的 Destination 無論路線裁成哪一條都要重寫** —— 已經記在那張票上：
「寫進 ADR 就算到了」今天不是一個可用的抵達條件。**本則不代替它重寫。**

⚠️ **本則不解鎖 `#87`。** `#87` 的驗收條件二今天「滿足於缺席」（C136 §十），
而它要的格式轉換正是 §一 否決掉的那一件。**它要先重問自己的前提。**
