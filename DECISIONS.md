...
沒有任何呼叫簽章的 export（`API_PREFIX`、`CONTRACT_ITEMS` 這類）的型別變更
被判成相容，只要求更新基準。

理由是常數的字面型別跟著內容跑：判成破壞性的話，每改一條設定就要人寫一份
不存在的 codemod，而那種紅燈會被關掉。代價是那類 export 少一個欄位不會被擋 ——
它仍然出現在基準檔的 diff 裡，但只靠人看。收緊得先能區分「內容變動」與
「形狀變動」，目前不划算（D16）。
