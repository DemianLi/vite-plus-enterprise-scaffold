...
### C51 — SAST：補的是 lint 做不到的那一半，而且它不是要交給稽核的那一份

原始需求寫的是「承受得住源碼掃描和弱點掃描」，而在此之前**自己寫的程式碼
一行 SAST 都沒有** —— Trivy 只掃相依，ESLint 的安全規則不是漏洞掃描器。

### 一、先確定它的角色，再決定規則

源碼檢測與弱點掃描由專業廠商執行，**那份報告才是稽核要的東西**。
所以這裡不是要取代它，是要讓廠商報告上少幾條。

這個定位直接決定了規則的取捨：**高精準、低噪音**。一道在開發流程裡亂叫的
閘門會被關掉，而被關掉的閘門連「少幾條」都做不到。

工具選 Semgrep 而不是 CodeQL：開源、可離線跑（與 D2 的退出論證同一個方向）、
規則可自訂成 repo 專屬。CodeQL 對私有 repo 要 GitHub Advanced Security 授權，
而那是一個要先問組織有沒有的成本。

### 二、只放 lint 做不到的那一類

lint 看的是**單一節點**：這一行有沒有用 eval、這個屬性是不是 v-html。
SAST 看得到的是**資料怎麼流** —— 路由參數進來，經過兩個函式，
最後流進 `innerHTML`，而每一行單獨看都沒問題。

所以規則只有三類：taint（路由參數 → DOM sink）、執行期組程式碼
（CSP `unsafe-eval` 的前提）、以及個資離開畫面。

### 三、第三條是**產生**的，而那是這套規則裡唯一通用掃描器做不到的

廠商的掃描器抓得到「有東西被寫進 localStorage」，但它**不知道哪些欄位是
個資** —— 那個知識只存在於本 repo 的 `Feature.personalData`。

手寫規則的話，加一個個資欄位就要記得回來改 YAML，而沒有人會記得。
所以 `.semgrep/generated-pii.yml` 由 `tools/sast` 產生、進版控、閘門比對 ——
與 `COMPLIANCE.md` 完全同一個形狀。

sink 選 log／localStorage／URL query 的共同點是**個資會離開畫面而且留下來**：
log 通常進第三方錯誤追蹤（跨境傳輸）、localStorage 登出也不會消失、
URL query 進瀏覽器歷史、Referer 與沿途每一個 access log —— 第三個最常被忽略，
而它是唯一會把個資送到**第三方伺服器**的那一個。

### 四、反向測試用 semgrep 自己的 fixture 格式

`.semgrep/fixtures/` 裡用 `ruleid:`／`ok:` 標記註明哪一行該命中、哪一行不該，
`semgrep --test` 驗這兩件事都成立。**那才是這套規則的反向測試。**

本地測試驗的是不需要 semgrep 也該成立的性質，而最重要的一條是
**每條規則都必須有 fixture** —— 少了它，加一條沒有 fixture 的規則會讓
`semgrep --test` 什麼都不驗而回綠，又是那個形狀。

還有一條驗「每條規則也要有 `ok:` 對照組」：只驗會命中的話，
一條寬到什麼都抓的規則會全綠，而誤報正是開發期閘門被關掉的主因。

### 五、對照組必須從它自己測的檢查裡豁免

fixture 裡**故意**有 `new Function()`、`setTimeout("字串")` 與
`el.innerHTML = x` —— 於是 oxlint 的 `no-implied-eval` 與 ESLint 的
`no-unsanitized/property` 當場對它開火，兩邊都紅。

兩邊都加了 `.semgrep/fixtures/**` 豁免。這與 `tools/pii-check` 的 `EXEMPT`
是同一條規矩：**一份為了證明檢查有效而刻意違規的檔案，必須從那個檢查裡
豁免，而豁免範圍要剛好等於那一份。**

### 六、撞到的坑：把工具鏈套進它自己裡面

同步檢查要經過 `vp fmt`（formatter 才是權威，這個 repo 第三次上這一課）。
我把那個比對也寫成一條測試 —— 於是 `vp run -r test` 底下又 spawn 了一次 `vp`。

症狀很難看懂：**單獨跑 `vp run -F @org/sast test` 全綠，放進 `vpr ready`
就有另一個 package 的測試以看不出原因的方式失敗**（一個正則抓不到輸出，
報 `expected NaN`）。

閘門本來就會跑那個比對，測試再跑一次只是多一個併發來源。拿掉了。

### 七、⚠️ semgrep 不在 `pnpm-lock.yaml` 裡

它是 Python 工具，以 pinned 的 container image 進來 —— **`tools/supply-chain`
的盤點看不到它**，SBOM 也不會有它。

這與 C48 那次「happy-dom 是純 JS，不進原生家族分類」不同：那次是量得到的
（144 → 144），這次是**整個在盤點範圍之外**。它是一個獨立於本 repo 供應鏈
論證之外的信任決定，已寫進 HANDOFF。
