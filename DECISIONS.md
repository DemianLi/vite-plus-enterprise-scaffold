...
建置成功、上游 Vitest **98 個測試全過**、應用程式原始碼一字未改。
...
**Pinia 不在裡面** —— 2026-08-15 實測退到上游 Vite 8.2.1，98 個測試全過，Pinia 一個字沒改。
...
| R9  | ~~D2 的退出保單未經測試~~ **已解除**：2026-08-15 實測退到上游 Vite 8.2.1，建置與 98 個測試全過       | —          | 已自動化為每季演練 + 每次 PR 的退出面檢查（`tools/exit-drill`）                      |
...
產物大小與 vite-plus 建置一致），上游 Vitest **98 個測試全過**，
**應用程式原始碼一字未改**，全程 17 秒（含 npm install）。
...
最新一次全套（2026-08-15）：**`vpr ready` exit 0**、129 檔案格式一致、
69 檔案 0 errors 0 warnings、**250 tests / 17 檔案全過**、建置 113 modules 成功。

| 項目                                   | 結果                                                                                                     |
| -------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `vp check`（Tier 1）                   | 0 errors 0 warnings，129 檔案格式一致（69 檔案型別與 lint 檢查）                                         |
| `vp run -r test`                       | **250 tests 全過**（17 個測試檔）                                                                        |
...
| **D2 完整退出演練**（R1／R9）          | ✓ 上游 **Vite 8.2.1** 建置成功、上游 Vitest **98 tests 全過**、原始碼未改                                |
