 ...
 * 驗收規格的目錄，**與 `src/` 平行**。
 *
 * ⚠️ 刻意不放在 `tests/` 底下：`.feature` 是**需求**，是人寫給 agent 讀的東西，
 * 不是測試。放進 `tests/` 會讓它讀起來像「開發者的資產」，而它要能被 PM 與
 * 驗收方直接打開來讀 —— 那是它唯一的價值。
 */
 ...
 *
 * 這正是本 repo 反覆記錄過的「寫了但永遠無效」，只是換到新的一層發作。
 */
export const USECASES_DIR = "src/usecases";

/**
 * usecase 層不得出現的 import —— 純度就是這一層的全部價值。
 *
...
/** 上面那個下限套用的範圍。設定檔一律從這裡取，不要重抄一份字串。 */
export const USECASE_COVERAGE_GLOB = `${USECASES_DIR}/**`;
