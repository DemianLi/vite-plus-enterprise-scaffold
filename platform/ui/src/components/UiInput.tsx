 ...
 * 而那正是 theme-verify 在防的那種「看起來有做」。
 *
 * ── 代幣對照 ⚠️ **這張表是人工核對的，沒有閘門在守** ─────────────────
 *
 * 實測過：把 `border-line` 改回上游的 `border-input`，theme-verify 仍然全綠
 * （「0 處原始顏色、0 處懸空引用」）。未翻譯的上游代幣既不是原始色、也不是懸空引用 ——
 * 它是一個「合法但不是我們的」名字，而 palette.ts 現有的兩類違規都認不得它（issue #57）。
 ...
 *
 * ⚠️ **React 沒有屬性穿透，所以收得下的屬性在下面逐一列出**（同 C235 對 `UiButton` 的
 * 處置）。`id` 與兩個 `aria-*` 是 `UiField` 交出的 `control` 那三格，少一格接線就斷、
 * 畫面照常 —— `tests/field-wiring-react.test.ts` 守著。要第二種屬性時加在這裡，
 * `api-surface` 記成相容變更。不給 `value` 就是非受控。
 */
