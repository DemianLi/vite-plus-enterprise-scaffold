 ...
 *
 * C81 一開始判它是「`UiDialog` 的變體：`footer` 放兩顆鈕就好」，讀了基元的原始碼之後推翻。
 * 輔具聽到的是 `alertdialog` 不是 `dialog`、**點外面不會關**（Esc 仍然會）、而且**初始焦點
 * 落在「取消」**。最後一項是安全設計：焦點若落在「確認」，一個 Enter 就刪掉了 —— 而畫面上
 * 完全看不出差別。四件事都在 `tests/alert-dialog-react.test.ts` 裡量。
 *
 * 同 C78 §3 的 `UiSwitch` vs `UiCheckbox`：**差別是語意不是外觀，而選錯的代價由使用者付。**
 *
 * ── ⚠️ 初始焦點靠的是按鈕列的順序 ─────────────────────────────────────
 *
 * Base UI 的 `Popup` 預設聚焦**內容裡第一個可聚焦元素**。實測在 `children` 裡放一個連結，
 * 焦點就落在連結上（reka 版同一個情境落在取消：它靠 `AlertDialogCancel` 在掛載時把自己登記給
 * content，與 DOM 順序無關，C237）。所以這裡用 `initialFocus` 指到**按鈕列裡的第一顆**，也就是
 * 取消。⚠️ 這條替身與預設那條靠的是同一個順序（取消寫在確認前面）：它補的是「`children` 裡有
 * 可聚焦的東西」那一格，不是與順序無關的保險 —— 對調兩顆，兩條路一起落到確認上。兩條各有一條測試。
 *
 ...
 * （同 C82 記下的形狀）。兩顆按鈕的樣式走 `UiButton` 既有的那幾格，不在這裡另開可覆寫的按鈕格 ——
 ...
 * - `confirmVariant` 預設 `danger`，因為 C81 判它進範圍的理由原句就是「CRUD 的 D 就是它」。
