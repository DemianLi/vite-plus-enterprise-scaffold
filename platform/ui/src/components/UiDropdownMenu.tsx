 ...
 *       見 `tests/dropdown-menu-react.test.ts` 的「★ 名字的載體還在 DOM 裡」。
 *
 ...
 *
 * ⚠️ 這個形狀（span 直接包住 `{label}`、class 只有一個 token）被 `tests/a11y.test.ts`
 * 抽 candidate 的正則與 `tests/dropdown-menu-react.test.ts` 的「★ 名字的載體」各自釘住。
 ...
 * 一個徽章或 sr-only 前綴，然後首字母跳轉安靜地對不上」。而首字母跳轉正是 C81 判它進範圍的
 * 四件事之一。`Menu.Item` 的 `label` 釘住跳轉讀的字：今天項目的文字就是 `label`，所以它沒有
 * 行為（reka 版的同一行變異驗過零條紅；Base UI 版沒量）。留著是因為日後在項目裡加快捷鍵提示、
 * 圖示時，跳轉讀的仍然是這個字。記成「現在是註解、將來是行為」，不假裝它有閘門在守。
 *
 * `items` 的型別刻意**內嵌**而不是抽成 `UiDropdownMenuItem` 匯出：契約 ④ 禁止 props 的 union
 * 用型別別名。同 `UiSelect` 的 `items`。`align` 預設 `end` ——「⋯」在表格列的右緣，
 * 從左緣展開會掉出容器。
 *
 * ── ⚠️ 選一項就會關，而且沒有「執行中」狀態 ────────────────────────
 *
 * 所以「按刪除 → 送請求 → 選單裡顯示 spinner」做不到 —— 需要那個的案子要的是
 * `UiAlertDialog` 或 `UiDialog`，不是選單。與 C86 記的「兩顆按鈕都會關掉」同一個形狀。
 * `onSelect` 被呼叫的那一刻選單還開著，關閉在那之後 —— 在處理器裡同步改 `open` 會被蓋掉。
 *
 * ── 對 Base UI 量過的（`tests/dropdown-menu-react.test.ts`）──────────────
 *
...
const DEFAULT_PARTS: Readonly<Record<UiDropdownMenuSlot, string>> = {
  ...
  // 寫法今天等價 —— ⚠️ 而**沒有任何測試分得出來**（reka 版變異驗過，改成 `focus:` 零條紅）。
  item: cn(
