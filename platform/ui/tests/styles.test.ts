...
describe("去註解器本身", () => {
  it("不會把 glob 裡的 /**/ 當成空註解吃掉", () => {
    // 這是實際踩過的坑：@source "../../../../**/*.{ts,tsx}" 裡的 /**/ 是一個
    // 合法的 CSS 空註解，天真的去註解器會把路徑少刨掉一層 ——
    // 而測試看到的宣告與 Tailwind 看到的變成兩回事，兩邊都不報錯。
    expect(declaredSources(entry())).toContain("../../../../**/*.{ts,tsx}");
  });
...
describe("公開契約", () => {
  it("不轉出基元", () => {
    // 直接轉出等於把「哪些基元可以用」交給每個團隊各自決定。Base UI 今天不注入
    // <style>（base-ui-no-style.test.ts），但哪一版開始注入，style-src 'self' 會安靜擋掉它。
    const index = readFileSync(join(PACKAGE_ROOT, "src/index.ts"), "utf8");
    expect(index).not.toContain('from "@base-ui/react');
    expect(index).not.toContain('from "react-day-picker');
